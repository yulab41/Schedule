...
const logs = [
  {
    createdAt: '2026-08-25 18:42',
    businessMonth: '2026-08',
    clientIp: '203.0.113.10',
    requestId: 'req-9a2…',
  },
  {
    createdAt: '2026-08-25 16:18',
    businessMonth: '2026-09',
    clientIp: '203.0.113.••',
    requestId: 'req-81c…',
  },
  {
    createdAt: '2026-08-24 09:07',
    businessMonth: '2026-08',
    clientIp: '203.0.113.••',
    requestId: 'req-43b…',
  },
...
<template>
  <main
    class="visitor-golden"
    :class="{ 'is-large-text': largeText }"
    aria-label="访客访问审计黄金稿"
  >
    <header class="visitor-topbar">
      <div class="visitor-brand">
        <span class="brand-mark" aria-hidden="true">+</span><span>排班台</span>
      </div>
    ...
    >
      <header class="section-heading">
        <div>
          <p class="section-kicker">访问脉搏</p>
          <h2 id="aggregate-title">近四个月访问次数</h2>
        </div>
    ...
    <section class="logs-card" aria-labelledby="logs-title">
      ...
      <div v-else-if="state === 'error'" class="state-panel state-panel--error" role="alert">
        <span class="state-icon">!</span><strong>访问记录暂时无法加载</strong>
        <p>请检查网络后重试，当前不会写入任何访问数据。</p>
        <button type="button" @click="showFeedback('预览状态：重新加载')">重新加载</button>
      </div>
      <div v-else-if="state === 'disabled'" class="state-panel state-panel--disabled">
        <span class="state-icon">i</span><strong>审计功能尚未开放</strong>
        <p>当前版本只保留排班核心能力，访客访问记录将在 RC 后开放。</p>
      </div>
      <div v-else-if="state === 'empty'" class="state-panel">
        <span class="state-icon">日</span><strong>暂无访客访问记录</strong>
        <p>访客查看排班后，记录会出现在这里。</p>
      </div>
      <div v-else class="log-list">
        <article v-for="log in visibleLogs" :key="log.requestId" class="log-row">
          ...
          <div class="log-main">
            ...
            ><span>查看 {{ log.businessMonth }} 排班</span>
          </div>
          <div class="log-meta">
      ...
      <footer v-if="state === 'ready'" class="logs-footer">
        <span>显示 {{ visibleLogs.length }} 条记录</span
        ><button type="button" @click="showFeedback('预览状态：加载下一页')">加载更多记录</button>
      </footer>
    ...
    <p class="visitor-privacy">
      ...
      仅群主和管理员可见；来源线索按权限脱敏，访客凭证不会出现在记录中。
    </p>
  </main>
</template>
...
.aggregate-card {
  background: linear-gradient(135deg, var(--blue-soft), #fff 72%);
}
.logs-card {
  margin-top: 14px;
}
...
.aggregate-chart {
  ...
  gap: 15px;
  border-bottom: 1px solid rgb(10 102 213 / 20%);
}
.aggregate-column {
  display: flex;
  height: 100%;
...
.aggregate-bar {
  width: 28px;
  min-height: 10px;
  background: linear-gradient(180deg, var(--blue), #77b7f9);
  border-radius: 8px 8px 3px 3px;
}
.aggregate-column:last-child .aggregate-bar {
  background: linear-gradient(180deg, #378c7a, #9ad9c4);
}
.aggregate-note,
.visitor-privacy {
  margin: 12px 2px 0;
...
.state-panel--disabled .state-icon {
  color: #7a5310;
  background: #fff5d9;
}
.log-skeletons {
...
.log-skeleton {
  display: block;
  height: 58px;
  background: linear-gradient(90deg, #eef3f8, #fafcfe, #eef3f8);
  border-radius: 12px;
}
.feedback {
  margin: 12px 0 0;
...
.is-large-text .visitor-heading p:not(.eyebrow),
.is-large-text .aggregate-note,
.is-large-text .visitor-privacy {
  font-size: var(--ui-font-size-sm);
}
