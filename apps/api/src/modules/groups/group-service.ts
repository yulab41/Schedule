...
export class GroupService {
  ...
  public async create(
    ...
    return runOrganizationMutation({
      ...
      requestFingerprint: createOrganizationFingerprint({ name: input.name }),
      run: (transaction, actor) => this.createGroup(transaction, actor, input.name),
  ...
  private async createGroup(
    ...
    return toGroupSummary(
      { id: groupId, name, ownerUserId: actor.id, version: 1 },
      'owner',
