...
export function getBusinessMonthsForWeek(
  ...
  const lastMonth = getBusinessMonthOf(days.at(-1)!);
...
export function getWeekLabel(businessDate: string): string {
  ...
  const end = parseBusinessDate(days.at(-1)!);
...
export function getVisibleWeekForMonth(businessMonth: string, today: string): string {
  const { year, month } = (() => {
    ...
    return { month: Number(monthText), year: Number(yearText) };
  })();
  parseBusinessDate(today);
  if (today.slice(0, 7) === businessMonth) {
    return getWeekStartDate(today);
  }
  return getWeekStartDate(`${String(year).padStart(4, '0')}-${String(month).padStart(2, '0')}-01`);
}
...
export function formatChinaDateTime(value: string): string {
  ...
  return (
    formatDate(new Date(date.getTime() + 8 * 60 * 60 * 1000)) +
    ` ${String(new Date(date.getTime() + 8 * 60 * 60 * 1000).getUTCHours()).padStart(2, '0')}:${String(new Date(date.getTime() + 8 * 60 * 60 * 1000).getUTCMinutes()).padStart(2, '0')}`
  );
}
