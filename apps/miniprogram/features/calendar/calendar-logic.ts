...
export function getCurrentBusinessDate(now = new Date()): string {
  return formatDate(new Date(now.getTime() + cstOffset));
}
...
export function formatChinaStandardTime(value: string): string {
  ...
  const shifted = new Date(date.getTime() + cstOffset);
...
export function sortCalendarAssignments(
  ...
  const getStartMinutes = (value: CalendarDutyAssignment): number => {
    const [hours, minutes] = formatChinaStandardTime(value.startsAt).split(':').map(Number);
  ...
  return values
    ...
    .sort(
      (first, second) =>
        ...
        getStartMinutes(first.value) - getStartMinutes(second.value) ||
