...
验收候选固定为 `0.1.0-p8.20260825.4`。生产 `organization=false`，所以本轮只在专用测试环境/专用测试群组验证写入；正式生产入口和 API 必须保持失败关闭。邀请 token、visitor key、二维码内容、绑定 URL、完整联系方式、密码和 subject 不得写入 Mini storage、相册、日志或离线队列。
...
3. 记录设备型号、Android、微信、基础库、系统字体缩放和页面 `buildLabel`，确认与 `0.1.0-p8.20260825.4` 一致。
