...
本清单只由用户在实体 Android 的微信原生运行时执行。Storybook、Vitest、`miniprogram-simulate`、桌面 JS 墙钟和 `miniprogram-ci` 均不能代替这些数据。Codex 可以操作微信开发者工具 GUI/CLI 做编译与截图预检，但预检结果不是本清单的一部分。
