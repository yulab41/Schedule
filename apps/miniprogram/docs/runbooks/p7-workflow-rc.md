...
本节是独立诊断门槛，不重开完整 P7 业务 RC。候选固定为
`0.1.0-p9.20260828.63`；必须先核对页面 `buildLabel` 与交付报告一致。该候选只增加
`UiWheelColumn + gesture-probe E`，生产年月选择器继续保持 `.51` 运行语义。

进入“Android 手势能力探针”，在 E 区按顺序执行：

1. 慢速向下一格后立即向上一格，连续 20 次；不得出现手指移动但数字静止。
2. 分别停在半格上方和下方后反向；“实时目标”必须随中心数字一致变化。
3. 在吸附尚未结束时反向拖动；新手势必须立即接管，不得被旧方向拉回。
4. 连续快速 flick，再点相邻行并立即拖动；每次松手只能发生一次归位。
5. 到达顶部/底部各 10 次；不得越界、回弹锁死或出现非有限样式。
6. 点击“重置滚轮”10 次；generation 每次递增，实时目标/最终静止都回到 2026年，随后仍可拖动。

同时观察数字在中心线附近逐像素放大/缩小、透明度连续变化，静止后中心数字、实时目标和最终
静止三者必须一致。失败时只需回复：

```text
buildLabel / device / Android / WeChat / baseLibrary / fontScale:
case: slow-reverse | half-reverse | snap-interrupt | flick-tap | boundary | generation-reset
symptom:
```

本节明确通过前不得把 WXS 组件接入生产 workflow picker；模拟器和自动测试不能代替此结论。

用户已在 `.63` 的目标 Android 明确确认本节全部核心效果正常且符合预期，阶段 A 门槛已通过。

## 2026-08-28 WXS 业务年月双滚轮

本节验收 production-profile 体验版 `0.1.0-p9.20260828.65`。该候选把同一
`UiWheelColumn + wheel-gesture.wxs` 组合为业务年份/月度双列；旧 scroll、CSS snap、逻辑层 timer
与逐帧 setData 已删除。日期日历和普通 selector 不在本节变化范围。

在请假、换班、加扣班任一会打开月份选择器的入口执行：

1. 年/月两列分别慢速下一格后立即上一格各 20 次，数字必须从第一段有效 move 跟手。
2. 分别在半格和吸附中反向；不得停住、跳回旧方向或发生第二次吸附。
3. 两列交替快速 flick、点相邻行后立即拖动、顶/底边界各 10 次。
4. 核对两列中心数字、标题摘要和完成后的 `YYYY-MM` 一致；字号与透明度逐像素连续变化。
5. 吸附过程中点击“完成”提交当前投影目标且只产生一次 change；点击“取消”不得改变业务值。
6. 关闭重开 10 次并交替打开多个 picker；不得串列、串实例或继承旧动画。

失败时反馈 `buildLabel + 入口 + year|month + slow-reverse|snap-interrupt|flick|tap|boundary|confirm`
及现象。全部通过后回复“P7 WXS 业务滚轮通过”；该反馈不等同于完整 P7 工作流重新验收，也不触发
审核或正式发布。

验收候选固定为 `0.1.0-p7.20260824.94`。先核对工作台可见 `buildLabel` 与本轮报告完全一致；`.82-leave/.83-swap/.84-duty` 是失败关闭的部分候选，`.85/.86/.87/.88/.89/.90/.91/.92/.93` 已被本轮 Web/Mini 自然日限制、请假 Sheet/冲突列表、无闪烁下拉、平滑滚轮、日期横滑定位和周视图字重修复版取代，均不用于本轮验收。体验版使用 production profile，会写入正式服务器，因此只在专用测试群组和未来日期排班中操作，禁止修改真实在用排班。
