...
验收候选固定为 `0.1.0-p7.20260824.85`。先核对工作台可见 `buildLabel` 与本轮报告完全一致；`.82-leave/.83-swap/.84-duty` 是失败关闭的部分候选，不用于验收。体验版使用 production profile，会写入正式服务器，因此只在专用测试群组和未来日期排班中操作，禁止修改真实在用排班。
