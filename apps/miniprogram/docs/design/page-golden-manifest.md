...
2026-08-27 自动验收策略：用户已明确“无需人工复核”，本轮人工复核豁免。已实现的 P7–P10 切片以对应自动回归、确定性构建、包边界、体验上传和生产 verifier 作为验收证据；“自动 verifier 已通过”不等价于提交审核或正式发布，P9 `insights`/`externalMessages` 仍按生产策略保持关闭。
...
| P10-A2/A3 | `subpackage-organization` 原生院内/员工通讯录                                                               | `miniprogram-parity-p10-directory-parity--internal-ready-390` / `--internal-ready-320` / `--employee-ready-390` / `--large-text-390` / `--loading-320` / `--empty-390` / `--error-320` / `--disabled-390` | 合成目录 facets、七级筛选、游标结果、号码隐私；ready/loading/empty/error/organization-disabled；原生页面已实现 | 黄金已固化       | 黄金已固化           | 自动 verifier 已通过 | Web Storybook 已完成；人工复核按本轮用户要求豁免 |
| P10-A4 | 主包 `pages/profile/index` 原生个人中心                                                                        | Web `web-ui-2-0-production-·-我的--mobile` / `--desktop`                                                                 | 登录态姓名/认证方式/按认证方式显示解绑/切换/退出；未登录空态与大字号；不读取真实业务统计                                                                 | Web 已固化       | Web 已固化           | 自动 verifier 已通过        | 体验版 `.35`；人工复核按本轮用户要求豁免 |
