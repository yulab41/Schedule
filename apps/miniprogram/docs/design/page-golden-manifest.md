...
| P1           | 动态月历                                                                                                                            | `miniprogram-parity-p1-calendar--month-390` / `--month-320`                                                                                                                                                                                                                                                                           | `pages/workbench/index` 月视图；5/6 周/今天/选择/底角/跨月/变更                                                                                                             | 已更新            | 已更新               | 待人工复测           | 已对齐当前 Web                                              |
| P1           | 7×7 矩阵                                                                                                                            | `miniprogram-parity-p1-manual-matrix--daily-390` / `--daily-320`                                                                                                                                                                                                                                                                      | `subpackages/scheduling/pages/manual/index`（7 人左右群组）；日常数据/冻结/滚动/单格更新                                                                                    | 已截图            | 已截图               | 待人工测试           | 已通过 Web 黄金                                             |
| P1           | 20×30 矩阵                                                                                                                          | `miniprogram-parity-p1-manual-matrix--maximum-390` / `--maximum-320`                                                                                                                                                                                                                                                                  | `subpackages/scheduling/pages/manual/index`（20 人 × 30 天群组）；最大数据/失效/撤销/滚动                                                                                   | 已截图            | 已截图               | 待人工测试           | 已通过 Web 黄金                                             |
| P3           | `pages/identity/index` 账号密码登录/微信快捷登录/密码绑定/建档；`pages/admin-bind/preview` 成员/管理员绑定；`pages/guest-entry/index` 独立访客码入口（保持现状）；`pages/identity/unbind` 解绑 | `miniprogram-parity-p3-identity-security--mini-login-390` / `--mini-login-320` / `--mini-link-password` / `--mini-register-profile` / `--mini-admin-link-preview` / `--mini-admin-link-confirm` / `--mini-unbind-confirm` / `--mini-unbind-confirm-320`                                                                               | Web 同源账号密码表单、微信已绑定成员快捷登录、`link_required`、既有账号、首次真实姓名、成员码完整姓名/工号预览与一次确认、平台链接脱敏预览、独立访客页入口、authenticated、当前 AppID 解绑确认          | 已确认            | 已确认               | 待人工测试           | 绑定页无访客入口；Chrome 390/320 几何代理通过，绑定页同 SHA 体验版待小米 14 复核        |
...
| EXP-FEAT-002 | P4 工作台班次详情事件记录                                                                                                           | 复用 `miniprogram-parity-p9-insights-suite--events-ready-390` 的事件卡片参考；本轮不新建第二套 Storybook 黄金                                                                                                                                                                                                                         | 有权限班次；共享 `ui-sheet`；真实 `insights.events` GET；升序时间线；loading/empty/error/retry；无权与切班次清理                                                            | 复用 Web 事件黄金 | 复用 Web 边界        | 待下一版 Xiaomi 14   | 本轮 Node/静态验证完成；原生待用户复核                      |
...
| P10-A2/A3    | `subpackage-organization` 原生科室/人员通讯录                                                                                       | 完整切换器对照 `web-ui-2-0-unified-directory--department-mobile-390` / `--people-mobile-390` / `--mobile-320`；状态及半屏筛选对照 `miniprogram-parity-p10-directory-parity--large-text-390` / `--loading-320` / `--empty-390` / `--error-320` / `--disabled-390` / `--half-filter-sheet-390` / `--half-filter-sheet-320`              | 双页 facets/收藏索引轻量预热、点击与横划、独立搜索/筛选/滚动状态、动态半屏筛选、折叠筛选层、游标结果、卡片分割线、号码隐私；ready/loading/empty/error/organization-disabled | 黄金已固化        | 黄金已固化           | 自动 verifier 已通过 | 当前 checkpoint 待小米 14 半屏、横划、滚动恢复和诊断复核    |
| P10-A4       | 主包 `pages/profile/index` 原生个人中心                                                                                             | Web `web-ui-2-0-production-·-我的--mobile` / `--desktop`                                                                                                                                                                                                                                                                              | Web 1:1 标题/身份/值班概览/四月节奏/下一班/账户/退出；头像有无、绑定/未绑定、成员/访客/无群组、loading/error/部分失败、密码 proof、390/320/大字号/减少动态                  | Web 黄金已固化    | Web 边界已固化       | 待实体 Android 复核  | `.60` 对照 Web 黄金完成最终视觉、头像、密码与导航复核       |
| Audit-T1     | `subpackages/diagnostics/pages/test-tools/index` 安全测试工具                                                                       | `miniprogram-audit-test-tools--ready-390` / `--ready-320` / `--large-text-390`                                                                                                                                                                                                                                                        | develop/trial；首屏构建校准、Grid/CSS变量/scroll-view探针、CSS/SVG图形对照和复制入口；下方保留设备、显示、性能、场景、网络、错误、存储及完整报告；release失败关闭           | 已复核；无横溢    | 已复核；无横溢       | 待异常实例体验版复核 | Web 黄金仅辅助，最终以匹配 SHA 的异常实例报告和截图为准     |
...
`pages/guest/guest`沿用P4工作台的ready/filter/week/list/empty/error日历结构与样式及calendar-month叶组件，移除电话、事件和变更筛选；自定义导航栏按状态栏/胶囊安全区承载独立扫码页返回，不改变已登录工作台。fixture为guest-runtime合成只读日历；390/320桌面及小米14原生结果单独记录于docs/audit/visitor-system-fix.md。用户已批准本轮完整方案；没有新增视觉方向或认证协议，原生效果待同一新版体验版复核。
