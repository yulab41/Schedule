...
| P3           | `pages/identity/index` 账号密码登录/微信快捷登录/密码绑定/建档；`pages/admin-bind/preview` 成员/管理员绑定；`pages/guest-entry/index` 访客码入口；`pages/identity/unbind` 解绑 | `miniprogram-parity-p3-identity-security--mini-login-390` / `--mini-login-320` / `--mini-link-password` / `--mini-register-profile` / `--mini-admin-link-preview` / `--mini-admin-link-confirm` / `--mini-unbind-confirm` / `--mini-unbind-confirm-320`                                                                               | Web 同源账号密码表单、微信已绑定成员快捷登录、`link_required`、既有账号、首次真实姓名、成员码完整姓名/工号预览与一次确认、平台链接脱敏预览、访客码入口、authenticated、当前 AppID 解绑确认          | 已确认            | 已确认               | 待人工测试           | 登录黄金已确认；绑定页按本批重构，390×844/320 视觉复核待有效截图        |
