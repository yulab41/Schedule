...
| P10-A2/A3    | `subpackage-organization` 原生科室/人员通讯录                                                                                       | 完整切换器对照 `web-ui-2-0-unified-directory--department-mobile-390` / `--people-mobile-390` / `--mobile-320`；状态及半屏筛选对照 `miniprogram-parity-p10-directory-parity--large-text-390` / `--loading-320` / `--empty-390` / `--error-320` / `--disabled-390` / `--half-filter-sheet-390` / `--half-filter-sheet-320`              | 双页 facets/收藏索引轻量预热、点击与横划、独立搜索/筛选/滚动状态、动态半屏筛选、折叠筛选层、游标结果、卡片分割线、号码隐私；ready/loading/empty/error/organization-disabled | 黄金已固化        | 黄金已固化           | 自动 verifier 已通过 | 当前 checkpoint 待小米 14 半屏、横划和滚动恢复复核    |
| P10-A4       | 工作台内嵌原生个人中心                                                                                             | Web `web-ui-2-0-production-·-我的--mobile` / `--desktop`                                                                                                                                                                                                                                                                              | Web 1:1 标题/身份/值班概览/四月节奏/下一班/账户/退出；头像有无、绑定/未绑定、成员/访客/无群组、loading/error/部分失败、密码 proof、390/320/大字号/减少动态                  | Web 黄金已固化    | Web 边界已固化       | 待实体 Android 复核  | `.60` 对照 Web 黄金完成最终视觉、头像、密码与导航复核       |
| Audit-T1     | `测试工具源码（正式包排除）`                                                                       | `miniprogram-audit-test-tools--ready-390` / `--ready-320` / `--large-text-390`                                                                                                                                                                                                                                                        | 源码保留、正式产物排除；首屏构建校准、Grid/CSS变量/scroll-view探针、CSS/SVG图形对照和复制入口；下方保留设备、显示、性能、场景、网络、错误、存储及完整报告；release失败关闭           | 已复核；无横溢    | 已复核；无横溢       | 当前正式包不验收 | Web 黄金仅辅助，最终以匹配 SHA 的异常实例报告和截图为准     |
