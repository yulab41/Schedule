...
| P8-B | `subpackage-organization` 群组/成员/配置/邀请访客/平台账号                                                     | `miniprogram-parity-p8-organization-parity--*`；34 个精确 ID 见下方                                                                                                                                                                                                                                                                   | production panels；群主/管理员/成员/后台/平台管理员；ready/loading/empty/error/conflict/confirm/success/disabled；大字号                         | 390 全状态已固化 | 320/大字号边界已固化 | 待人工测试        | 原生页面已实现；待体验版与实体 Android RC |
...
| P9-A5 | `client-core` 事件时间线与排班统计只读边界                                                               | 由原生 P9-A7 `subpackages/insights/pages/insights/index` 消费；对照 P9-A4 `events-ready-390` / `statistics-ready-390`                                                                                 | 事件列表/详情、按月/按年统计；严格 decoder、Bearer、URI 编码；共享边界已接入原生页面                                                                 | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | 共享边界与原生页面已实现；待体验版与实体机验收 |
| P9-A6 | Mini runtime 事件/统计 transport bridge                                                                    | 由原生 P9-A7 `subpackages/insights/pages/insights/index` 消费；复用 P9-A5 `InsightsReadClient`                                                                                                     | `insights` capability resolver、Bearer 单飞会话恢复、平台 transport；不读取业务数据                                                                 | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | runtime bridge 与原生页面已实现；待体验版与实体机验收 |
| P9-A7 | `subpackage-insights` 事件时间线与排班统计只读页                                                         | 原生 route `subpackages/insights/pages/insights/index`；对照 `miniprogram-parity-p9-insights-suite--events-ready-390` / `--statistics-ready-390`                                                | 事件/统计 tab、按月统计、岗位完成度、loading/empty/error/insights-disabled；原始 payload 与身份标识不进入页面状态                         | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | 原生页面已实现；待体验版与实体机验收 |
| P9-A8 | `client-core` 通知中心与导出任务 JSON 边界                                                            | 由原生 P9-A10/P9-A11 `notifications/index` 与 `exports/index` 消费；对照 `miniprogram-parity-p9-insights-suite--notifications-ready-390` / `--export-ready-390`                                      | 通知列表/未读/已读、导出任务创建/状态；二进制下载留给 Mini platform adapter，不进入共享包                                                        | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | 共享 JSON 边界与原生页面已实现；待体验版与实体机验收 |
| P9-A9 | Mini runtime 通知/导出任务 transport bridge                                                            | 由原生 P9-A10/P9-A11 消费；复用 P9-A8 `P9InsightsActionsClient`                                                                                                                                   | `insights` capability、Bearer transport、通知/导出 JSON 任务；二进制下载仍不在本批                                                                 | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | runtime bridge 与原生页面已实现；待体验版与实体机验收 |
| P9-A10 | `subpackage-insights` 原生通知中心                                                                        | 原生 route `subpackages/insights/pages/notifications/index`；对照 `miniprogram-parity-p9-insights-suite--notifications-ready-390`                                                | 未读数、单条/全部已读、加载更多、loading/empty/error/insights-disabled；正文不写缓存/日志                                                        | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | 原生页面已实现；待体验版与实体机验收 |
| P9-A11 | `subpackage-insights` 原生导出任务与登录态下载                                                            | 原生 route `subpackages/insights/pages/exports/index`；对照 `miniprogram-parity-p9-insights-suite--export-ready-390`                                                   | 创建任务、状态轮询、下载文件、openDocument、insights-disabled；URL 不携带 token，不自动写相册/缓存                                      | Web 黄金已固化 | Web 黄金已固化      | 待人工测试       | 原生页面已实现；待体验版与实体机验收 |
| P10-A2/A3 | `subpackage-organization` 原生院内/员工通讯录                                                               | `miniprogram-parity-p10-directory-parity--internal-ready-390` / `--internal-ready-320` / `--employee-ready-390` / `--large-text-390` / `--loading-320` / `--empty-390` / `--error-320` / `--disabled-390` | 合成目录 facets、七级筛选、游标结果、号码隐私；ready/loading/empty/error/organization-disabled；原生页面已实现 | 黄金已固化       | 黄金已固化           | 待人工测试        | Web Storybook 已完成；原生体验版待验收 |
| P10-A4 | 主包 `pages/profile/index` 原生个人中心                                                                        | Web `web-ui-2-0-production-·-我的--mobile` / `--desktop`                                                                 | 登录态姓名/认证方式/按认证方式显示解绑/切换/退出；未登录空态与大字号；不读取真实业务统计                                                                 | Web 已固化       | Web 已固化           | 待人工测试        | 体验版 `.15` 待实体 Android RC |
