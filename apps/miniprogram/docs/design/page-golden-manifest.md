...
| P10-A2 | `subpackage-organization` 原生院内/员工通讯录                                                               | `miniprogram-parity-p10-directory-parity--internal-ready-390` / `--internal-ready-320` / `--employee-ready-390` / `--large-text-390` / `--loading-320` / `--empty-390` / `--error-320` / `--disabled-390` | 合成目录 facets、七级筛选、游标结果、号码隐私；ready/loading/empty/error/organization-disabled；原生页面待实现 | 黄金已固化       | 黄金已固化           | 待人工测试        | Web Storybook 已完成；原生待验收 |
