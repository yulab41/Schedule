...
- 已实现 `ManualScheduleCell` 与矩阵 PoC：`pages/manual-matrix-poc/index?mode=daily|maximum` 分别生成 7×7 和 20×30 确定性数据；日期与班次组合为一个完整宽度的原生横向内容项，人员与班次共用一个纵向 SharedValue，左上角固定。两种模式都使用 390px 固定视口（82px 表头 + 7 个 44px 人员行），页面级滚动关闭；单一原生滚动代理直接锁定横/纵轴，横向保留 compositor，纵向由代理自身 Worklet 更新人员/班次，不再依赖嵌套 handler 转交，滚动期间不调用 `setData`。
