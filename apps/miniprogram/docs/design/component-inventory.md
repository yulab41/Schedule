...
- 月历使用原生三面板 `swiper`，Android 触控、PC 鼠标拖动和程序按钮共用同一翻页路径；关键箭头/定位图标使用真实 WXML 子节点，不依赖伪元素。
- 已实现 `ManualScheduleCell` 与生产手排矩阵（`subpackages/scheduling/pages/manual/index`）：可复用组件保留完整单测/simulate；页面为首屏性能不实例化 600 个组件，而以七组样式声明逐项一致的内置 `view` 渲染格子。矩阵共用 390px 四层 WXS 视图层引擎（`subpackages/scheduling/pages/manual/matrix-gesture.wxs`）：左上角固定，日期只随 X，人员只随 Y，班次随 X/Y，进度条复用 X；矩阵内部没有原生滚动容器或 gesture-handler，横纵轴锁与有界惯性不经过高频 `setData`。开发期的 `pages/manual-matrix-poc` 与 `pages/calendar-poc` 已于 2026-09-19 删除（能力已由本页与 `components/calendar/calendar-month` 承担）。
