...
- 已实现 `ManualScheduleCell` 与矩阵 PoC：`pages/manual-matrix-poc/index?mode=daily|maximum` 分别生成 7×7 和 20×30 确定性数据；日期与班次共用一个原生横向滚动面，人员与班次共用一个纵向 SharedValue，左上角固定。20×30 视口按手机剩余可视空间动态计算，单次手势锁定一个方向，滚动期间不调用 `setData`。
