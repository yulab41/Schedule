...
- 已实现 `ManualScheduleCell` 与矩阵 PoC：`pages/manual-matrix-poc/index?mode=daily|maximum` 分别生成 7×7 和 20×30 确定性数据；日期与班次组合为一个完整宽度的原生横向内容项，人员与班次共用一个纵向 SharedValue，左上角固定。20×30 视口按手机剩余可视空间动态计算，页面级滚动关闭；纵向与横向使用双向协商的 Skyline 方向专用识别器，使 Android 内外层手势都可达，滚动期间不调用 `setData`。
