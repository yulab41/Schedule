...
- 状态：P0 与 P1 非视觉工具链已完成；月历已通过用户 Android 人工验收。`.35` 的 WXS 矩阵首次手势可用，但静止渲染后出现回弹、第二次无响应和跨轴仅单层移动；现已改为模块级坐标、每帧重新取当前节点、X/Y+revision 渲染后恢复，并以 `78e341b`/体验版 `.23` 保留旧方案回退基线。C/D 连续多次实体 Android 复测通过后才进入 P2
