...
- 状态：P0–P7 已完成；P7 `.94@0975b2d` 于 2026-08-25 获得用户实体 Android 明确通过；P8 组织管理、P10 通讯录/个人中心及 P9 数据/消息原生切片均已完成自动回归、体验上传、生产 verifier 与状态矩阵收口，用户已明确本轮无需人工复核。用户随后已明确授权开启 P9 能力、接入已配置订阅模板并继续审核/发布流程；当前生产 `organization=true`、`insights=true`、`externalMessages=true`，候选 `.38` 已上传并加入白名单，正式审核/发布仍受微信平台提交接口所需的第三方凭证限制。
...
9. 手工排班模板与编辑矩阵限制 20 人、30 天周期、600 格；模板应用、预览和草稿批次的首尾日期总区间最长 366 天，服务端按月份拆分草稿与发布周期。
...
P2 完成审计以最小垂直切片为边界：会话持久化、401 单飞重登和身份协调进入 P3/P4；写请求幂等与重试随 P5–P7 的实际 Web 调用点迁移；`wx.downloadFile`/FileSystem 随 P9 导出迁移。不得为满足共享包名义完整性提前建立未被 Web 使用的大而全 API。
...
- 2026-09-20 用户批准：登录成员可按 owner+group 持久缓存已获授权的联系方式（独立快照，不在每个月份重复保存），通过前台变更推送、版本补查和低频兜底静默更新。明确失权、退出或离群清理；离线查看仍限 24 小时。详见 [数据缓存专项](../../../../docs/audit/loading-cache-server-20260920.md)。不缓存访客凭证、导出文件或请求正文。
...
| P2 共享核心（已完成） | presentation-core、client-core、transport、tokens、fixtures、解码器        | Web 先用且回归；Mini bundle 边界/黄金解码一致性通过                                      |
...
| P5 排班闭环           | 手排、限制、撤销、预览/应用、风险、草稿/发布版本、补录、电话同意           | 20/30/600 模板、366 天应用区间、原子补录、版本和隐私通过                                  |
| P6 核心 v1            | 弱网、后台、权限、缓存、性能、隐私、回滚硬化                               | 体验版与回滚演练通过，用户批准后 100% 正式发布                                           |
| P7 工作流（已完成）   | 请假、换班、加扣班、审批/撤销/冲突和消息                                   | `.94@0975b2d` 已完成阶段 RC、用户人工实体机测试和批准                                    |
| P8 组织管理（进行中） | 群组/成员/预设、班种/岗位/规则、邀请/访客码、平台账号后台                  | 阶段 RC、权限/身份、用户人工实体机测试和批准                                             |
