...
- 状态：P0 与 P1 非视觉工具链已完成；月历已通过用户 Android 人工验收。`.40` 的矩阵在 3–5 秒初始化后快慢速手势、冻结层、进度条、点格和撤销均正常；现将 49/600 个格子组件实例扁平为视觉等价内置节点，并把横向边界从 `onReady` 异步测量改为首帧同步计算，以 `78e341b`/体验版 `.23` 保留旧方案回退基线。C/D 首次响应与完整交互实体 Android 复测通过后才进入 P2
