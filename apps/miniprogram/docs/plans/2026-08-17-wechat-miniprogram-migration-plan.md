...
- 状态：P0 与 P1 非视觉工具链已完成；四项 Web 黄金基线均已由用户确认；P1 原生基础控件、同源 WXSS 令牌、42 格 Skyline 月历及 7×7/20×30 手排矩阵 PoC 均已通过静态、simulate 和确定性构建门禁；用户于 2026-08-18 决定不使用 MiniTest/Minium 云测。首次人工验收确认月历通过，并发现基础控件页安卓按钮轨道换行及全端无法纵向滚动；两项均已修复。为避免用户只能通过开发者工具切换独立路由，基础页底部新增三个原生人工测试入口；P1 当前等待用户复测基础控件 A 并直接进入 C/D，明确反馈通过后才进入 P2
...
首页月历固定 6×7，不虚拟化；今日、选中、历史、补录、周末、节假日、跨月、班次和变更独立建模；三面板 Worklet 保留 Web 的方向锁、阈值、缓动和回弹。

手排采用一个横纵滚动主体、成员行纵向虚拟化和独立冻结覆盖层。每个可见成员行最多 30 格；`worklet:onscrollupdate` 同步冻结表头/人员列；滚动不高频 `setData`；点击只更新目标格/行；撤销保存 `{key,before,after}`；禁止整屏 Canvas。
