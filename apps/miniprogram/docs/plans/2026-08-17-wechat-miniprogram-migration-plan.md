...
- 状态：P0 与 P1 非视觉工具链已完成；月历已通过用户 Android 人工验收。`.38` 已解决慢速连续手势，但快速滑动进入惯性后因 Skyline RAF 不提供时间戳产生 NaN、回弹并使主体失效；现改为固定 16ms 步长、纯乘法衰减和 NaN 防御，并以 `78e341b`/体验版 `.23` 保留旧方案回退基线。C/D 快慢速连续实体 Android 复测通过后才进入 P2
