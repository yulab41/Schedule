...
- 状态：P0 与 P1 非视觉工具链已完成；用户人工真机反馈进一步确认当前 Web 月历为动态 5/6 周，并暴露小程序 Pan 手势、关键图标及矩阵冻结层的跨运行时差异。P1 已改为原生三面板 `swiper`、显式图标节点和 WXS 视图层表头同步，仍等待用户在 PC 模拟器与 Android 复测 A/B/C/D；明确反馈通过后才进入 P2
...
手排采用一个横纵滚动主体、成员行纵向虚拟化和独立冻结覆盖层。每个可见成员行最多 30 格；WXS 在视图层把日期表头按横轴、人员列按纵轴同步，滚动不高频 `setData`；点击只更新目标格/行；撤销保存 `{key,before,after}`；禁止整屏 Canvas。
