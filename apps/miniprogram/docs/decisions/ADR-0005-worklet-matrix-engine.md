...
- 状态：已接受；7×7/20×30 Android 人工验收通过。**2026-09-19 更新**：矩阵仍使用本文的四层 WXS 方案（渲染器无关），但文中"Skyline 提供 UI 线程手势/SharedValue"的前提已作废——渲染器固定为 WebView（ADR-0007），页面里最后的 `wx.worklet` 探针（`pages/gesture-probe` A 区）与相关类型声明已删除。
- 日期：2026-08-20
