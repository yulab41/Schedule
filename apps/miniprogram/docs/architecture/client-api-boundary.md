...
- 24 小时排班只读缓存与会话分开。2026-09-20 用户批准已授权联系人按 owner+group 独立持久缓存；前台变更流只传无业务内容提示，随后经鉴权/version 读取静默更新；断线/回前台补查及 120 秒低频校验兜底。退出、离群、401/403 清除对应缓存。访客 key/token、导出文件和请求正文不得写入业务缓存。
