...
> 历史审计记录：其中群组码功能与入口已由 UX-CLEANUP-10（2026-09-06）退役；认领流程也已在后续截图反馈中退役；当前仅保留合法邀请/预设转换及内部关系维护，以下认领内容均为历史记录。历史迁移和审计内容保留。
...
状态（2026-08-25）：P8-C–F 原生页面已完成，四项原生页面均已实现并上传体验版 `0.1.0-p8.20260825.5`；生产 `organization=false`，等待 P8 RC 自动契约与用户实体 Android 复核。实现 checkpoint 依次为 P8-C-1 `70f9a98f`、P8-C-2 `38233039`、P8-D `ddd5c107`、P8-E `c0ea31e9`，登录双入口与会话切换增量随本次体验版提供。
