...
5. 完整 P7 候选固定为`0.1.0-p7.20260824.91`；`.82-leave/.83-swap/.84-duty`仅用于逐切片可追溯上传，部分切片候选永不加入 production allowlist；`.85/.86/.87/.88/.89/.90` 是前六轮完整 RC，已被本轮 Web 视觉与滚轮对齐版 `.91` 取代。`.91` 上传、代码部署、自动 RC 和回滚探测通过后，才在 release 锁下原子加入 allowlist；旧版本继续保留自身 UI 行为，不新增离线写队列。
