...
- 普通视图上的唯一 `pan-gesture-handler` 在同一个 UI 线程 `ongesture` 中读取 `deltaX/deltaY`、选择并锁定单轴。由于四层子节点全部绝对定位，handler 与 `.matrix-pan-surface` 都以内联像素值绑定 `matrixViewportHeight`，并显式使用块级命中区；不得只写百分比高度。横向更新日期/主体 X，纵向更新人员/主体 Y；两个轴都在 END 使用有边界的 `decay`。零位移、小于 2px 或未达到 1.2 倍方向优势时保持未决。独立矩阵页继续声明 `disableScroll: true`，ACTIVE 全程不调用 `setData`。
...
- `pages/gesture-probe/index` 是 diagnostic-only 最小页：A 区复刻官方普通视图 Pan Worklet 蓝点，B 区只计数普通触摸，C 区显示设备运行信息；它不依赖矩阵、滚动组件、自定义单元格或业务 API，结论只能用于选择下一排查方向。
