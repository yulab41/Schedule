...
- 手势只使用一个 `pan-gesture-handler native-view="scroll-view"`：它直接代理横向 `scroll-view`，并在同一个 UI 线程 `ongesture` 中读取完整 `deltaX/deltaY`、选择并锁定单轴。横向锁定只让内部原生 `scroll-x` 工作，Worklet 不写纵向偏移；纵向锁定不依赖事件冒泡或父级接管，代理自身直接把同一 SharedValue 写给人员和班次轨道，END 使用有边界的 `decay`。零位移、小于 2px 或未达到 1.2 倍方向优势时保持未决；禁止恢复被目标 Android 证明纵向 handler 不可达的“外层 vertical + 内层 horizontal native proxy + simultaneous/should-response”嵌套。独立矩阵页继续声明 `disableScroll: true`，全过程不调用 `setData`。
