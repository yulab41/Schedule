...
- 一个 `pan-gesture-handler` 与代理原生 `scroll-view` 的 `horizontal-drag-gesture-handler` 同时协商。移动累计达到 10px 后，横纵距离比达到 1.25 才锁定对应轴；一次手势锁定后直到 END/CANCELLED 不切轴。横向继续交给原生惯性滚动；纵向在 UI 线程将同一 SharedValue 同步应用到人员轨道和班次轨道，松手使用带边界的 `decay`，全过程不调用 `setData`。
