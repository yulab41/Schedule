...
- 手势使用互斥的方向专用识别器：外层 `vertical-drag-gesture-handler` 负责纵向 SharedValue，内层 `horizontal-drag-gesture-handler native-view="scroll-view"` 代理原生横向滚动。两者不声明 `simultaneous-handlers`，由 Skyline 默认的内层优先与方向识别在一次触摸中只选一个轴；横向继续交给原生惯性滚动，纵向在 UI 线程将同一 SharedValue 同步应用到人员轨道和班次轨道，松手使用带边界的 `decay`，全过程不调用 `setData`。
