...
- 外部模板已经过逐文件 SHA‑256 校验迁入；原 Skyline 兼容层已于 2026-09-19 整体删除（`docs/audit/webview-only-cleanup-20260919.md`）。
- 正式 AppID 保留在 `project.config.json`；私有配置保留但被 Git 忽略。
- 默认模板首页和 navigation-bar 已移除，以无业务外观的技术 bootstrap 取代。
- `src → dist` 编译、源码/产物边界、包体、Worklet 指令（渲染器固定为 WebView 后已成为"仓库内不得出现 `'worklet'` 指令"的反向守卫）、双构建确定性和无凭证 CI dry-run 门禁已完成。
- P1 基础控件批次新增从 `@schedule/ui-tokens` 单源生成并复制到 `dist/styles/tokens.wxss` 的路径；`app.wxss` 必须显式导入该文件，缺失或漂移由测试/产物审计阻断。
- P1 月历按 Web 的实际 5/6 周高度生成三个面板，并使用原生 `swiper` 统一 Android 触控、PC 鼠标和程序翻页。
...
- WXS 惯性不读取 `requestAnimationFrame` 回调参数：WebView 不保证浏览器式时间戳。每帧使用固定 16ms 位移和 `velocity *= 0.92` 衰减，避免第二帧计算产生 NaN；边界函数对 NaN 防御性归零。快速滑动和慢速滑动因此共用同一有限坐标状态，惯性结束后可立即同轴或换轴。
...
- `pages/gesture-probe/index` 是 diagnostic-only 最小页：D 区验证 WXS 黄色点，B 区计数普通逻辑层触摸，C 区显示设备运行信息，另有 E 区年月滚轮与 F 区工作台压力探针。目标 Android 已确认黄色点横纵同步跟手，矩阵因此使用同一 WXS 原语；探针仍不属于产品 UI。原 A 区 Pan Worklet 蓝点在渲染器固定为 WebView 后不会执行，已于 2026-09-19 删除。

官方依据：[`WXS 响应事件`](https://developers.weixin.qq.com/miniprogram/dev/framework/view/interactive-animation.html)、[`scroll-view`](https://developers.weixin.qq.com/miniprogram/dev/component/scroll-view.html)。WXS 官方 `ComponentDescriptor` 明确提供 `setStyle`、`callMethod` 与 `requestAnimationFrame`；本实现只用这三项，尺寸仍由逻辑层低频测量。
