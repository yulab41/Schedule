...
- 日期与班次只有一个原生横向 `scroll-view`，人员与班次共用一个纵向 SharedValue；不得恢复三个滚动容器或逐帧 `ScrollViewContext.scrollTo()`。WXML 必须使用包住人员覆盖层的外层 `vertical-drag-gesture-handler` 与内层 `horizontal-drag-gesture-handler native-view="scroll-view"`，双方以唯一 `tag` 和双向 `simultaneous-handlers` 共同参与嵌套手势协商。两个 `worklet:should-response-on-move` 必须共用轴锁并覆盖零位移、等幅、近斜向、明确横向、明确纵向和选轴后反向抖动；内层在纵向锁定时始终返回 `false`。纵向 ACTIVE 只写一次共享偏移并同时作用于人员/班次轨道，END 使用有边界的 `decay`；横向 `worklet:onscrollupdate` 只更新进度 SharedValue，普通 `bindscroll` 仅作进度提示兜底。点击只更新目标格或目标行；撤销保存 `{key,before,after}`。
