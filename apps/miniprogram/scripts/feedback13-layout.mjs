...
const output = path.join(root, 'runtime/audit/feedback13/layout');
...
try {
  for (const width of [390, 320])
    for (const [mode, html] of [
      ...
      const geometry = await page.evaluate(() => {
        ...
        if (weekly.length)
          document.querySelector('.week-swiper').style.height =
            Math.ceil(Math.max(...weekly) + 28) + 'px';
        return {
          overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth,
          bottomGaps: [...document.querySelectorAll('.week-day')].flatMap((day) => {
            const groups = day.querySelectorAll('.week-shift-group');
            return groups.length ? [rect(day).bottom - rect(groups[groups.length - 1]).bottom] : [];
          }),
      ...
      assert.ok(
        geometry.bottomGaps.every((gap) => gap >= 23.5),
        `${mode}/${width} bottom corner clearance`,
      );
...
console.log('feedback13 production CSS geometry passed: 390/320 month/week/list');
