...
describe('workflow controller lifecycle generation', () => {
  ...
  it('makes repeated disposal idempotent and preserves a fresh synchronous controller path', () => {
    const onLoad = vi.fn();
    ...
    registerWorkflowPanel(() => ({
      ...
      handleSync() {
        this._syncCount += 1;
        this.setData({ result: `sync-${this._syncCount}` });
      },
      onLoad,
      onUnload,
    }));
    ...
    componentDefinition.lifetimes.attached.call(instance);
    const firstTask = captureWorkflowControllerTask(instance);
    componentDefinition.methods.handleSync.call(instance);
    expect(instance.data.result).toBe('sync-1');
    ...
    instance.properties.groupId = 'group-b';
    componentDefinition.lifetimes.attached.call(instance);
    const secondTask = captureWorkflowControllerTask(instance);
    componentDefinition.methods.handleSync.call(instance);
    ...
    componentDefinition.lifetimes.detached.call(instance);
    componentDefinition.lifetimes.detached.call(instance);
    expect(secondTask.isCurrent()).toBe(false);
