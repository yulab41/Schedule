...
describe('shared picker unification', () => {
  ...
  it('removes native pickers and routes every migrated field through shared components', () => {
    ...
    const invite = read('subpackages/organization/components/invite-visitor-panel/index.wxml');
    expect(occurrences(invite, '<ui-selector')).toBe(3);
    expect(occurrences(invite, 'placement-boundary="{{pickerBoundary}}"')).toBe(3);
    ...
    expect(
      JSON.parse(read('subpackages/organization/pages/invite-visitor/index.json')).usingComponents[
        'ui-selector'
      ],
  ...
  it('passes a local scroll boundary to every page-level shared list selector', () => {
    const contracts = [
      ...
      ['subpackages/organization/components/invite-visitor-panel/index.wxml', 3],
