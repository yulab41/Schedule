...
- Diagnostic sources `pages/index`, `pages/gesture-probe`, and `subpackages/diagnostics/pages/test-tools` and their tests remain in the repository for reuse, but are not registered routes or uploaded production files. The production build excludes their fixed directories and the test-only `platform/diagnostics-access.ts`; any reappearance in `dist/` blocks upload. Shared business diagnostics remain. `pages/manual-matrix-poc/matrix-gesture.wxs` is imported by the production manual schedule page: never delete that file without moving its production import first.
