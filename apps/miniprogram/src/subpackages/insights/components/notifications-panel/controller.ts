...
async function loadNotifications(page: NotificationsPageInstance): Promise<void> {
  ...
  try {
    ...
    if (requestSerial !== page._requestSerial) return;
    page.setData({
      errorMessage: error instanceof ClientCapabilityDisabledError ? error.message : toUserMessage(error, '通知暂时无法加载，请稍后重试。'),
...
async function loadMore(page: NotificationsPageInstance): Promise<void> {
  ...
  try {
    await requireClientCapability('insights');
    const result = await page._actionsClient.listNotifications({ groupId: page.data.groupId, cursor, pageSize: 20 });
    page._nextCursor = result.nextCursor;
    const notifications = [...page.data.notifications, ...result.notifications.map(toNotificationCard)];
    page.setData({ loadingMore: false, nextCursor: result.nextCursor ?? '', notifications, unreadCountLabel: `${result.unreadCount} 未读` });
  } catch (error) {
    page.setData({ errorMessage: toUserMessage(error, '更多通知暂时无法加载，请重试。'), loadingMore: false });
  }
}
...
async function markRead(page: NotificationsPageInstance, id: string): Promise<void> {
  ...
  try {
    ...
    page.setData({
      actionBusyId: '',
      notifications: page.data.notifications.map((item) => (item.id === id ? { ...item, isRead: true } : item)),
    });
  } catch (error) {
    page.setData({ actionBusyId: '', errorMessage: toUserMessage(error, '通知状态暂时无法更新，请重试。') });
  }
}
...
async function markAllRead(page: NotificationsPageInstance): Promise<void> {
  ...
  try {
    ...
    page.setData({ actionBusyId: '', notifications: page.data.notifications.map((item) => ({ ...item, isRead: true })), unreadCountLabel: '0 未读' });
  } catch (error) {
    page.setData({ actionBusyId: '', errorMessage: toUserMessage(error, '通知状态暂时无法更新，请重试。') });
  }
}
