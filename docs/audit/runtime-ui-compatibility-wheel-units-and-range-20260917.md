...
- 3.17.2：年月数字**没有"年/月"单位**；中间的选中项**没有变大/变粗**；向下只能滚到 2027 与 5月，
  抬手再拖也不能继续向下，但可以向上回滚；标题里的草稿值（2031年12月）与滚轮可见位置（2024..2028）**不一致**。
- 3.17.3：单位、选中放大、完整范围、草稿与可见位置一致，全部正常。
- 关键反证：3.17.2 的截图里行与行**存在大小/浓淡渐变**，说明 WXS 的 `setStyle`（行样式）在 3.17.2 是**到达渲染器**的；
  因此问题不在"样式通道"，而在**数据交付通道**：属性/观察器在 3.17.2 不可靠，组件自身 `data` 与条目数据可靠。
