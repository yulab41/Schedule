...
- 当前累计分支以 merge parent 纳入 `5285dd17`（其祖先包含 `1ffab10c`）；`packages/ui-icons` 与 46 个
  generated Mini SVG 的树内容逐项等同该原提交，旧 `web-*` 文件和引用均为 0。
- Web 已无 `tdesign-icons-vue-next` 直接 package dependency 或生产 source import；锁文件仍有 3 处传递引用，
  来源是继续使用的 `tdesign-vue-next`，不是新增页面私有图标来源，也不进入 Mini runtime。
- 当前累计验证：Mini 定向 7 files/63 tests、Web/token 5 files/39 tests、Mini 全量 122 files/663 tests；Web
  production build 4,249 modules/17.42s；Mini production 302 files、total/main
  `5,169,730/1,731,703 B`，相对 B2 入口为 `+17,837/+15,984 B`，低于总包预算且未新增 warning 类别。
- `pnpm verify` 全仓 format/lint/build/typecheck 通过，Mini 再次 122/663，根 Vitest 247 files passed /
  37 skipped、1,178 passed / 364 skipped。浏览器 smoke 可打开登录页，但本地 API 3000 未运行，管理员登录
...
- 验证期间前进的 `origin/main@765b5c09`（含 `fa10d5ba`）只改变项目 guardrail/工具/文档，对本报告的
