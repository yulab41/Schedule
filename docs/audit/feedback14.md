...
基线 e163fde8（累计体验版110应用8f441d2d）。用户本轮要求排查并修复两项；实施阶段未获新的部署、上传或放行授权；后续授权交付见[feedback14-trial-release.md](feedback14-trial-release.md)。general-4/5独占复用依赖，无安装；不操作开发者工具。
...
更多位移完成代码修复与浏览器复核，待用户同版本原生验收；导出保持待定位。已获用户后续授权并交付111/6b8a9e9；下一步拿到同版本安全报告再做有证据的导出修复，不部署API/Web或重复上传。
...
本次提交消息：fix(miniprogram): stabilize More navigation and expose export render stages。这是更多位移修复与导出可观测性检查点，不是两项全部修复。后续用户批准6b8a9e9c上传/追加放行，111已交付并验证，服务器应用保持e163fde8；见交付记录。
