...
## 当前批次：补录周历与手排预览共用日历（已实现，待上传与真机复核）
...
- 定向 42/42、根 `pnpm verify` 1306/451 跳过、Mini production verify（4,801,743 B）、开发者工具三处 WXML 编译通过。模拟器补录路由一直显示加载中，真实排班视觉及小米 14 均未验证；检查点提交信息 `fix(miniprogram): reuse calendar week view in scheduling previews`，提交与推送结果在收尾确认。
- 状态 `UPLOAD_REQUIRED`：当前轮次未授权上传，未分配版本或放行。唯一下一任务：获授权后上传干净累计体验版，并以同构建小米 14 核对补录及生成、草稿、发布预览；此前不写原生验收通过。
