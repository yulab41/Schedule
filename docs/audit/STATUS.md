...
## 当前批次：共用周历体验版 .197 已上传放行，待小米 14 复核

- 首页、补录和手排生成/草稿/已发布预览改用同一周格、班种顺序、浅色分组和姓名换行；月格保留原组件，周分页共用环形状态，补录跨周定位预取并沿用读取失败禁用。详情见 [轮次记录](schedule-week-parity-20260925.md)。
- `pnpm verify` 通过（Mini 1274/17 跳过、根 1306/451 跳过），production verify、源码/包体检查和开发者工具局部 WXML/WXSS 编译通过；模拟器补录仍停在既往排班加载，Console error 过滤为空。开发者工具未取得含真实排班的三入口截图，连续手势、320px、原生动画及小米 14 当前构建效果暂未验证。
- `ad6c09fe` 已推送；`.197@ad6c09fe` production Manifest、receipt 与远端 tag 一致。用户随后授权生产操作；备份 `c1e4ee1c-e729-4e98-a4de-1321930e9413` 后部署同 SHA，完整 ECS verifier 通过；可信 allowlist 只追加 `.197`，独立验证和 `.197/.196=200`、未知版=426 通过。未提审或正式发布。唯一下一任务：小米 14 使用同构建 `.197@ad6c09fe` 复核补录和三类预览的周格顺序、浅色分组、换行、切周和定位动画；无真机证据前保持待用户复核。
