...
- 访客月窗复用、访客独有返回登录、二维码删除相册链路已在325f82ea实现并推送；CSV下载失败确定为公众平台downloadFile合法域名配置。
- `.122`在微信调用前因最新累计`.121`非祖先被血缘门禁拒绝，不删除/复用该号码。当前合并`.121`，保留其导出真实转换修复、二维码预览和轮换自动读取。
- 合并后完整verify通过：Mini1159/16跳过、根1254/439跳过、依赖保护81；Mini verify/Worklet2/2/确定性/包体/smoke:check-core通过，主包1709746/总4549449字节。
- 下一步：提交推送新累计SHA，再分配新版本上传、仅追加放行并运行完整验证。自动化不代替小米14验收。
