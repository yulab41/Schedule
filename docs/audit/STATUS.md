...
- 附件确认9月8—9日源快照简称为全，其余28日为全天；不是CSV截断，生产历史来源未在无L4授权下查询或修改。代码/API生产部署也未授权。
- 最终全门禁通过，9e603fdb已推送；体验版0.1.0-p10.20260913.123上传成功，Manifest 03986dd5…f5495c。追加放行在SSH前因DoH公网地址host key与正式域名ED25519记录不一致而停止，未改变生产；122=200、123=426。
- 唯一下一任务：独立核对host key后重试123 add-only allowlist。API功能需另行授权部署；放行和部署完成前不进行小米14最终验收。
