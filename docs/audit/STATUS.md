...
- 首次正式上传在调用微信平台前被`5285dd1`等价血缘证明安全拒绝，未占用版本；兼容提交只为工作台TS增加精确版本判定的import、data类型和初始字段，未修改受保护方法，已更新精确blob证明并复跑门禁。本轮尚无微信开发者工具、上传、allowlist、生产部署、数据库、提审或正式发布操作。唯一下一任务：推送新的精确检查点并完成获准的上传/追加放行，再由两个微信实例对同一体验版原生复核。
