...
- 当前阶段：`.75@24a847f` 小米 14 第二批启动诊断已返回，10 秒服务端主查询长尾未复现
...
- 两个独立 App 启动首搜样本为 10,016ms 与 685ms；当前只能定性为一次服务端 rows/main-query 长尾，
  不能定性为稳定首搜退化。第二批另有 1 条被新搜索替代，服务端仍正常完成 200，页面正确未提交旧结果。
- 唯一下一任务：再做 1 次独立“下次 App 启动首次搜索诊断”，仍以同一个 3 字姓名查询作为页面第一次
  搜索并只复制该条。若继续在亚秒级则停止改动、转为观察长尾；若再次达到数秒，再单独授权查询计划/
  缓存调查。此前不改搜索、数据库、索引或部署，不再次上传、修改 allowlist、备份或同步 release。
