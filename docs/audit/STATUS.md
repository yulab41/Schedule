...
- 完整verify2次启动1次通过；浏览器smoke PASS，最后Mini增量定向及生产构建通过。新源码未部署/上传，旧.90管理员需在后端退役时配套升级。
- 唯一下一步：确认新增认领退役候选的API/Web与新体验版发布。记录见 `ux-cleanup-10-feedback1.md`。不将下述旧.90交付当新源码证据。
