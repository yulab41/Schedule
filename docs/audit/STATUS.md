...
- 开发者工具门禁、局部编译、390px 模拟器布局/删除胶囊和 Console 过滤通过；截图只算模拟器证据。`eacfd752` 已推送，production 候选上传为 `.190`（232 文件、ZIP 2,675,243 B、Manifest `79c51d52…d4b7`），不可变身份与 receipt 一致。新 API 未部署且 `.190` 未加入生产 allowlist；当前消息无生产授权，未连接或修改生产。320px/大字号由静态门禁覆盖，仍待小米 14 `.190@eacfd752` 原生复核。详见 [本轮记录](manual-schedule-inline-editor-20260923.md)。
