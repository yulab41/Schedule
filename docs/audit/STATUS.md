...
- 新一轮按用户明确输入顺序取得第二个慢 `xmb`：request `125ca48c…` 的 main/server/total 为
  8,932/9,176/9,276ms；旧慢 `2b12f7ee…` 为 9,696/9,942/10,016ms；快 `23b3bec8…`
  同词 main/total 为 331/734ms。此前“单个孤立样本、停止调查”结论已失效。
- 已排除稳定客户端首搜退化；准确口径是 `xmb` 首字母查询已至少两次复现 9–10 秒服务端主查询长尾，
  同词也能亚秒完成。首字母查询结构与数据库/服务冷态、执行计划页读取或资源等待的组合关系仍未定根因。
...
- `xmb` 已至少两慢一快，足以拒绝“每次首搜必慢”和“只出现一次”两种说法。production 当前计划会
  扫描当前员工批次并逐行计算四类 alias/fulltext 相关度；同一非数字主查询 digest 的最大值为 9,877ms。
  正常首搜约 0.7–0.8 秒、精确计数约 180–200ms，仍与 9–10 秒主查询长尾分开处理。
...
- 本轮到只读根因报告和 docs-only checkpoint 后停止。唯一下一任务是等待用户确认最多三个候选方向；
  本轮不改代码/查询/索引/缓存，不再次上传、部署、备份或操作其他外部状态。
