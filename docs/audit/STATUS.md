...
- 当前批次：`EXP-UX-002`；用户已批准并复核请假/加扣班弹窗外壳设计，代码修改和自动化验证已完成；当前状态为
  “待 Xiaomi 14 真机复核”。
...
- 本批实现：请假新建/审批与加扣班发起/管理员直达/撤销五个弹窗均复用既有 `ui-sheet`，统一 fixed/z400、
  78vh/max660、safe-area、独立 `workflow-sheet-scroll`、固定 footer 和顶部 drag region；direct leave/swap/duty
  Page manifest 已显式注册 `ui-sheet`，未改 controller/API/路由语义。
...
- 旧根因已用 `git log -S`/`git blame` 定位：请假旧表单 `9fae3869`，工作流旧表单路径 `80ddadf0`/`bc32a4f1`；
  本批合同在旧源码上先红 4/4，修复后 4/4；定向 5 files / 33 tests，Mini 全量 115 files / 625 tests 通过。
...
- 本批 clean production package audit 为 `5,109,717B`，相对上一批同口径 `5,113,419B` 实际减少 `3,702B`，
  其中 workflows 减少 `3,701B`；主包与矩阵 warning 未新增类别。

## 验证与边界

- `pnpm miniprogram:test` 为 115 files / 625 tests；Mini TypeScript、276-file production build、source/verify、
  全仓 Prettier、ESLint、`git diff --check` 和 `pnpm smoke:check-core` 均通过。新合同由标准 `--dir scripts` 自动发现。
...
- 本批不上传体验版、不部署 production；五张截图的构建身份不可由当前工具读取，故本批仍不能写成 Xiaomi 14
  真机验收通过。

## 唯一下一任务与停止条件

- 唯一下一任务：在与最终 checkpoint 一致的体验版上，于 Xiaomi 14 复核请假/加扣班五个 sheet 的高度、
  safe-area、内部滚动、顶部拖动阈值/回弹、busy 保护和系统侧滑返回；在人工证据到达前保持“待用户复核”。
