...
- 独占 `general-4`、`REUSE_ONLY`、安装 0，源码基线 `b65c76f3`。用户撤回顶部导航改版，五个主页面原导航与页面标题保持不变；二维码四字段按最新反馈调整为原生 40px、Storybook 20px。
...
- Mini 已移除邀请生成/接受/分享/撤销及群组码界面。为避免旧正式版严格契约崩溃，服务端旧邀请能力、旧表、旧双码接口仅作阶段一兼容保留；阶段二须在新正式版稳定且旧调用归零后，另获生产破坏性操作授权再删除。
- 自动化：Mini 1229 通过/16 跳过；根 Vitest 1303 通过/452 跳过；真实 MySQL 专项 116/116；typecheck、lint、format、build、Storybook、生成契约与浏览器 smoke 通过。视觉比较缺少成对夹具，开发者工具授权 pending，当前工具未取得原生编译/截图/扫码证据。
- 包体总量 −53,317 B，organization 分包 −105,480 B，主包 +25,353 B。未生产部署、未上传体验版、未提审/发布、未做小米 14 验收；本地/浏览器证据不能替代这些层级。
- 唯一下一任务：用户明确授权生产操作后，创建并验证生产备份，部署阶段一 API 与 schema 63，再冻结/上传体验版并验证环境码；停止条件是上线授权或外部授权状态变化。详见 [完整记录](profile-qr-visitor-audit-20260921.md)。
