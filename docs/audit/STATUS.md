...
- CSV 下载失败已定位为微信 `downloadFile` 合法域名配置；production URL 与错误分类回归通过，代码不使用 URL token 绕过。公众平台配置仍需外部核对。
- 匿名访客页三视图复用仅驻内存月窗并合并请求；页面隐藏、卸载、显式重试和访客码失效仍清空敏感状态。返回登录按钮只在访客页视图控制行出现，不改成员页或共享组件。
- 二维码相册按钮、权限、临时文件和保存 API 已删除，保留 PNG/JPEG 显示及轮换，启用微信长按菜单。主包1708198/总4545686字节，较上一检查点减少661/4735。
- RED3→定向51通过；完整verify、icon parity、Mini verify、Worklet2/2、确定性、包体和smoke:check-core通过。自动化不是小米14原生验收。
- 唯一下一任务：提交推送 clean 检查点，按当次授权上传新体验版并仅追加放行；不部署API/Web、不改数据库、不提审或正式发布。交付后由用户在小米14验证三控件布局、三视图无频繁全页转圈、二维码长按及CSV下载。
