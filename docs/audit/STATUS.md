...
## 当前批次：按日期发布与绑定页面修复（实施中）

- 基线为 `.191@c6c4fcd2` 累积源 `38d08103`；本轮源码仍未提交或上传。成员绑定页改为一次确认、完整姓名/工号、访客码入口；草稿/发布预览定位一次切换，冲突提示只列真实重复日期。
...
- 正式服务器 2026-09-24 只读核对：live `c6c4fcd2`；12 月当前版只有 31 日 1 班次，旧归档版保留 1–30 日 30 班次，归档班次无关联换班或加扣班。恢复前仍需复核版本和数据并完成加密备份。
- Docker Desktop 已通过官方 `disable model-runner` 恢复，旧重解析点保留未触碰。唯一下一任务为 diff 审查与提交推送，随后按生产备份、部署、2026-12 受控恢复、不可变体验版上传顺序交付；受影响工作流如有撤销则暂停恢复写入。小米 14 验收只在取得同构建真机证据后记录。
