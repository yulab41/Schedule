...
## 当前批次：正式包移除六个闲置路由，保留诊断源码（`.202` 已部署放行，待小米 14 验收）

- 范围：独立“我的 / 通讯录 / 换班”包装页删除，工作台五标签和共用业务组件保留；基础控件演示、手势探针、测试工具源码/测试留仓库，正式构建与上传排除，更多入口移除。正式路由 24→18。
- 同口径 production 原始包 4,819,158→4,326,081 B（减少 493,077 B），主包 1,835,334→1,616,037 B（减少 219,297 B）；本地 ZIP 代理减少 119,398 B。均超过用户停止阈值；微信 CI 该版 ZIP 为 2,360,179 B，但无同口径旧版 CI ZIP 可比较。详见[轮次记录](mini-package-slim-20260926.md)。
- 静态/Node：正式产物无诊断页/模块，注入文件会触发上传前拒绝；Mini 1,273 通过/18 跳过，根测试 1,307 通过/452 跳过，Mini verify、lint/build/typecheck、icon parity、smoke:check-core、CI dry-run 和血缘检查通过。全仓格式检查在五个未改文件上有基线失败，未混入本批修改。
- 开发者工具：已登录且兼容，工作台五标签状态逐项匹配；登录、工作台、访客和四个业务分包代表模板编译成功。访客缺有效链接，直接打开后自动化读取超时，真实业务和小米 14 仍未验证。
- 精简检查点 `e6ef714b` 和血缘证明检查点 `5b12c724` 已推送。首次上传因旧 blob 证明过期在分配版本前停止；红绿测试和精确证明更新后，Git tag 读取曾遇瞬时网络错误，再次重试同一候选成功。production 体验版 `0.1.0-p10.20260926.202@5b12c724` 已上传；Manifest `bdb22991a43ecdf3d9c5f22064a8125a244564d6e4c2e9a688fb6ccddabe61f7`，CI 上传 ZIP 2,360,179 B，tag/receipt 一致。330 个正式产物文件经预上传门禁核对，未含测试工具和旧包装页；实际 CI ZIP 无同口径基线，无法报告它的减量。
- 本轮用户明确授权部署并放行。服务器实时前驱 `833a11e4` 与候选 `8ab6663b` 的 21 项应用/控制/schema 哈希一致；加密备份 `e465caa7-6d6c-491d-b652-adc9d9bde087` 文件大小和 SHA-256 核验后，可信无停机复用发布到 `8ab6663b`，前后完整 verifier 通过。`schedule-client-version-allowlist ensure` 只追加 `.202`、保留 `.201` 等旧版本；独立 verify、完整 ECS verifier 和公网 `.202/.201=200`、未知版=426 通过。交付文档检查点按同构建哈希另行备份并同步生产 release 元数据。
- 唯一下一任务与停止条件：等待同构建小米 14 的登录、工作台五标签、访客与业务分包真机证据；版本/SHA 不符或业务回归则停止验收并处理。不提审、不正式发布。

## 上一批次：日历定位按钮位置统一（体验版 `.201` 已放行，待小米 14 复核）

- 用户反馈：日历首页的定位按钮在月视图与周视图位置不一致、**周视图偏左**，以月视图为准；首页、访客等所有定位按钮都要检查并统一，且只改这一件事。
- 根因（先测量）：月视图用共享组件 `.locate-button`（40×44，距卡片右外沿 71px，引入点 `1f715c96`），周/列表视图用页面内联 `.calendar-locator`（44×44 + `margin-left: 4px`，引入点 `ad4cfb2c`），准星因此偏左 2px。两次独立测量一致：`.198` 开发者工具截图月视图准星中心 443 / 周视图 440（左右 chevron 完全一致）；真实 WXSS 390px 无头几何月卡片 70.00px / 修复前周视图 72.00px。
- 修改：`pages/workbench/index.wxss` 的 `.calendar-locator` 改为 `width: 40px` 并删除无布局作用的 `margin-left`，与月视图参考盒完全一致；`.calendar-step` 换期 chevron、标题、按压/hover 时间、定位动效与 `handleLocateToday`/列表滚动定位路径未改。访客页 `@import '../workbench/index.wxss'`，同批生效；列表视图按 Web 金标准保留最右槽位，只统一按钮盒。
- 验证：新增“月/周/列表定位按钮盒一致”断言先失败（44px≠40px）后通过（该文件 27 passed）；Mini 全量 `183 passed | 2 skipped`（1282/18 跳过）；Mini production verify（4,817,890 B、manifest `40f35b70…`）、`pnpm typecheck`、`pnpm lint`、`icon:parity:check`、`smoke:check-core` 通过。开发者工具（`.201` 版本绑定构建、真实排班、390px、WebView）实测月/周准星 `left` 均为 299.6（卡片右沿 378.4，距右外沿 70.8）、两侧 chevron 同为 339.6/30.8，列表 343.6。
- 交付：`287691ff` 已推送；体验版 `0.1.0-p10.20260926.201`（Manifest `72e7f32900c6f1550514d82300983bf5d39e24667748868f05783ccad9439060`、239 代码文件、ZIP 2,703,186 B），远端 tag、allocation、manifest 与 receipt 同一 SHA；可信 add-only `ensure` 只追加 `.201` 并保留 `.200`，独立 verify、完整 `ecs-verify.sh`（live release 仍 `833a11e4`、schema 64 未变）与公网 `.201/.200=200`、未知版本 `=426` 通过。未部署生产应用、未备份或迁移数据库、未提审、未正式发布。
- 未验证：访客页模拟器实测（需要有效访客链接，由共享 `@import` 与同结构模板断言覆盖）；模拟器截图接口本轮只返回 35×75，几何取自 `boundingClientRect`；小米 14 同构建真机未验证。详见 [轮次记录](calendar-locate-button-20260926.md)。
- 唯一下一任务与停止条件：用户用小米 14 打开 `.201@287691ff`，复核首页与访客页在月/周/列表下的定位按钮是否与月视图一致；取得同构建真机证据前保持“待用户复核”，不进入其他功能开发。

## 上一批次：补录移除能力 + 预览精简 + 圆角统一（生产已部署，体验版 `.200` 已放行）

- 服务端 `20c024c6` 给批量补录加可选 `removals`（同事务/同幂等/同审计软删除，`version + 1`），真实 MySQL 集成 `13/13`；客户端把补录点击改为「已存在＝暂存移除、否则＝暂存新增」，只在「确认补录」时一次提交 `items + removals`，草稿支持同日多条，周/月统一 `state: removed|added`（删除线/暗红）且按班种+人员锚定，删掉（原）（拟）与「同日第一条」旧判断；手排预览的单元格点击详情（绑定/模板/样式/模型）已整体删除。
- 圆角核验（用户追问）：首页/访客周视图、月视图、补录与预览周视图三套结构均为 17px 一致圆角，无头量测圆弧/直线覆盖率 0.996 / 1.045 / 1.073，开发者工具实拍月视图左下圆角与直线等宽；访客页 `@import` 首页样式、预览复用 `calendar-month`。
- 验证：Mini `183/1281`、补录控制器 `15/15`、presentation-core `7/7`、`pnpm typecheck`、`pnpm lint`、root `pnpm test`、Mini production verify（4,817,910 B，manifest `1870568db54fec0788e3925e0dc4df12229a93267654ea0e4fa9555aa01db50a`）通过；DevTools 首页月/周与补录页加载无 console error。
- 交付：`833a11e4` 已推送并完整部署生产（备份 `7fd67e5c-3d9c-49a5-9b4a-643bc9620bc3`，`ecs-verify.sh` 通过，回滚候选 `ad6c09fe`）；体验版 `0.1.0-p10.20260926.200` 上传、只追加放行，独立 verify 与完整 verifier 通过，公网 `.200`/`.199`=200、未知版=426。
- 唯一下一任务：小米 14 打开同构建 `.200@833a11e4`，复核护士群多班种补录（点击已有＝暂存移除、确认后才写入、删除线/暗红）与首页/访客/补录/预览的周月圆角。

## 上一批次：周视图网格圆角统一体验版 `.199` 已上传放行，待小米 14 复核
...
- 本轮基线 `5befe6a3`，修复班种徽标显示、补录读取群组周/月偏好，并让三类手排预览周历共用既有日历分页/单元格；详细根因与证据见 [轮次记录](schedule-week-preview-20260925.md)。代码仅 Mini 与文档范围，生产操作另获本轮授权。
- 定向 42/42、根 `pnpm verify` 1306/451 跳过、Mini production verify（4,801,743 B）、开发者工具三处 WXML 编译通过。模拟器补录路由一直显示加载中，真实排班视觉及小米 14 均未验证；`aef86299` 已推送。
- `0.1.0-p10.20260925.196@aef86299` 的 production Manifest `16a51495e92e8f2d60988432632602d0490bb8a5335bb54e559576e83e4e0beb`、receipt 与 tag 一致。备份 `49c9cf91-8a30-45b5-ab83-d74f1bf0d80f` 文件核验后，哈希相同的 release 无停机切至 `17c1c90f`，前后完整 verifier 通过；`.196` 只追加白名单且独立 verify、ECS verifier、`.196/.195=200`/未知版=426 均通过。未迁移、提审或正式发布。唯一下一任务：小米 14 同构建验证补录与三类预览；无真机证据前不写原生通过。

## 上一批次：排班补录与预览修复体验版 .195（已上传放行，待小米 14 复核）

- 累计候选 `d4f0cbe3` 已推送；`0.1.0-p10.20260925.195` 的 production 上传、Manifest、receipt 与远端 tag 一致。定向 49/49、Mini production verify、完整 `pnpm verify`、格式及图标门禁通过；本地浏览器 smoke 因 5173 服务未启动而未通过，不作为运行证据。
- 经用户追加授权，生产备份 `a521826a-24c6-42b7-a38d-319cfb09da67` 已核验；相同产物哈希的 release 无停机切至 `d4f0cbe3`。体验版 `.195` 仅追加白名单；allowlist 与完整 ECS verifier 通过，公网 `.195`=200、未知版=426。未提审或正式发布。
- 唯一下一任务：在小米 14 打开同构建 `.195@d4f0cbe3`，复核补录中秋已有班次、切月/岗位/返回及读取失败提示、班种和人员选中样式，并分别检查默认周/月/列表时手排生成、草稿和已发布预览。没有同构建真机证据前不标记原生验收通过。

## 上一批次：邀请绑定页访客内容清理与布局修复（自动验证完成，待新体验版复核）

- 用户最终范围：绑定邀请目标页不含访客功能或二维码；独立 `pages/guest-entry` 页面内容保持不变。绑定页去掉访客入口及常驻错误弹窗，错误/成功反馈改为限时 Toast，增加独立返回登录动作；已有会话时返回登录仍停留在登录表单。
- 来源证据：`git log -S '暂不绑定，进入访客页面'` 与 `git blame` 均定位到 `162ef4c1`。guest-entry 四个页面源文件没有 diff。绑定页 WXML/TS/WXSS/JSON 无访客功能引用。
- 定向 15/15；390×844、320×844 Chrome CSS 几何代理覆盖预览态和错误态，截图仅是 Node/浏览器代理证据。最终生产 Mini verify 通过（总包 4,802,913 B、主包 1,825,803 B）；相对已上传 `.192` 构建 4,801,680 B 增加 1,233 B，是返回登录与瞬时提示所需代码的净变化。最终 `pnpm verify` 通过（Mini 1262/17 跳过，根 1306/451 跳过），`pnpm smoke:check-core` 通过。
- 本轮没有上传；`.192@162ef4c1` 不含本轮改动，小米 14 结果未验证。唯一下一任务：授权后上传当前干净 checkpoint 的不可变体验版并按同 SHA 在小米 14复核；状态 `UPLOAD_REQUIRED`，未获当前轮次上传授权前不得分配版本。
- 轮次记录：[邀请绑定页访客清理与布局修复](invite-binding-guest-removal-20260925.md)。

## 上一批次：按日期发布与绑定页面修复（体验版已上传，待小米 14 复核）

- 基线为 `.191@c6c4fcd2` 累积源 `38d08103`；本轮 `162ef4c1` 已推送，成员绑定页改为一次确认、完整姓名/工号、访客码入口；草稿/发布预览定位一次切换，冲突提示只列真实重复日期。
- 真实 MySQL 手排 35/35、发布 3/3、仓库合并/工作流及恢复 10/10、绑定 8/8、迁移 28/28；本地 API/Web 浏览器 smoke 与完整 `pnpm verify` 通过（Mini 1257/16 跳过、根 1306/450 跳过），恢复脚本追加后定向 build/typecheck/lint/数据库复测通过。Mini production verify 与开发者工具局部 WXML/WXSS 编译通过。模拟器原始截图仅为 35×75，390×844 和 320px 视觉对照及小米 14 体验版尚无有效证据。
- 加密备份 `9c86d238-cdb6-438c-bb7a-37549106b6e5` 核验后部署 `162ef4c1`/schema 64；2026-12 只读守卫通过后受控恢复，当前修订 6 覆盖 1–31 日，31 日 ID 保留，日历/CSV 各 31 条，统计刷新、补偿通知 0。恢复后完整 ECS verifier 通过。
- 不可变体验版 `.192@162ef4c1`（Manifest `5c94240f…beefa`）上传并 add-only 放行，`.192/.191=200`、未知版 426；未提审/正式发布。开发者工具截图两次因 `waitForAutomatorReady timeout` 失败，390×844、320px 和小米 14 视觉未验证。唯一下一任务：取得与 `.192@162ef4c1` 一致的小米 14 原生截图/录屏，核对绑定、访客入口、预览定位和日期级覆盖；未取得前保持“待用户复核”。详见 [交付记录](date-granular-publish-binding-20260924.md)。

## 上一批次：手动排班预览对照与访客审计去重（生产与体验版已交付，待小米 14 复核）
...
- 模板/岗位首行约 2:1 且不换行，其余两行等宽；共享 selector 的删除 action 默认关闭、独立冒泡边界，不改变其他选择器。
- 未保存/已修改编辑可直接预览并用同一不可变快照创建草稿；模板保存与一次性排班彻底解耦。周期矩阵仍为 30 天/20 人/600 格，应用范围独立为含首尾最多 366 天，保存模板保留自选日期。
- 已保存模板提供带摘要二次确认的软删除；删除当前模板回到新建，删除其他模板保持当前编辑。
- 自动化：Mini 1248/16、根 Vitest 1302/444、真实 MySQL 手排 34/34；typecheck、format、lint、build、生成器、production verify/package/source/determinism、CI dry-run和浏览器 smoke 通过。浏览器本地合成管理员标记已恢复，服务已停止。
- 开发者工具门禁、局部编译、390px 模拟器布局/删除胶囊和 Console 过滤通过；截图只算模拟器证据。`eacfd752` 的 `.190`（232 文件、ZIP 2,675,243 B、Manifest `79c51d52…d4b7`）身份/receipt 一致。生产备份 `18760517-efb4-45fa-9012-5b478e658db5` 后部署 `b17a4e57`/schema 63；`.190` add-only 放行，allowlist/ECS verifier、`.190/.189=200`、未知版 426、新路由 401 通过。未写业务数据；仍待小米 14 `.190@eacfd752` 原生复核。详见 [本轮记录](manual-schedule-inline-editor-20260923.md)。
...
- checkpoint `531d7c399cde3407844162fba0104d8fde8d797f` 已推送；干净 production 候选上传并只增放行 `0.1.0-p10.20260922.189`（232 个代码文件、ZIP 2,648,616 B、Manifest `5b8e241ec169e38fe21a71a95d3522682a0d62170055f42ce0b15cbe140da2e1`）。健康、策略与完整生产 verifier 通过；生产应用仍为 `cfa934d1`/schema 63，未部署、备份、迁移、提审或正式发布。唯一下一任务是小米 14 冷启动 `.189@531d7c39` 后分别第一次打开手机号/短号弹窗；取得同构建真机证据前不写原生通过。完整上下文见 [审计记录](profile-qr-visitor-audit-20260921.md)。
