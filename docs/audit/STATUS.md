...
## 当前批次：全量选择器统一与长列表安全区（已交付，待小米 14 复核）

- 27 个列表型入口已统一到共享 selector；邀请/访客、补录岗位和诊断目标的原生列表 picker 已迁移，排班配置 4 个时间 picker 已复用共享滚轮，生产源码原生 `<picker>` 计数为 0。应用仍由自动门禁强制 WebView-only。
- 长列表缺陷根因是滚动视口自身承载 padding；现由内层内容盒提供安全区，向上和向下展开滚到底均不会让末项侵入圆角。所有页面级 selector 都测量自身滚动容器边界后决定方向和高度。
- 自动化：Mini 1244 通过/16 跳过，typecheck/lint/format/smoke/source/package/determinism/dry-run/production verify 均通过；主包 1721294 B、总包 4630569 B，Worklet=0。开发者工具 WebView 模拟器验证双方向末项安全区与 `08:05` 时间滚轮，Console 无 error；小米 14 尚未验收。
- 交付：`91b19bcf` 已推送；体验版 `0.1.0-p10.20260920.184`（Manifest `407a236f…bb33`）上传成功，远端 tag/receipt/分配记录一致。可信 allowlist 只追加 `.184` 并保留 `.183`，verify 通过；公网 `.184`/`.183`=200、未知版=426、health=200，线上 release 仍为 `bc5fc307`。未提审、未正式发布、未部署 API/Web。
- 当前唯一下一任务：用户在小米 14 复核所有迁移入口；取得同构建证据前不得写真机验收通过。
