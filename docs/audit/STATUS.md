...
- 开发者工具编译/渲染正常，模拟器截图确认图标对齐，Console 无相关错误；工具选择器不能跨自定义组件边界，故真实整行点击仍需新体验版小米 14 复核。当前 checkpoint 推送后状态为 `UPLOAD_REQUIRED`；没有当前精确 SHA 的上传授权，不分配版本。
...
- 唯一下一任务：用户授权上传本轮精确 checkpoint 后生成新体验版；小米 14 用新构建复核联系方式弹窗。`.185` 只继续承担二维码、访客详情及原导航复核；未取得同构建证据前不得写真机通过。不提审/正式发布。详见 [完整记录](profile-qr-visitor-audit-20260921.md)。
