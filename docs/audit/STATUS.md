...
## 当前批次：补录周历与手排预览共用日历（.196 已上传，放行待前置生产部署）

- 本轮基线 `5befe6a3`，修复班种徽标显示、补录读取群组周/月偏好，并让三类手排预览周历共用既有日历分页/单元格；详细根因与证据见 [轮次记录](schedule-week-preview-20260925.md)。仅 Mini 与文档范围，不触发生产部署。
...
- 经本轮授权上传 `0.1.0-p10.20260925.196@aef86299`，production Manifest `16a51495e92e8f2d60988432632602d0490bb8a5335bb54e559576e83e4e0beb`、receipt、远端 tag 一致。放行手册要求同 checkpoint 先部署生产，本轮未获明确备份/部署授权，尚未连接生产或放行。唯一下一任务：获 L4 授权后备份、部署、验证、只追加 `.196` 白名单，再取小米 14 同构建证据。
