...
- 开发者工具 0.3.11 门禁、页面/selector WXML/WXSS 编译、390px 模拟器布局和下拉删除胶囊截图通过，Console error/exception/fail 为空；截图只算模拟器证据。新 API 尚未部署生产，当前消息没有生产授权；体验版上传不代表生产端到端可用。下一步为提交推送、不可变体验版上传；320px/大字号由静态门禁覆盖但仍待小米 14 原生复核，生产接口也继续标记未验证。详见 [本轮记录](manual-schedule-inline-editor-20260923.md)。
