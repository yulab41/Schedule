...
## 补充：模拟器不是 3.17.3 的可信参照（重要边界）

为了做"视觉完全一致"的逐像素对照，我在测试工具里放了同一个对照轮（11 项 2021–2031、`selected-index=5`），同一构建分别用 3.17.2 与 3.17.3 基础库截图对比（crop 10,495–360,705）：

- 3.17.2（compat 分支）：滚轮正确停在 2026、中间项放大、单位 12 行出墨；均方差异 1.556。
- 3.17.3（WXS 分支）：组件数据 **正确**（`compat=false`、`wheelTrackStyle='margin-top:-220px'`、`wheelLayoutIndex=5`），但**渲染**停在第 0 项（2021）且**单位不出墨**。

→ 该模拟器**在两个基础库上都不应用内联 `margin-top`、也不绘制 `color:currentColor` 的单位文本**，所以它只能当"受影响版本"的参照，不能当 3.17.3 的参照。真机 3.17.3 这两条都正常（用户多轮截图互证）。

**因此"与 3.17.3 逐像素一致"无法在模拟器里证明**，只能靠小米 14 双实例对照；本轮不再把模拟器对照当作该结论的证据。修复本身不依赖模拟器：它规避的正是两个模拟器/受影响设备都不支持的通道（内联 `margin-top`、`currentColor`、WXS `setStyle`），改走数据与原生滚动。
