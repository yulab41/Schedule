...
- 置信度：高。
- 状态：代码 `7952f1d1` 已推送并完成生产备份、部署和 full verifier；尚未上传匹配体验版，小米 14 待验证。
- 验证：Mini 110 files/555 tests、root 244 files/1,139 tests；MySQL 集成因本机数据库不可用跳过。数据库查询路径中位数 50.12→48.36ms（-3.5%），P95 62.17→57.34ms；端到端 HTTP、原生动画和小米 14 当前工具无法测量。
