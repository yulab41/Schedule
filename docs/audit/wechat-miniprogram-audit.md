...
- 当前阶段：`MINI-G1-001` 异步生命周期失效边界已完成先红后绿修复；本轮起始主线为 clean
  `origin/main@eb36d70fa28cf8705b0795a516ea3d4420399efc`
- 更新时间：2026-09-01（Asia/Hong_Kong）
- 修复分支/worktree：`codex/fix-mini-g1-001-lifecycle` /
  `runtime/external-project-worktrees/mini-g1-001-lifecycle-20260901`
- 修复 checkpoint：以 `fix(miniprogram): invalidate stale workflow controllers` 识别；提交前最终 fetch
  后 `origin/main` 仍为 `eb36d70fa28cf8705b0795a516ea3d4420399efc`，任务起止主线一致
- 证据继承：远端审计提交 `4ddaa38e70adbc356e2cc390ea2c3ebe2ad30fd2` 只以内容合并方式带入三份
  状态/审计文档，没有从旧审计分支带入源码
- 本批性质：只修复 `MINI-G1-001`；未修改 `MINI-G1-002`～`004`、API、数据库、缓存策略或页面交互，
...
第 1 组审计找到的四项中，本轮只处理了 `MINI-G1-001`。旧实现的逻辑层竞态现已由仓库永久测试确认：
...
这项修复有 Node/静态自动化证据，不需要用户重复真机制造竞态。微信原生层是否曾把该竞态表现为可见
故障仍未直接确认，因此不能把逻辑复现夸大成“真机 P1 已复现”。其余 `MINI-G1-002`～`004` 的等级、
证据和建议保持原审计结论，本轮未顺手处理。

### 基线与工具事实

- 本修复轮次 `git fetch origin` 后从当时最新 `origin/main@eb36d70f` 建立独立 clean worktree 和新分支；
  起始状态为 clean，依赖按仓库策略在该 worktree 内安装并预构建 packages，没有借用主工作区 dist。
- 修复 worktree：`runtime/external-project-worktrees/mini-g1-001-lifecycle-20260901`；用户脏主工作树、历史
  `runtime/` worktree、release worktree和其他并行任务均未修改或清理。
...
| ----------- | ---- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| MINI-G1-001 | P1   | 已确认并修复（逻辑层） | `apps/miniprogram/src/subpackages/workflows/components/controller-host.ts:45-104,136-151,267-306`；leave `controller.ts:434-837`；swap `controller.ts:316-350,492-1011,1137-1170`；duty `controller.ts:427-902`；`apps/miniprogram/scripts/workflow-controller-lifecycle.test.mjs:29-329` | 永久测试在未改源码时 7/7 红：detach/unload 后回写、A→B 覆盖、重挂碰撞、并发乱序；统一 token/guard 后 12/12 绿；结构审计覆盖 32 个 async/83 个 await 和 3 个显式 `.then` | 修复前慢网、切群组或快速返回可让旧续体覆盖新状态；修复后旧续体零状态/UI 副作用，B 正常结果仍更新 | 已实施 host attachment/controller 对象 token、统一 dispose 和全异步 continuation task guard；保留 API/幂等语义 | 自动化已证明逻辑契约；微信原生可见故障未确认，当前不要求真机复现 |
...
continuation 合同，共 12/12。结构合同固定 leave/swap/duty 的 async/await 数量分别为 `8/23`、
...
| 修复后生命周期定向                  | 12/12 通过；8 个行为场景 + 3 个 controller async/await 合同 + 1 个显式 `.then` 合同             |
| 相关 host/controller                | 9 files / 61 tests 通过                                                                         |
| Mini 全量（修复后）                 | 112 files / 592 tests / 0 failure；73.03s                                                       |
| `pnpm miniprogram:verify`（修复后） | 通过；2/2 Worklet；总包 5,120,949B；main 1,677,802B；matrix 1445/1506；manifest `ad7c65bd…1420` |
...
- 本轮只修复 `MINI-G1-001`；`MINI-G1-002`～`004` 未修改。未上传体验版、未提审、未正式发布、
  未改 allowlist/production/数据库。

本轮在提交并推送新分支后停止，不自行开始 `MINI-G1-002`～`004`。若继续第 1 组，唯一建议下一轮只处理
`MINI-G1-002`（同年 holidays 请求去重），仍需独立先红后绿并保持缓存/API 语义；包体积与分包边界仍是
另一审计问题组，不与之混做。
