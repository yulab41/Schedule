...
## 2026-08-27 pnpm 构建脚本策略固定（已完成并部署）
...
- checkpoint/发布：首个 `aa2380a0`（`fix(tooling): stabilize pnpm build policy`）已推送，但在打包验证暴露第二层重复 install 后没有单独部署；最终 `51d74dcd7b33d1ac47788327498b7aa54d0c3a4d`（`fix(tooling): disable implicit dependency preflight`）已推送。该批不改变 Mini runtime，未另传体验版。部署前备份 `cee7a5a6-88ee-4ebf-ad9f-1a28e4ffe0af`（54 表、178,654 行、82,309,268 bytes、SHA-256 `d29b64bc0a9995a00e973d999c0675858433775232c814aceef4cfe8c7b3de13`）后部署最终 release；预热一次 502 后恢复、privacy 0/0，完整 verifier、`.45` capability、未知版本拒绝、env/产物/控制面/迁移/Host 隔离均通过，远端临时目录已删除。下一活跃批次仍只迁移 organization 的 directory、group-settings、scheduling-config 三页并停止。
...
- 实体结果：用户确认 `.43` 访客页已打开；生产 Page/controller 两个指纹和访客聚合/日志 API 请求完整出现，架构根因确认，转入上方 Phase 4 迁移其余两页。
...
- 实现/语义：全部 P9 页面显式建立 `page` 100% 高度、裁切和背景，并保持真实组件宿主 100% 高度；通知设置修正宿主选择器及页面配置。controller、请求接收者、Promise/catch、权限、capability、空值、调用次数、通知授权、导出和业务数据均未改；同分包通知中心一并覆盖相同页面壳边界。
...
- 红绿/验证：旧实现新增模板接入断言先失败 1 项；接入后通知 controller 6/6、订阅边界 4/4、原生通知设置 3/3、最终 P9/P10 矩阵 3/3（合计 16/16）通过；Mini 全量 84 files/388 tests、production typecheck/verify、source/package/performance/determinism、CI dry-run、根 build/typecheck、`smoke:check-core` 均通过。verify manifest `ee7a88d51c809688e04cec81e44b92cbb2cc32e83380a8d5e43a3e8a99f5079a`，总包 `5,758,943` bytes；仅既有组织分包 1.5M 预警和 600 格节点 best-effort 警告。代码 checkpoint `f1e45fe5` 已提交并推送。
- 生产能力/体验：备份 `b78654b6-f599-4dd7-8619-919e96d9d57b`（54 表、176,868 行、81,712,736 bytes、SHA-256 `28a45630ab7dcf23fb48e52075c62f4a3bded08b69ea205a8bfdf67d141953eb`）保护能力切换；`insights=true`、`externalMessages=true` 已通过双锁策略探针。`.38` 体验版上传成功（153 个代码文件、zip `1,404,386` bytes、上传 manifest `faf80861587f5a10a9b95fe2b2dad4983c2df22785e605d3fd8dafb9b54d23d8`）并已加入白名单；代码 ECS checkpoint `f1e45fe5` 已在备份 `55184e32-c479-4e83-9714-ad915467fb0d` 后部署，随后状态 docs checkpoint `13b3b1fc` 在备份 `41fa7b5e-39dc-4fbc-9cae-6394bf24ed6a` 后完成 docs-only 部署；两次完整 verifier、`.38` 探针、`root:root/0600` 和临时目录清理均通过，当前 `current-release=13b3b1fc`。
- 微信平台状态：官方类目查询返回“工具/办公”；使用服务器普通 App access token 调用 `get_latest_auditstatus` 与 `submit_audit` 均返回 `errcode=86000`（仅允许第三方 `rid`），未生成审核编号，因此未执行 `release`。浏览器控制台被安全策略阻止读取，未绕过或改用替代浏览器。
- 下一步/停止条件：需要微信第三方平台 `component_access_token/authorizer_access_token` 或在公众平台控制台完成 `.38` 提交审核；取得审核通过状态后再执行正式 `release`，随后复核微信平台状态、生产能力和回滚边界，才可收口全部阶段。
...
- checkpoint/下一步：P9/P10 自动矩阵、P7–P10 清单状态和迁移计划 capability 边界审计均已完成；代码/文档审计 checkpoint `640ff2c8d20dea9640688fa54ceef0ddb70552ef` 已提交、推送、体验上传、白名单追加和生产部署/验证；本状态同步前数据库备份 archive `b5346c59-eaa5-48ff-aa74-2cc237a6420f`（54 表、176,643 行、81,638,740 bytes、SHA-256 `c8f042c9a3d1ec0b78a44e815849df22a18ddb9147d9962dce82cb67da61dc2a`）。剩余仅为需用户明确授权的 `insights`/`externalMessages` 开启、订阅消息和正式审核/发布，不由自动验收推断；下一活动批次在获得授权前保持 capability 关闭并继续保留可接续状态。
...
- 当前策略：用户已明确“无需人工复核”，本项以自动回归、确定性构建、包边界和生产 verifier 验收；生产已支持体验候选 `.36`，`organization=true`、`insights/externalMessages=false`，不提审、不正式发布、不自行开启 P9 能力。
- 线上发布：代码 checkpoint `a2aa7a20b5339e4b1ea1924334d12f1bf6c1cbac`（`fix(miniprogram): close export capability state`）已推送；体验版 `0.1.0-p9.20260827.36` 上传成功，153 个代码文件、zip `1,404,292` bytes、上传 manifest `83c82f93bf3e060c9e877649d56b29364a7527c77e9f0d64983c445168817123`。代码部署前数据库备份 archive `a9120019-71c3-4223-9913-f251c80d6ab7`（54 表、176,499 行、81,586,224 bytes、SHA-256 `0773f119bbcb9f050e6e697344c0c6668cafefdf206f4b0196b043db35dff6eb`）；`.36` 已原子加入版本白名单，能力值未新增开放。
- 线上验证：代码 release `a2aa7a20b5339e4b1ea1924334d12f1bf6c1cbac` 与最终 docs-only release（本提交）均已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.36` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 与 Git HEAD 一致，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- checkpoint/下一步：代码 checkpoint `a2aa7a20b5339e4b1ea1924334d12f1bf6c1cbac` 与状态 docs 连续 checkpoint 均已提交、推送并完成生产部署/验证；最终同步前数据库备份 archive `2b8697a2-924d-48bf-9218-8ab7a0dcdbca`（54 表、176,510 行、81,590,260 bytes、SHA-256 `79fa7ca0c37701a43e407c3a70ef5d07cf21e5434cf7591738ce609bc224da37`）。本轮用户已明确无需人工复核，自动测试、构建、能力探针与 verifier 为验收依据。下一活动批次为 P10/P9 剩余状态分支自动审计（只处理 1 项），停止条件是覆盖矩阵无未覆盖项且验证/发布闭环通过。
...
- 当前策略：用户已明确“无需人工复核”，本项以自动测试、确定性构建、包边界和生产验证验收；生产已支持体验候选 `.35`，`organization=true`、`insights/externalMessages=false`，不提审、不正式发布、不自行改变能力。
- 线上发布：代码 checkpoint `1725ee7a5e62a65237193d5cf95092f435cad16b`（`fix(miniprogram): support group settings large text`）已推送；体验版 `0.1.0-p9.20260827.35` 上传成功，153 个代码文件、zip `1,404,131` bytes、上传 manifest `94c3e00e3577acc3639f54b8a36fb587455a98efcc847e1c27671dd348826d65`。代码部署前数据库备份 archive `7823ca56-7120-423e-a34d-b696bb2535ea`（54 表、176,331 行、81,527,260 bytes、SHA-256 `0574d8a6407416bd21aececfbdb870df26486fff21836148033a9f201beef9b1`）；`.35` 已原子加入版本白名单，能力值未新增开放。状态 docs 的最终同步 checkpoint 为本提交。
- 线上验证：代码 release `1725ee7a5e62a65237193d5cf95092f435cad16b` 与最终 docs-only release（本提交）均已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.35` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 与 Git HEAD 一致，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- checkpoint/下一步：代码 checkpoint `1725ee7a5e62a65237193d5cf95092f435cad16b` 与状态 docs 连续 checkpoint 均已提交、推送并完成生产部署/验证；最终同步前数据库备份 archive `a7818262-51bf-485c-ba8d-9f1c4846ed92`（54 表、176,457 行、81,572,068 bytes、SHA-256 `93899bf9eb6fe11f671b73fd8f0bdda257126a7cb4f8677aadfba957509979c8`）。本轮用户已明确无需人工复核，自动测试、构建、能力探针与 verifier 为验收依据。下一活动批次为 P10/P9 剩余状态分支自动审计（只处理 1 项），停止条件是覆盖矩阵无未覆盖项且验证/发布闭环通过。
...
- 当前策略：用户已明确“无需人工复核”，本项以自动测试、确定性构建、包边界和生产验证验收；生产已支持体验候选 `.34`，`organization=true`、`insights/externalMessages=false`，不提审、不正式发布、不自行开启 P9 能力。
...
- 线上验证：代码 release `3b32fd0feee51d755ff83f86d757c194662398f1` 与最终 docs-only release `c82dcf6922345023aa3483ea5dcf81c4bcb672a8` 均已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.34` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 为 docs-only release，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- checkpoint/下一步：代码与 RC 文档 checkpoint `3b32fd0feee51d755ff83f86d757c194662398f1` 已提交、推送、体验上传并完成生产部署/验证；状态文档 checkpoint `c82dcf6922345023aa3483ea5dcf81c4bcb672a8` 已推送并完成 docs-only 部署；本轮用户已明确无需人工复核，自动测试、构建、能力探针与 verifier 为验收依据。下一活动批次为群组设置 panel 与剩余 P10/P9 状态分支自动审计（只处理 1 项），停止条件是覆盖矩阵无未覆盖项且验证/发布闭环通过。
...
- 线上发布：代码 checkpoint `e5dac58d03b98d5b1a43e3dc27df6fef83887eed`（`fix(miniprogram): support p8 panel large text`）已推送；体验版 `0.1.0-p9.20260827.33` 上传成功，153 个代码文件、zip `1,402,885` bytes、上传 manifest `d0dd0d8efef2724f001ff3c03bc1bdd20f096bf580adda2a37543d279aa05788`。加入白名单前数据库备份 archive `d4a3acce-ba47-4f25-b6ed-3a2c188d3149`（54 表、176,108 行、81,451,776 bytes、SHA-256 `b432bb1d566f8eae1530705f2e0931843b851e76d979984ca1816c7bda102c18`）；原子追加 `.33`，未改变能力开关。状态文档 checkpoint `499746ffb55950603ba443f69bfe650b45cbcdd6` 也已推送并完成 docs-only 部署；该次备份 archive `46b4f561-e651-428a-81c9-32e8b42e644f`（54 表、176,138 行、81,462,080 bytes、SHA-256 `eeef9d954ae2c307f3169ff559cb9c44c60833bbf564caa5586f1edf8c81457b`）。
- 线上验证：代码 release `e5dac58d03b98d5b1a43e3dc27df6fef83887eed` 与 docs-only release `499746ffb55950603ba443f69bfe650b45cbcdd6` 均已部署，预热首个 TLS/健康请求短暂失败后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.33` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 与 Git 一致，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- 当前策略：用户已明确“无需人工复核”，本项以自动状态/静态矩阵、构建、能力探针和生产 verifier 完成验收；`.33` 不提审、不正式发布，群组设置 panel 保持后续独立任务边界。
- checkpoint/下一步：代码与状态文档 checkpoint 均已提交、推送并部署，Git、origin 和服务器 `current-release` 一致；本次 docs-only 发布的 verifier、能力探针和远端临时目录清理均通过。下一活动批次为群组设置 panel 与 P10/P9 剩余状态分支自动审计（只处理 1 项），停止条件是覆盖矩阵无未覆盖项且验证/发布闭环通过。
...
- 自动验证：Mini 全量 83 files/377 tests、typecheck、production verify、source/package/determinism/CI dry-run、根 build/typecheck 已通过；最终 verify manifest `007b88f71ee90f8f23bd95aa166671c762dd34f48b1e9e94b4323328fb091101`，总包 `5,745,584` bytes，organization 分包 `1,742,633` bytes（仅既有内部 1.5M 提示）。生产当前 `.30` 白名单，`organization=true`、`insights/externalMessages=false`。
- 当前策略：用户已明确“无需人工复核”，不等待微信/Android 操作；`.31` 仅修正 P10 profile 大字号可读性，不提审、不正式发布、不改变能力。
- 线上发布：代码 checkpoint `e2cdf561c0b2d459fe24afcf7023d0c47330de22`（`fix(miniprogram): keep profile large text readable`）已推送；体验版 `0.1.0-p9.20260827.31` 上传成功，153 个代码文件、zip `1,401,670` bytes、上传 manifest `4f5d67dd7cdb756dfb038a87e2a2dce70fc0adce71da2a7841c57a08d206fbdf`。加入白名单前数据库备份 archive `8cab0ece-1a3a-4594-a98c-836cfa2a11fc`（54 表、175,922 行、81,388,008 bytes、SHA-256 `6f9f1143b02bffd5a38af9af9173edb0fd1c2e3bc449cbdc233ec6256b9dd457`）；原子追加 `.31`，未改变能力开关。
- 线上验证：release `e2cdf561c0b2d459fe24afcf7023d0c47330de22` 已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均由 `ECS_PUBLIC_IP=120.77.220.79 bash infra/scripts/ecs-verify.sh` 通过。`.31` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；`current-release` 与 Git 一致，远端本轮临时目录已清理。
- 当前策略：用户已明确“无需人工复核”，本项以自动测试、确定性构建、能力探针和生产 verifier 完成验收；`.31` 不提审、不正式发布，P9 能力仍不自行开放。
- checkpoint/下一步：线上结果待状态提交，提交消息拟为 `docs(status): record p10 profile readability deployment`；提交后按根规则备份并部署 docs-only release，使 Git、origin 和服务器 release 再次一致。本对话已完成 3 个实现批次；下一活动批次为 P10 最终路由/角色/状态/错误分支自动对等收口（只处理 1 项），停止条件是全量矩阵无未覆盖分支、验证/生产发布/能力探针全部通过。
...
- 范围与冻结：只把未修改 Web `ExportDialog.vue` 的导出规则、筛选、状态和安全下载适配到 Mini；`apps/web` tracked diff 仍为 0。生产 `insights=false`、`externalMessages=false` 保持，访客访问留给下一独立任务。体验版 `.27@82840db` 已上传，未提审/未正式发布。
...
- Mini 产物：production verify manifest `f7d1e989f2b187212b6658613723c05197f2c21845112a281a0f6a51d1afa0bf`，总包 `5,729,604` bytes，insights 分包 `1,302,744` bytes，source/output worklet 均为 2；CI 上传 153 个代码文件、zip `1,396,770` bytes、manifest `8efd9c4e8e439710ed0a9d16118a3cc8389b3b02430651e1c3ea4c9809529460`。organization 分包既有 1.5M warning，未触及本批功能限制。自动验证已完成，320/390/大字号实体微信渲染仍由用户体验版/实体 Android 复核关闭。
...
- checkpoint/生产：代码 `82840db9d0cfbccf12a389b24397d662a11a5239`（`feat(miniprogram): align Web export and secure download`）已推送并部署；部署前加密数据库备份 archive `332096ae-6ae1-42bf-81e3-30d1278502d6`（54 表、175,543 行、81,259,740 bytes、SHA-256 `fa2043f0784c7cd67b7afa0490d91cbb82faf8a870bcc973e5754229faf5b4bd`）。为追加 `.27` 白名单再次备份 archive `d9cd370c-4d74-40e3-b02c-1d9c66846a25`（54 表、175,555 行、81,264,260 bytes、SHA-256 `430f478f40a1fe779fbe12b6372c5f08bef710d74369caee5349f2e3f066b332`），只追加版本、不改变能力开关；当前 release 与 Git 一致，远端临时目录已清理。
- 线上验证：部署预热一次 502 后恢复；两次 `ECS_PUBLIC_IP=120.77.220.79 bash infra/scripts/ecs-verify.sh` 均通过健康、产物/控制面哈希、迁移、域名隔离、容器和依赖检查；privacy-retention `deletedRows=0/remainingRows=0`。`.27` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`。上传前已主动说明版本、更新内容、影响范围、能力开关和验收重点；不提审、不正式发布、不打开 P9 capability。
- 当前状态：已完成（含代码发布、体验上传、生产备份、版本白名单、部署与线上验证）→待用户体验版/实体 Android 复核。320/390/大字号实体微信渲染、能力开启隔离环境中的导出真实任务和下载/打开行为仍需用户验收。
- 下一活动批次与停止条件：本批最终状态 checkpoint 需把上述线上结果写入状态文档并同步生产 release 后停止；等待用户复核 `.27` 的 loading/筛选/超时继续/失败重试/下载安全和 320/390/大字号。导出用户复核关闭后，下一实现批次才进入访客访问 Web→Mini 适配。
...
## 2026-08-26 组织/P9 panel 生命周期与缺失群组状态修复（已实现，待体验版复核）
...
- 验证：P10 个人中心静态回归 3/3（新增账号密码解绑可用性回归，先红后绿）、Mini typecheck、production verify/source/package/determinism/CI dry-run 通过；总包 `6,945,752` bytes，组织分包 `1,649,991` bytes，manifest `2ea366c256a0308e3e8d28c0c2f7f17a1eee134c8c94fb373c1dc13d01d4a0ec`。
...
- 增量 checkpoint：代码 `a9966222`（`fix(miniprogram): guard profile unbind by auth method`）已推送并部署；账号密码会话不会再显示可触发失败的微信解绑操作。
- 增量体验/运行：`0.1.0-p9.20260826.15` 已上传成功，147 个代码文件、zip `1,303,738` bytes、manifest `c230c0a01eadb38a1eea8b2dca86e1f92f420e883f6b16d070e4a934f55b5701`。生产备份 archive `16fbc2dc-0960-46dc-9d44-7cb4dcc7b5a5`（54 表/171,835 行/80,028,896 bytes/SHA `2d950ca2d75c4bff0a18b81ffb14e2eb2e3eeabfcbe6ef64f44de363c286b4cf`）后追加候选白名单；候选 capability HTTP 200，`organization=false`、`insights=false`、`externalMessages=false`。
- 增量发布：代码 checkpoint `a99662229cb1fba71d6203c787e49f6736a3cf84` 部署 release `a99662229cb1fba71d6203c787e49f6736a3cf84`；ECS full verifier、health 200、候选版本 capability 和远端临时目录清理通过。
...
- 页面黄金清单已与 `src/app.json` 对齐：组织管理、访客访问、事件/统计、通知、导出、通知设置、通讯录和个人中心路由均已登记；P8-B 与 P9-A4/A5/A6/A8/A9 不再误报原生页面缺失。
- 新增 `p9-p10-golden-manifest.test.mjs`，覆盖 11 个原生路由和已实现边界状态；定向测试 3/3 通过。生产能力开关与体验版候选不变。
...
- 回归补强：新增原生 controller 2/2，实际驱动 mock `wx.request` 覆盖 facets、文本搜索、`campusCode` 字段映射、独立筛选和 cursor 加载；修复了筛选键与页面 option/index 字段名不一致导致的搜索异常。
- 拨号补强：完整号码归一化后提供 44px 可触达的 `wx.makePhoneCall` 入口；只有内线号的联系人保持文本只读，不会误触发拨号。
- 员工模式补强：controller 回归扩展至 4/4，切换 `employee` 后筛选标签改为组织根/一级至五级组织，且不会被旧属性 observer 重置回 `internal`。
...
- 回归修复发布：`0.1.0-p9.20260826.11` 已上传成功，144 个代码文件、zip `1,292,661` bytes、manifest `35a0aa33ad9553001d8aba7d2add291047274c4073cc33de3365c5ae9d705c1d`。生产备份 archive `c19a59f1-fb07-4a5f-b326-182a265964b5`（54 表/171,476 行/79,902,028 bytes/SHA `104f8a2ac6e92d177c92c33b091b7aa76203c9f77c521255b77671075d2e8c74`）后追加候选白名单；代码 checkpoint `b7485aa2`（`fix(miniprogram): harden p10 directory filters`）部署 release `b7485aa200b70b011a127584a8aad4ff462dfd08`，ECS verifier 通过，`organization=false` 保持关闭。
...
- 员工模式发布：`0.1.0-p9.20260826.13` 已上传成功，144 个代码文件、zip `1,293,836` bytes、manifest `a098b9020049984ce89c29d0f2655b8589c762913ad20b4a00fda7eb1235b36d`。生产备份 archive `055250d6-7a68-4da3-b06f-7fa71b063f3d`（54 表/171,580 行/79,939,128 bytes/SHA `47c91335b4e96626ee935ed232108ddf371bbc17e0ecf783022d163997494765`）后追加候选白名单；代码 checkpoint `8f5b1b42`（`fix(miniprogram): align p10 employee filters`）部署 release `8f5b1b42d6e30d10422bbb3d8185becddd5092d7`，ECS verifier 通过，`organization=false` 保持关闭。
- 状态同步：docs checkpoint `2c43bff5` 在备份 archive `a768b5fe-1236-46a1-a78d-d8caea7faf38`（54 表/171,591 行/79,943,164 bytes/SHA `c3a17898de3b5c39aa7f3fec226b8d463b29b48eeaf334a3eb3c7517b5da20e8`）后部署 release `2c43bff52c13ab6e970ca9b71449e5a1dbe8541e`；full verifier、候选 capability 200 和远端临时目录清理通过。
- 员工模式文档同步：docs checkpoint `6f5a5e16` 在备份 archive `cb2a044e-e355-41f4-a26d-78ad5c85c901`（54 表/171,606 行/79,948,648 bytes/SHA `a20693b16fa03f3e0d7353d1a571ba4aecc8096dbb8320402538ba9fac675b51`）后部署 release `6f5a5e16faf95ab5aa4f4a9a7bad55e21342d781`；full verifier、候选 capability 200 和远端临时目录清理通过。
- 当前文档同步：docs checkpoint `e2490ab4` 在备份 archive `21258b3a-cd58-4c9d-bfc3-42fb34bc282b`（54 表/171,620 行/79,953,648 bytes/SHA `e76d56fa607d93658669ed6441929507ccba4cbc887fc292fb83b222e2979157`）后部署 release `e2490ab4f4d9cb288d26492e7a3bb6cbdbec46c6`；full verifier、候选 capability 200 和远端临时目录清理通过。
- 最终状态同步：docs checkpoint `c33b2923` 在备份 archive `308ee848-a138-4acd-9cde-baa6135f7c70`（54 表/171,631 行/79,957,684 bytes/SHA `8f502f1cb2d402688262af25391934b7dcac43aa05fcfab63b3d2c44ed7d5275`）后部署 release `c33b2923aac45334dd33f2aedddf045dca778f2d`；full verifier、候选 capability 200 和远端临时目录清理通过。
...
- 状态同步：docs checkpoint `e96b167f` 在备份 archive `fd0defd9-da51-4ffd-b52c-50071bfb2693`（54 表/171,083 行/79,765,584 bytes/SHA `91130ca2394a5869f59610ae70e1dbd0c86bfcd079a8caee269ca801c3ba7290`）后部署 release `e96b167f221bcca62c310d7d758b6f8ace7304ca`；候选 capability 200、full verifier 和远端临时清理通过。
...
- 状态同步：docs checkpoint `6283f637` 已推送；在备份 archive `7333873d-ee32-4d0a-88c5-0769ae291345`（54 表/170,712 行/79,635,364 bytes/SHA `f604b5f15bf99219ce2858f055b476064d635132a8824c0bacb1cbebac4ce312`）后部署 release `6283f637bdf94b1e45aa047ee9020843c7c3a9b2`，full verifier、候选版本 capability 200 与远端临时目录清理通过。
- 体验/状态：移除本机代理后使用仓库外 key 上传 `0.1.0-p9.20260825.1` 成功，117 个代码文件、zip 957,670 bytes、manifest `493a1e034a0307745d8cdae677f68e6c7e3791bacbbbb3300624fc310528920d`，未提审/未正式发布。随后生产备份 archive `e03509e7-7ae5-4fae-9579-cf2f7877d2ed`（54 表/170,702 行/79,631,812 bytes/SHA `56b9a9525dfbeff4becb520a8b8440d3398c56e98b008efb87b7fbbf335c02a5`）后原子追加 `0.1.0-p9.20260825.1` 到支持版本白名单；有效能力 HTTP 200，`core/workflows/guest=true`、`organization=false`、`insights=false`，env 保持 `root:root/0600`，full verifier 再通过。正式审核/发布不在本批范围。用户需在 Android 实机验证管理员/成员权限失败关闭、访客访问记录、加载更多、弱网、前后台和大字号。
