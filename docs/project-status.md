...
- Debug rounds: 1–18 已完成；最新验证基线 `pnpm verify` 352/352（60 个测试文件，隔离 MySQL）
- Next actions: 部署后由用户验收（当前重点：8/3 请假审批应无重排、8/6/8/7 手动安排提示、换班候选过滤、事件弹窗、草稿整段发布与发布记录历史）；用户后续反馈按 debug 日志模板追加轮次
...
- 2026-08-03 调试期轮次 1–18 全部完成并部署（用户反馈、根因、修复、验证详见 debug 日志；最新基线 352/352）。
