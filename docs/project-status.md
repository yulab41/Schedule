...
2. 用户再完成 1 次独立“下次 App 启动首次搜索诊断”，以同一 3 字姓名查询作为页面第一次搜索并只
   复制该条；若继续亚秒则停止改动、观察长尾，若再次数秒再评估查询计划或缓存。

停止条件：最后一条复现数据返回前不改搜索、数据库、索引或缓存，不再次上传、修改 allowlist、部署、创建
生产备份或同步服务器 release；微信原生 Console、半屏拖拽/回弹/滚动手感继续待人工验收。
