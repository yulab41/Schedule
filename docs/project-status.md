...
- `.75` 三次独立启动首搜为 10,016/685/799ms；后两次亚秒，10 秒 rows/main-query 长尾转观察，
  仅在 total >2,000ms 或 main query >1,500ms 再调查，不授权改查询、索引、数据库或预热。
...
3. 唯一下一任务：从最终 `origin/main` SHA 开始“静态审计第 1 组：页面状态、异步链路与列表性能”。

停止条件：本轮完成 docs-only checkpoint 并推送后停止，不开始静态审计第 1 组；不再次上传、提审、
发布、修改 allowlist、部署、创建生产备份、同步服务器 release，或改 API/数据库/索引/缓存。
