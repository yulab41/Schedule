...
## 当前批次：Feedback13四项日历修复代码完成，待授权交付

- 用户要求简称同步、月历隐藏全天标识、护士周历底部防圆角碰撞和点选高度稳定。根因、引入点和验证见docs/audit/feedback13-calendar.md。
- API按稳定班种ID覆盖当前简称，只改只读响应、不写历史记录；前端去掉全天“全”硬编码，月历按isAllDay隐藏标识、保留人员。周历下留24px，按实际内容缓存实测高度，空/迟到测量不覆盖有效高度。
- pnpm verify完整通过：Mini1170/15跳过、根1251/436跳过、依赖保护81；追加普通周/8月31日跨月周两项高度回归通过。隔离MySQL calendar36、390/320生产CSS/ViewModel几何及完整本地浏览器冒烟通过；最终Mini verify通过，未获小米14同版本验收。
- general-4/5独占复用，无安装、新依赖或迁移。检查点fix(calendar): sync abbreviations and stabilize week layout；本轮没有生产连接/部署/上传/放行。
- 唯一下一任务：对精确检查点申请API/Web部署及累计体验版上传/追加放行；需同时包含已实现的五类通知授权按钮。上一轮服务器3864b275/五模板已启用、手机109为历史已验证记录，本轮未重新查询生产；不重复通知配置、护士导入或历史推送。
