...
- 下一任务/停止条件：小米 14 打开体验版 `0.1.0-p10.20260919.179`，确认（1）切月/切周不出网无等待；（2）进入页面/回前台只发一次 `calendar-changes`；（3）后台改一条历史排班后回前台静默更新；（4）观感与 `.177` 一致。未取得与 `063bb67a` 一致的真机结论前，不得写"小米 14 验收通过"。仍有两项未闭环：MySQL 集成用例需在有测试库的环境补跑；开发者工具请求数探针与端到端静默更新观察未做。
