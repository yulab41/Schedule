...
- 本次功能 checkpoint 提交信息：`fix(miniprogram): close EXP-UX-001 experience regressions`（本文件更新后提交）。
...
- Mini：`pnpm miniprogram:build`、`pnpm --filter @schedule/miniprogram typecheck`、source audit、
  `pnpm miniprogram:verify` 均通过；verify packageBytes `5,113,437`，manifest
  `c3d3ffe59e425da12766d1c17f2f58a17a80a1bf3e504f4685939a29ad61ecee`。
- 包体同口径从 `5,121,616` 降至 `5,113,437`，实际减少 `8,179` bytes；主包 warning 和矩阵 warning 保持
  既有类别。
