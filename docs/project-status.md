...
## 当前批次：Skyline 3.17.2 专属 UI 兼容已实现，待体验版授权
...
- Mini verify仍仅被既有未改手排节点预算`1507>1506`阻断，本轮不修改手排或放宽测试。检查点以`fix(miniprogram): isolate Skyline 3.17.2 UI fallbacks`识别；首次正式上传在调用微信平台前因工作台TS blob变化使`5285dd1`等价血缘证明失效而安全停止，未占用版本。兼容改动未触及受保护方法，现更新精确blob证明并复跑血缘门禁。未调用微信开发者工具，尚未上传、放行、部署生产、改数据库、提审或正式发布。
- 唯一下一任务：检查点推送后由用户精确授权上传，再用同一新体验版分别验证3.17.2实例恢复和3.17.3实例视觉不变。
