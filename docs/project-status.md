...
- Debug rounds: 1–22 已完成；最新验证基线 `pnpm verify` 353/353（60 个测试文件，隔离 MySQL）
- Next actions: 本地 `pnpm dev` 验收 + 部署后线上验收（当前重点：加扣班事件人类化显示、草稿编号与归档折叠、8/3 请假审批应无重排、8/6/8/7 手动安排提示）；下一轮规划：月历式草稿/发布/应用预览、发布记录撤销/恢复/删除、发布前工作流清理、未来日期发布限制与既往排班模块
...
- 2026-08-03 调试期轮次 1–22 全部完成并部署（用户反馈、根因、修复、验证详见 debug 日志；最新基线 353/353）。
