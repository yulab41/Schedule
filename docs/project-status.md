...
## 当前批次：补录默认视图与预览周历复用（已实现，待上传与真机复核）
...
- 共用徽标现按首页规则显示 N/NP/A/D/电脑；补录按群组默认周/月，列表映射月；手排生成、草稿、发布预览的周历复用已有日历分页/单元格，保留灰色已有排班、节假日、日期详情、滑动和定位。改动限 `apps/miniprogram/**` 与文档，不触发生产部署。
- 定向 42/42，根 `pnpm verify` 1306 通过/451 跳过，Mini production verify 通过（总包 4,801,743 B、主包 1,826,112 B），开发者工具三处 WXML 编译通过。模拟器补录页未获得排班数据，仅停在加载状态，故数据填充视觉、真实滑动及小米 14 均未验证。现有主包/矩阵警告仍在。检查点提交信息：`fix(miniprogram): reuse calendar week view in scheduling previews`；提交与推送结果在本轮收尾确认。
- 本轮无体验版上传授权；保持 `UPLOAD_REQUIRED`，不分配新版号、不放行。唯一下一任务：取得上传授权后用干净累计候选上传并只追加放行，再用同构建小米 14 核对护士群补录周历与三类预览。
