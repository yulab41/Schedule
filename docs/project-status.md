...
- Debug rounds: 1–27 已完成；最新验证基线 `pnpm verify` 354/354（60 个测试文件，隔离 MySQL）
- Next actions: 本地验收（Web `http://localhost:5173`，本地账号 `local-admin`/`local-member`；当前重点：添加成员后列表自动刷新、本地节假日标注恢复、换班事件叙述恢复完整姓名）+ 部署后线上验收；下一轮规划：月历式草稿/发布/应用预览、发布记录撤销/恢复/删除、发布前工作流清理、未来日期发布限制与既往排班模块
...
- 2026-08-03 调试期轮次 1–27 全部完成并部署（用户反馈、根因、修复、验证详见 debug 日志；最新基线 354/354）。
