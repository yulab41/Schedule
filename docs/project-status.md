...
## 当前批次：个人联系方式、单环境二维码与访客审计（阶段一已实现，待上线链路）

- 独占 `general-4`、`REUSE_ONLY`、安装 0，基线 `b65c76f3`。用户撤回顶部导航改版，五个主页面原导航/标题保持不变；二维码四字段按最新反馈收至原生 40px、Storybook 20px。
- 已实现账号级手机号/短号弹窗与跨群同步、`0063` 确定性回填、单环境成员/访客二维码、严格 POST 访客读取、可降级 OpenID 换码、白名单设备上下文及可展开审计详情。Mini 已删除邀请生成/接受/分享/撤销与群组码界面。
- 阶段一为保护旧正式版，服务端旧邀请表/服务、旧双码接口和兼容响应暂留；新正式版稳定且旧调用归零后，另获生产破坏性操作授权再物理删除。
- 验证：Mini 1229/16 跳过、根 Vitest 1303/452 跳过、真实 MySQL 116/116；typecheck、lint、format、build、Storybook、契约生成与浏览器 smoke 通过。视觉比较无成对夹具，不记为通过。
- 包体总量 4,632,072 → 4,578,755 B（−53,317），主包 +25,353，organization −105,480。未生产部署/上传/提审/发布；新 Mini 依赖未部署 API，DevTools 授权 pending，无当前小米 14 证据。唯一下一任务是获授权后备份并部署阶段一 API/schema，再冻结上传体验版。
- 详见 [审计报告](audit/profile-qr-visitor-audit-20260921.md)；checkpoint 以 `feat(mini): add account contacts and environment-aware QR audit` 识别，提交/推送结果在完成后报告。
