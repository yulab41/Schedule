...
## 当前批次：头颈 DOCX 一值轮转矩阵已修正，待交付授权

- 用户实际导出的 9 月 DOCX 暴露旧实现按人员分别聚合且只认当前成员 ID，导致历史排班日期被漏掉、列数错误，也不能表达月初/月末空白和中途轮空。引入点为 `c60aa523`。
- 已改为固定一值行顺序下按日期先上后下、先左后右推进：9 月首日为林恩宇时首列前五行留空；末日为冯钦时末列林恩宇行留空；中途跨过的轮转槽显示 `-`。历史成员 ID 按稳定用户 ID 归一，未知人员拒绝生成。二值、三值和既有 Word 视觉参数不变。
- 回归定向 8 项通过；完整 `pnpm verify` 通过（Mini 1169/16 跳过、根 1266/442 跳过，format/lint/build/typecheck/icon parity 全部通过）。Microsoft Word 实际渲染并逐页检查正常 30 天和中途轮空两个单页夹具，列优先、首尾空白、横线及底部布局符合规则。
- 当前只完成代码、测试和本地 Word 视觉确认，尚未生产部署、上传或放行新体验版，也未取得小米 14 验收。唯一下一任务：形成并推送代码检查点后，等待用户另行明确授权生产部署及新体验版上传/追加放行。
