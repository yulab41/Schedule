...
- 本轮检查点拟用 `fix(miniprogram): share weekly calendar across schedule views` 标识；代码仅 Mini 与文档，未上传新体验版、未放行、未做生产操作。唯一下一任务：从本检查点上传新体验版并在小米 14 对照首页验证补录及三类预览；本批停止于干净提交与推送，真机证据到达前保持待用户复核。
