...
- 最终状态 checkpoint：待提交 `docs(status): record native evidence deployment`，只同步上述部署事实与更正后的 ECS 脚本入口，不含应用行为变更。
- 停止条件：提交、推送并部署本最终状态 checkpoint，使 Git `HEAD`、`origin/main` 和服务器 `current-release` 一致后停止并询问用户配置；不进入 P2，不以 Storybook/simulate 代替原生验收。
