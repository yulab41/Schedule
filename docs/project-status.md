...
- 当前状态：42 格月历已由用户人工确认通过；基础控件按钮轨道与页面滚动修复已实现待复测，7×7/20×30 手排矩阵尚因前述阻塞未完成实体 Android 验收。P1 不能提前标为完成。
- 下一批次：用户按 `apps/miniprogram/docs/runbooks/manual-native-testing.md` 先复测基础控件 A，确认可滚到底且 390px 两列/320px 单列按钮铺满对应轨道后继续 C/D；明确反馈“通过”后才进入 P2 共享核心。若失败，只修复用户指出的页面/状态并重新交付同一清单。
