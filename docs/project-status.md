...
## 2026-08-25 P8-A2-4 平台身份写入安全硬化（已实现待上传/部署）
...
- 验证/checkpoint：API 72/478（真实 MySQL）、Contracts 18/62、client-core 14/45、Web 105/606、Mini 45/254；全端 typecheck、API/Web build、generated freshness、Mini production verify、任务 ESLint/Prettier/diff 通过。Mini 2/2 Worklet、3,460,689 bytes、manifest `4cceb7469ce2f33e8d95758a2c229092a886353da1c8da64abeeae54a4820b93`，仅既有 600 格 best-effort warning。运行/浏览器验证：`pnpm smoke:browser` 等价入口 `node scripts/smoke-browser.mjs` 在当前源码 127.0.0.1:4173 完整通过管理员、成员、访客/vkey 与访问记录且无浏览器错误，临时服务已按 PID 关闭。checkpoint 识别消息 `feat(auth): harden p8 platform identity mutations`；提交推送后以 `0.1.0-p8.20260825.99-identity` 上传体验轨道但不加 allowlist，再备份/部署/full verifier。下一活动批次为 P8-B Web 黄金：固化组织管理全状态 390/320 和大字号证据，不提前写 Mini 原生页面或开启 capability。
