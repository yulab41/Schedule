...
## 2026-08-25 P10 通讯录原生首个切片（已实现，待体验/实体复核）

- 用户授权：用户已明确允许进行 P10 通讯录视觉确认；本批在 P8 RC 人工通过前继续作为显式范围偏差记录。P8 组织管理写能力仍为 `organization=false`，不提审、不正式发布。
- 实现：新增原生 `pages/directory/index`，从工作台“更多”对正式成员/管理员开放；支持科室/人员模式、搜索、七级 facets 筛选、加载/空/错误/重试、游标加载更多、号码长短号展示与拨号边界。搜索、筛选、cursor、结果和敏感号码均只驻留当前页面内存。
