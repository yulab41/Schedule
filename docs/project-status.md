...
- 当前状态：用户确认 C 通过；D 横向丝滑通过、纵向无法滚动。方向专用识别器修复及完整验证已完成，checkpoint 识别消息为 `fix(miniprogram): use exclusive directional matrix gestures`；等待提交、ECS 部署和微信体验上传后只复测 D。P1 仍不能进入 P2，且不使用 MiniTest 云测。
- 下一批次：用户只需人工复测 D：20×30 纵向人员/班次能否同步滚到最后一行，以及斜向手势是否只选择一个轴。C 和 D 横向已通过，不重复验收；D 通过后再继续 P1，失败需提供具体设备、手势方向和是否有任何纵向位移。
