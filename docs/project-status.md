...
- 应用8e68a480ba7608025a9af262e7d6fe5369071161已合入main、推送并部署生产。覆盖解散按钮、周期弹窗、月历预览/补录、草稿接口、邀请卡片/接收流程、通知四字段及投递保护。最终pnpm verify通过（Mini996/15跳过，root1215/402跳过，依赖保护81）；弹窗回归定向21及最终Mini verify通过，浏览器/同API源码MySQL15项证据复用。尚未上传feedback8体验版。
- 用户取消全删11账户，改为仅筛选姓名重复或从未出现在历史排班中的待设置账户；具体3账户及关联影响再次确认后，2026-09-10已完成永久删除。清理备份e03aba70-d331-48f0-a865-e383b625dd15及hash核验通过，事务演练、删除、独立查询与生产verifier通过；剩32账户（24已设置、8历史待设置），全部4011条排班及成员归属保留，详见docs/audit/feedback8-account-cleanup.md。随后应用交付另见feedback8-server-release.md。
- 预览重叠采用原姓名删除线＋拟姓名暗红字；邀请限好友/群聊卡片。唯一班种自动选择未确认前保留手动选择。旧verify曾中断，现已重跑通过；本轮Docker自身Inference socket启动故障，未额外重跑真实MySQL，不能把root跳过的集成测试计为通过。账户清理以实际生产回滚演练及提交后校验为证。
- 账户操作文档检查点57e6330a：`docs(ops): record confirmed feedback8 account cleanup`；随后应用检查点8e68a480完成提交及生产部署。文档记录不单独触发重部署、重复备份或生产发布标识同步。
- 应用检查点：`fix: repair scheduling previews, invitations and WeChat reminders`；主包1764091/总包5324841字节，Worklet2/2、确定性通过，保留已有内部主包和矩阵节点预警。独立只读复核发现的长确认弹窗裁切已限定为月历预览，原确认滚动保留；补测和桌面几何通过。
- 服务器交付按用户根AGENTS的API检查点自动部署规则执行；部署前新备份d5dac482-6607-434e-94cc-8e1711738942及文件hash核验通过，回滚候选即时读取并锁定36fae3d1。最终生产verifier、既有版本允许列表验证、部署代码四字段模拟发送及32/24/8/4011数据核验通过。没有真实发送通知或修改允许列表。
- 唯一下一任务/停止条件：UPLOAD_REQUIRED。用户确认当前干净提交的体验版上传及新增版本放行后，按既有流程分配版本、上传并追加允许列表（保留旧版），再进行小米14视觉、好友/群聊接收及本人收信复核。账户清理及服务器部署完成，不重复执行；不沿用旧版上传授权。当前文档检查点`docs(release): record feedback8 server delivery`不重复部署或同步生产发布标识，生产应用保持8e68a480。
