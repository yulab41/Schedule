...
## 当前批次：3.17.2 分页提交期保住可见面板 + 单元格内联 px（修箭头/定位闪动与手势横向抖动），体验版163已放行

- 用户回传 `.162`：高度已**基本同步**（仍稍落后于滑动）；但新增两个视觉问题——①**左右按钮与定位跳转时单元格内容闪动**（像内容快速变化），手势滑动不闪；②**手势左滑（未来月份）单元格横向抖动**，右滑（既往月份）不抖。
- ① 定因：`.162` 把"换干净环 + 轨道归零"放在同一个更新里，但换环时**中间面板的内容确实换成了新月份**（提交时它正变成可见面板）→ 单元格按 `businessDate` 键整体重建 → 可见帧闪动；手势路径因为换环发生在屏外的侧面板上，所以不闪。
- ② 定因（推断，合成触摸无法驱动 Skyline 滑动，只能按证据链）：兼容分支里单元格仍是百分比宽度 `14.285714%`，而 3.17.2 在原生滚动容器里解析百分比链不可靠；左滑常伴随新月份数据回填触发一次重排 → 百分比被重新解析 → 单元格横向抖动；右滑多为已缓存月份、无二次回填，所以不抖。
- 修法（3.17.3 swiper 分支逐字未改）：①提交时改用**保住可见面板**的映射（`[prev,cur,cur]` / `[cur,cur,next]`），轨道**等这一帧渲染完再归零**（下一 tick），干净环仍走既有清理兜底；这样可见面板的单元格键不变、不会重建。②高度**提前一帧**下发（真机上重排本身要一帧），让两者同时落位。③兼容分支的**单元格也走内联 px 宽度**（`width:52px;`，与 `.156` 的面板 px 同源），并同样镜像到请假页日期网格。
- 验证（3.17.2 模拟器）：112 个单元格宽度直方图全部为 52px；一次切月轨道 -351→-534→-641→-681→-703→-713→-715（≈250ms）后提交、归零，结束态 `track` 空、`cleanup=false`、`viewportHeight==gridHeight`；连按 4 次逐步推进不卡死；连按后手势仍正好一月。新增断言在旧源码上先失败（RED）。
...
- 交付：检查点 `76712d29` 已推送；候选在独占 `general-5` 冻结（`check-worktree-safety` 前后 `RESULT=PASS`）；`0.1.0-p10.20260918.163` 上传（Manifest `a25a9f58…36d8`），可信 `ensure` 追加并保留 `.162`；`ecs-verify.sh` `[verify] complete`；公网 `.163=200`/`.162=200`/未知 `=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 唯一下一任务：小米14 打开 `.163` 复核 ①箭头/定位切换是否不再闪动；②手势左滑横向抖动是否消失（右滑不应变化）；③高度与滑动是否同时落位；并确认 3.17.3 无变化。详情见 `docs/audit/runtime-ui-compatibility-period-pager-20260917.md`。
