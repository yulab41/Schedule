...
- 矩阵实现：`pages/manual-matrix-poc/index?mode=daily|maximum`、自绘 `ManualScheduleCell` 和确定性 7×7/20×30 fixture 使用四层坐标模型。日期与班次共用一个原生横向 `scroll-view`；人员与班次通过同一个 UI 线程纵向 SharedValue 同步；左上角固定。矩阵高度按手机剩余可视空间动态计算，7×7 完整显示，20×30 最少显示 3 行；单次手势达到 10px 且方向比 1.25 后锁轴，横向原生惯性、纵向有界 `decay`，滚动期不调用 `setData`。
...
- 当前状态：`e1a597d` 已被本轮四层坐标方案取代；代码与回归已完成，checkpoint 识别消息为 `fix(miniprogram): add adaptive axis-locked matrix viewport`。等待提交、ECS 部署和新的微信体验上传后，由用户人工复测；P1 仍不能进入 P2，且不使用 MiniTest 云测。
...
- 下一批次：用户人工复测 7×7 横向和顶部蓝条；20×30 的动态视口、横向日期/班次、纵向人员/班次、斜向锁轴与格子点击/撤销。通过后再继续 P1 C/D，失败需提供具体设备、方向和错位现象。
