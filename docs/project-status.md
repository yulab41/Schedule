...
- Web UI 2.0 Task UI2-03/04/05/06/07 已分别以 `11bb812`、`5b00fa7`、`7c80488`、`1c84fd6`、`2d6f0a2` 推送；UI2-08 换班与加扣班移动工作流已完成实现和验证，等待独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. UI2-09：只改成员、成员认领和事件记录的手机卡片列表与明确操作入口；完整字段和桌面高密度表格保留，危险操作继续显式确认，不增加滑动删除；
2. UI2-09 完成后停止，不提前改手动排班矩阵、排班补录、统计表或全局收尾；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：先为已完成并验证的 UI2-08 创建、推送独立 checkpoint；下一实现对话只完成 UI2-09，浏览器与全仓库验证通过后停止。
