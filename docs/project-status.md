...
- 当前状态与下一批次：已实现并完成本地/运行验证；下一步只创建并推送 `fix(miniprogram): align p4 calendar markers and feedback` checkpoint，上传同一精确 production-profile 体验版，备份并部署同一 Git release，记录最终状态后暂停，等待用户实体微信复核上述 3 项；不补独立 staging、不进入 P5。
