...
- 测试先行：新增回归用例先在旧实现上失败（首页结构/高度/事件、补录灰显、预览圆角与高度），修复后 Mini 全量 `1280 passed / 18 skipped`，根 `1306 passed / 451 skipped`，Mini production verify 通过（4,817,496 B），格式与开发者工具 WXML/WXSS 编译通过。模拟器首页仍停在排班读取，真实排班视觉、连续手势及小米 14 未验证。
- 当前代码尚未上传或部署；上一体验版 `.197@ad6c09fe` 不包含本修复。停止条件为新的干净检查点、上传身份核对和用户另行授权的体验版放行；不提交审核或正式发布。
