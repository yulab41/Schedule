...
- Web UI 2.0 Task UI2-03/04/05/06 已分别以 `11bb812`、`5b00fa7`、`7c80488`、`1c84fd6` 推送；UI2-07 请假与审批移动工作流已完成实现和验证，等待独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. 当前批次剩余任务为 UI2-08：只改换班和加扣班列表/表单的移动卡片与响应式 Sheet；危险操作继续显式按钮/菜单与确认，不增加滑动删除；
2. UI2-08 完成后本批停止，不提前改成员认领、手动排班矩阵、补录或统计表；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-07 已完成实现、浏览器和全仓库验证；创建并推送其独立 checkpoint 后只进入 UI2-08，完成换班与加扣班后停止。
