...
- 代码检查点 `e8286722` 已推送；本轮获用户明确授权上传并放行。独占 warm `general-6`、REUSE_ONLY、未安装依赖，拟以 `chore(release): refresh weekly calendar trial lineage proof` 记录已审查的首页文件证明更新。`origin/main=f8010455` 与最新累计体验版 `.196@aef86299` 均为候选祖先；版本须由独占分配器动态产生，上传后只追加放行。当前尚未上传、放行或操作生产；本批停止条件为新体验版身份及放行独立核验，真机效果仍待小米 14 同构建证据。
