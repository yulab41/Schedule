...
- Web UI 2.0 Task UI2-03/04/05/06/07/08/09 已分别以 `11bb812`、`5b00fa7`、`7c80488`、`1c84fd6`、`2d6f0a2`、`2bb9fce`、`23c1d9f` 推送；UI2-10 已完成，checkpoint 识别消息为 `feat(web): improve dense schedule interactions`。下一批次为 UI2-11/12 统计表及通知/导出响应式界面。业务逻辑、权限、API 和数据结构未修改。
...
1. UI2-11：只改统计页的数据密集表格；保留横向滚动并固定表头/关键首列，增加明确滚动提示和手机可读汇总，不修改统计口径；
2. UI2-12：只改通知与导出响应式界面；手机使用响应式 Sheet 和不小于 44px 的显式操作，保留现有权限、API、筛选与导出语义；
3. UI2-11/12 完成后停止，不提前改群组管理、排班配置、访客排班、离线/错误状态或全局收尾；
4. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-10 checkpoint 以 `feat(web): improve dense schedule interactions` 识别并推送后，本轮停止；下一实现对话只完成 UI2-11/12，浏览器与全仓库验证通过后停止。
