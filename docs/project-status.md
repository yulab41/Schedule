...
## 2026-08-18 微信小程序迁移 P0/P1（当前批次，原生证据工具）

- 批次范围：在用户确认四项 Web 黄金基线，且基础控件、42 格月历与 7×7/20×30 Skyline 手排矩阵 PoC 完成后，只补齐 P1 MiniTest 安全 runner、四页证据清单和自有视觉比较器；不接入业务 API，不实现 P2，不启动本地微信开发者工具，也不执行微信正式发布动作。
...
- 原生证据工具：`testing/p1-minitest-plan.json` 锁定基础控件、月历、7×7、20×30 四条原生路由、Android/iOS、唯一截图名与 98%/2%/2px 门槛。`scripts/minitest.mjs` 只封装官方非破坏性的 `POST /thirdapi/plan` 与 `GET /thirdapi/plan`，按官方 `rtn=0`/`data.plan_id` 解析，robot/dev account 强制同为 1–10，测试计划 ID 强制正整数，平台与传输错误均脱敏；不实现删除/覆盖基线接口。`scripts/visual-compare.mjs` 分别计算稳定区 RGB 相似度、通道差值大于 24 的显著像素率与关键几何偏差，输出 ignored `report.json`/`diff.png`，图片、遮罩、几何不匹配或非批准遮罩均失败关闭且不隐式缩放。
...
- 证据工具验证：测试先因比较器/MiniTest helper 和 `pngjs` 不存在出现 2 个失败 suite；实现后相同图、批准/全屏遮罩、超过 2% 像素、超过 2px/非法几何、尺寸/几何失败关闭、固定四页截图名与性能门槛、官方 payload、robot 一致、正整数计划 ID、`rtn` 响应和错误脱敏共 12 项通过，Mini 全套 41/41。staging/production verify 继续分别为 101,250/101,243 bytes、11 个 Worklet，manifest 分别为 `31185775e55f0d6db94182627b8fb85a84b7690a9ee4496014803ecc3384239b` / `48946ae37d7a3cece71b44a78fe66f22ec9fa1b5dda306564f625eb373fe8230`；无凭据 `miniprogram-ci` 与 MiniTest dry-run 通过。根 lint/build/typecheck、任务文件 Prettier/ESLint、排除用户自有 `runtime/**`/`src/**` 后主工作区 118 文件/699 项（31 文件/261 项按环境跳过）、`pnpm smoke:check-core` 与 `git diff --check` 通过，未触及核心 Web 链路；根 `format:check` 仍只因用户自有 `project.config.json` 失败。
...
- 外部门禁：`WECHAT_CI_PRIVATE_KEY_PATH`、`WECHAT_CI_ROBOT`、`MINITEST_USER_TOKEN`、`MINITEST_GROUP_EN_ID`、`MINITEST_TEST_PLAN_ID` 与 `MINITEST_DEV_ACCOUNT_NO` 均未注入。仓库 runner 已就绪，但仍必须先由同一 1–10 号 robot 生成开发预览，再提交已在 MiniTest 配置的 P1 自定义计划。因此本轮只执行无凭据 dry-run，未生成二维码、未上传开发/体验版、未提交云测、未改变微信平台状态。Storybook/simulate 不能证明原生视觉，四项 PoC 均保持“已实现待 MiniTest/实体机复核”。全程未启动、唤醒或控制微信开发者工具。
...
- 本轮 checkpoint：待提交 `test(miniprogram): add native visual evidence tooling`，只包含 P1 runner、比较器、清单、文档、workspace 委托命令与 `pngjs` 锁文件，不包含用户自有 `project.config.json`、`runtime/` 或 `src/`。
- 当前状态：42 格月历和 7×7/20×30 手排矩阵 PoC、MiniTest runner 与自有视觉门禁均已实现；MiniTest 原生视觉、双轴滚动手感和性能因仓库外凭据/平台入口缺失尚未执行，不能标为 P1 原生完成。
- 下一批次：等待用户在仓库外配置微信代码上传私钥，并在 MiniTest 创建/提供 P1 自定义测试计划及用户 token、产品英文 ID；随后只为基础控件、42 格月历和两种矩阵生成同 robot 开发预览并执行 Android/iOS，下载截图/几何/性能证据、锁定设备与 mask，再等待用户实体 Android 确认。
- 停止条件：最终状态 checkpoint 显式暂存、推送并作为同仓 release 部署，使 Git `HEAD`、`origin/main` 和服务器 `current-release` 一致后停止询问用户配置；不进入 P2，不以 Storybook/simulate 代替原生验收。
