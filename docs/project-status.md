...
## 当前批次：共用周历 .197 已上传放行，待小米 14 复核

- 基线 `f8010455`，独占 warm `general-4`、REUSE_ONLY、未安装依赖。引入点：首页周格 `ad4cfb2c`，独立预览周格 `aef86299`；本轮将首页周格抽为 `calendar-week-panel`，首页、补录和手排三类预览共用分组、排序、换行和高度计算，月历保留原组件。补录与预览继续复用 `calendar-period-pager` 的环形翻页；跨周定位预取所需月份后过渡，补录读取失败显错并禁用。
- 验证：测试先行的护士排序/分组/高度用例由红转绿；`pnpm verify` 通过（Mini 1274/17 跳过、根 1306/451 跳过），production verify/源码/包体检查通过，总包 4,815,725 B。开发者工具状态正常，共用周历、月历、首页 WXML 及周历 WXSS 局部编译通过；模拟器补录停在“正在加载既往排班”，Console error 过滤为空。真实排班视觉、连续手势、320px 和小米 14 尚无同构建证据。
- `e8286722` 代码检查点与 `ad6c09fe` 发布证明检查点均已推送。独占 warm `general-6`、REUSE_ONLY，未安装新依赖；生产 Mini verify 通过（总包 4,815,771 B），CI dry-run、源码/包体/确定性、lineage、16/16 定向、格式与静态门禁均通过。production `.197@ad6c09fe` 上传成功，Manifest/receipt/远端 tag 一致。
- 用户另行明确授权生产备份和发布标识同步。实时基线 `17c1c90f`；加密备份 `c1e4ee1c-e729-4e98-a4de-1321930e9413` 后，可信复用因归档哈希不同安全拒绝，完整部署 `ad6c09fe` 成功。只追加 `.197` 放行，allowlist 与完整 ECS verifier、公网 `.197/.196=200` 和未知版=426 通过。未提审或正式发布。交付记录检查点为 `docs(release): record weekly calendar trial 197 delivery`；按文档例外不重复上传或部署。唯一下一任务：取得同构建小米 14 补录和三类预览的顺序、分组、换行、切周和定位动画证据；此前保持待用户复核。
