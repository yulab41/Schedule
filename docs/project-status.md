...
- Web UI 2.0 Task UI2-03 已完成并以 `11bb812` 推送；UI2-04 响应式应用框架已完成实现和验证，当前为“待用户复核”，等待本轮第二个独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. 下一轮只进入生产月历 UI2-05～UI2-06：先以失败测试覆盖默认/变更选中日期、多人班次行、事件标识和 56px 横滑意图，再实现选中日期详情轨道；
2. 本批停止在生产月历选中日期、详情轨道和横滑意图，不提前改成员、认领、请假、换班、加扣班等工作流卡片；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-03 与 UI2-04 已完成实现和验证；本轮创建并推送 UI2-04 checkpoint 后停止，未改生产月历选中日期、横滑、详情轨道或工作流页面。
