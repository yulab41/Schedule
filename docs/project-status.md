...
## 当前批次：Feedback26 手动排班刷新与结束日期已实现

- 手动排班四阶段点击现均重新读取对应数据；草稿发布成功后进入发布记录页，避免继续显示旧草稿。
- 编辑表单改为模板+岗位、开始+结束、周期+人员三行；结束日期用于真实预览/草稿范围并校验含首尾1–30天。详情见`docs/audit/feedback26-manual-schedule-refresh.md`。
- 定向回归先失败后通过，Mini TypeScript通过；独占general-5，REUSE_ONLY且未安装依赖。微信开发者工具未调用，小米14原生待后续同SHA体验版复核。
