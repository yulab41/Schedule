...
## 当前批次：头颈 DOCX 轮转矩阵待交付

- 修复按人员聚合造成的漏日/错列：一值先上后下、先左后右，月首/月尾留空，中途缺位为`-`；历史ID归一。二值、三值及Word视觉不变，详见`docs/audit/head-neck-docx-export.md`。
- 定向8项及Word视觉通过；已获部署、上传和追加放行授权，候选正整合`.130`，尚未执行外部变更。
