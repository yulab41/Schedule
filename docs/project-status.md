...
- 群组页面展示修复已完成，待新的 Web release 部署：工作台和群组管理页会明确显示当前群组、角色和四位群组码，重新生成群组码后会刷新并显示新码。
...
- 群组页面回归修复已完成：增加当前群组/群组码展示、重新生成群组码后的新码提示和浏览器回归断言；本轮代码 checkpoint commit message：`fix(web): show group and regenerated group codes`。
...
1. 部署新的 Web release 后，用户退出并重新登录 `D0796`，人工复核群组是否显示为“林恩宇”、群主权限、排班管理、人员管理和节假日管理页面；不要把密码发给我；
