...
- 服务器已完成数据库备份，首次发布因新增认证表外键排序规则不兼容自动回滚；现网旧 release 已恢复健康，当前仍需用修正后的新 release 重试迁移、重建和核验。现有服务器会话密钥已存在，网站 AppID/AppSecret 不再是阻塞项。
...
- 修正后的迁移尚未重新部署；首次发布失败后自动回滚，现网首页和 API 健康检查已恢复。
...
1. 提交并推送迁移排序规则修正，从当前 HEAD 重新生成生产包；
2. 使用已有数据库备份保护，在服务器上重试发布并执行 `ecs-verify.sh`；
