...
- 发布/下一步：生产备份 archive `59253ae6-ad98-40d1-866e-11d56f050882`（54 表/169,605 行/79,208,604 bytes）后部署 release `c0ea31e977abadd4347aa55d8fd964a359271795`；预热一次 502 后恢复。随后 docs-only checkpoint `01bf73ef` 在生产备份 archive `ece6efb0-0c50-4b18-813e-6085a7066154`（54 表/169,622 行/79,214,568 bytes/SHA `3905ab679feb1d39cfc0a0eb03118a1c253cd4f8ade4b1917509f34fa3ad6aaa`）后部署 release `01bf73ef8805fd5ed7da6a8b5567433d5df6ac26`；最终 `ECS_PUBLIC_IP` full `ecs-verify.sh`、health 200、P8 capability allowlist/organization=false、artifact/control-plane/migration/unknown-host 检查通过，远端临时目录已清理。`.3` 白名单追加后的备份为 `5ffd2615-21a5-4e66-88d6-86b96f41ea68`，三个 P8 版本 capability 均已复核 HTTP 200。最终状态 checkpoint 识别消息为 `docs(status): record p8 platform account final deployment`；下一批 P8 RC 综合验收，停止条件是先取得本批页面用户复核。
