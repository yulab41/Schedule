...
## 当前批次：补录默认视图与预览周历复用（体验版已上传，放行待前置生产部署）
...
- 经用户本轮授权，从干净 `aef86299` 上传 production 体验版 `0.1.0-p10.20260925.196`；远端不可变 tag、receipt、构建身份及 Manifest `16a51495e92e8f2d60988432632602d0490bb8a5335bb54e559576e83e4e0beb` 一致。未执行生产放行：手册要求先部署同一 Git checkpoint，本轮授权未明确涵盖生产备份/部署，已单独请求授权。唯一下一任务：如获授权，按 L4 备份、部署、验证后仅追加 `.196` 白名单；随后用同构建小米 14 复核。
