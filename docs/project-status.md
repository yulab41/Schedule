...
- Web UI 2.0 Task UI2-03/04/05/06/07/08/09/10 已分别以 `11bb812`、`5b00fa7`、`7c80488`、`1c84fd6`、`2d6f0a2`、`2bb9fce`、`23c1d9f`、`1203dc7` 推送；UI2-11/12 统计表及通知/导出响应式界面已完成，checkpoint 识别消息为 `feat(web): polish statistics notifications and exports`。业务逻辑、权限、API 和数据结构未修改。
...
- Web UI 2.0 Task UI2-12：通知中心和导出迁入共用 `ResponsiveSheet`，手机为全宽贴底页；通知卡按业务类型使用可访问语义徽标，通知设置改为响应式卡片，导出增加实时内容/周期摘要、安全说明及显式取消/导出操作。全部可见操作不小于 44px，原权限、轮询、筛选、API、下载与审计语义不变。本 checkpoint 识别消息为 `feat(web): polish statistics notifications and exports`。
...
1. UI2-13：只改群组管理与排班配置响应式界面，沿用同一令牌、卡片/高密度桌面策略和 44px 点触目标，不修改权限、API、契约或配置语义；
2. UI2-14：只改访客排班、空状态、离线提示和错误状态，保留访客访问权限、vkey 链路、缓存与错误处理语义；
3. UI2-13/14 完成后停止，不提前开始最终全局焦点、减少动态和跨视口收尾审计；
4. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-11/12 checkpoint 以 `feat(web): polish statistics notifications and exports` 创建并推送后本轮停止；下一实现对话只完成 UI2-13/14，浏览器与全仓库验证通过后停止。
