...
- 发布与停止条件：代码 checkpoint `9045dc02` 已推送；production-profile `.72` 已上传但因补充截图纠正滚动状态判断而不部署，修正 checkpoint 识别消息 `fix(miniprogram): preserve list scroll state`，计划上传 `.73`，不提审/正式发布；备份、部署、验证和最终状态 checkpoint 后转为待实体微信只复核上述 5 项，不进入 P5。
