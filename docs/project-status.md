...
## 2026-08-18 微信小程序迁移 P0/P1（当前批次，Skyline 手排矩阵 PoC）

- 批次范围：在用户确认四项 Web 黄金基线及 42 格月历后，只实现 7×7/20×30 Skyline 手排矩阵 PoC；不接入业务 API，不实现弹层、导航或微信正式发布动作。
...
- 外部门禁：环境变量 `WECHAT_CI_PRIVATE_KEY_PATH` 未注入，robot/version/description 元数据也不存在；仓库当前没有独立 MiniTest runner，必须先由 `miniprogram-ci` 生成开发预览版本才能进入云真机。因此本轮只能完成无凭证 dry-run，未生成二维码、未上传开发/体验版、未改变微信平台状态。Storybook/simulate 不能证明原生视觉，基础控件、月历和矩阵均保持“已实现待 MiniTest/实体机复核”。全程未启动、唤醒或控制微信开发者工具。
...
- 当前状态：42 格月历和 7×7/20×30 手排矩阵 PoC 均已实现、推送并完成 ECS 同仓 release 同步；MiniTest 原生视觉、双轴滚动手感和性能因仓库外凭据/平台入口缺失尚未执行，不能标为 P1 原生完成。
- 下一批次：等待用户在仓库外配置微信代码上传私钥并提供可用 MiniTest 项目/权限；随后只为基础控件、42 格月历和两种矩阵生成开发预览并执行 Android/iOS，产出带 commit、fixture hash、设备、基础库和遮罩版本的原生证据，再等待用户实体 Android 确认。
