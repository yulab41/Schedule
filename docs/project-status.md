...
- Web UI 2.0 Task UI2-03/04/05 已分别以 `11bb812`、`5b00fa7`、`7c80488` 推送；UI2-06 选中日期值班轨道已完成实现和验证，等待独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. 下一轮进入工作流卡片批次：UI2-07 只改请假列表/审批卡片及其响应式表单 Sheet，UI2-08 再改换班和加扣班列表/表单；危险操作继续显式菜单与确认，不增加滑动删除；
2. 本批在请假、换班、加扣班的移动卡片与响应式 Sheet 完成后停止，不提前改成员认领、手动排班矩阵、补录或统计表；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-05/06 的生产月历与详情轨道均已完成；本轮创建并推送 UI2-06 checkpoint 后停止，尚未改工作流页面。
