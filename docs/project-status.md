...
- c4a9b67f已推送部署，完整生产verifier通过、schema57；所有导入演练已回滚。发现计划人员尚无实际人员时的成员匹配遗漏，RED为3条而非2条；修复后MySQL12项复测通过，API build/lint/格式与smoke:check-core通过。
- 后续检查点：fix(schedules): match planned members during backfill。下一步备份部署该补充修复、完成同一139条导入与独立回读；照片已确认的1处NP/A时间重叠如实保留，其他冲突不放行。正式写入完成前不标交付。
