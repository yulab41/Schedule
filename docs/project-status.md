...
## 当前批次：头颈 DOCX 已上线，累计体验候选待最终交付

- 用户批准一值使用当前有效成员，二值按一值ID映射固定显示姓名，三值固定显示彭汉伟、郭海鹏；不新增、恢复或授权成员。生产只读核对后当前一值为许少伟、徐漫彬、黄耿杰、洪晨善、冯钦、林恩宇，徐敏inactive而未纳入。
- DOCX应用`ececc967`已完成备份、schema60迁移、API/Web部署与配置启用；备份`ffa6570f-c3f3-4c66-9395-ef7daa281191`（54表、108168328字节）成功，部署及配置后的完整ecs-verify通过。配置哈希已核对，生产UUID/姓名配置只存ignored runtime。
- 上传前`origin/main`前进到Feedback24 `2a8c7c67`，按累计体验版规则拒绝直接上传旧候选。已把DOCX变更重叠到最新主线并保留Feedback24导出选择器/访客改动；当前需完成累计候选验证、推送和必要的生产同步，再动态分配体验版并只追加放行。
- 累计候选完整`pnpm verify`的构建、格式、lint、typecheck和icon parity均通过；Mini全套在系统负载下出现导出控制器5秒测试超时及上传转换10秒hook超时，日志保留。无源码修改后将两文件隔离串行复测，29项和5项全部通过，判定为全套负载时序波动，不放宽超时或断言。
- Mini production verify通过：主包1727851、总包4580318字节、Worklet2/2、确定性通过，保留既有1.5M和矩阵告警；`smoke:check-core`通过。唯一下一动作：提交推送累计候选并同步生产，然后动态分配体验版。自动化、生产健康和Word视觉均不代替小米14原生验收。
