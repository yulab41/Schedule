...
## 2026-08-19 员工通讯录显示数据去噪（已实现待生产发布）
...
- 当前状态：已实现待生产发布；生产仍是上一版员工快照。
- 下一批次与停止条件：提交并推送本次去噪 checkpoint；生产先备份，再以 SSH stdin dry-run/原子发布新版员工 manifest，运行 `ecs-verify.sh` 与正式域名只读核验后停止，等待用户复核。
