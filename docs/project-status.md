...
- Web UI 2.0 Task UI2-03 已完成：共享语义令牌、TDesign 主题映射和直接图标依赖已落地；UI2-04 应用框架已实现，等待本轮第二个独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. 完成 UI2-04 第二个独立 checkpoint：登录页、顶部栏、群组上下文、桌面侧栏、手机底栏、“更多”底部页和账号区；运行 1280/390/320 浏览器、核心 smoke 和全仓库验证后提交推送；
2. 本批停止后，下一轮只进入生产月历 UI2-05～UI2-06：选中日期/详情轨道与横滑意图，不提前改工作流卡片；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-03 已完成；UI2-04 只完成应用框架与响应式导航，不改生产月历选中日期、横滑、详情轨道或工作流页面，完成验证与第二个 checkpoint 后停止。
