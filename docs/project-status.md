...
## 当前批次：撤回 `.163` 的延迟归零（修其"更卡/高度 1 秒后才落/内容假死"），并让排队手势就地提交，体验版164已放行

- 用户回传 `.163`：**比上一版更卡**；高度要等活动结束约 1 秒才落；快速切月出现**"有切换动画但内容不变"的假死**；定位闪动更明显；手势横向抖动仍在。另外补充关键手感线索：3.17.2 **必须等蓝色选中框出现才能触发下一次手势滑动**，之前滑会"落空"。
- `.163` 回归定因（已定位到代码）：①"提交后下一 tick 再归零轨道"依赖 `_compatSlideResetPending`，该标志被任何一次 `panels` 更新提前消费 → 归零丢失 → 轨道停在侧面板、`_compatSlideActive` 不复位 → 清理被挡 → `gridHeight` 观察器一直让位 → **内容假死 + 高度迟到**；②高度单独提前一帧下发把原本同帧的两次过渡拆开，反而更慢。
- 修法：**撤回 `.163` 的这两处**（回到 `.162` 行为：高度与位移同一次 `setData`、提交时原子换环 + 轨道归零），只保留**单元格内联 px 宽度**（`width:52px;`，与面板 px 同源，并镜像到请假页日期网格）。
- "落空"手感定因与修法：忙窗口内到来的手势原先在队列后立刻 `recenterCompatPanes` 把滚动**弹回原位**，看起来那一滑被吃掉；现在改成**保留手指走到的位置**，并在上一步结束后把这个队列增量**就地提交**（`_compatQueuedInPlace` + `finishMonthSwipeAt`，不再额外滑一格动画）。左右按钮的排队仍走滑动动画。
- 验证（3.17.2 模拟器）：一次切月轨道 -351→-641.8→-714.6 与高度 372→322.5→310.1 并行（≈250-300ms），提交后归零，结束态 `cleanup=false`、`viewportHeight==gridHeight`；连按 4 次 → 12 月→2027-04 逐月推进不卡死；程序化切月途中补一次手势 → 月份再前进 1 个且结束态干净（该滑不再落空）。新增/恢复的断言在旧源码上先失败（RED）。
- 仍未解决且需要证据：①箭头/定位闪动；②手势左滑横向抖动（内联 px 未消除）。合成触摸驱动不了 Skyline 滚动，**已请用户提供 5 秒手机录屏**再定因。
...
- 交付：检查点 `4bd30e5e` 已推送；候选在独占 `general-5` 冻结（`check-worktree-safety` 前后 `RESULT=PASS`）；`0.1.0-p10.20260918.164` 上传（Manifest `a38c0e58…0e61`），可信 `ensure` 追加并保留 `.163`；`ecs-verify.sh` `[verify] complete`；公网 `.164=200`/`.163=200`/未知 `=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 唯一下一任务：小米14 打开 `.164` 复核 ①是否恢复 `.162` 的顺滑、不再假死、高度不再迟到；②忙窗口内手势是否不再"落空"；③闪动/横向抖动是否仍在（在则请附 5 秒录屏）。详情见 `docs/audit/runtime-ui-compatibility-period-pager-20260917.md`。
