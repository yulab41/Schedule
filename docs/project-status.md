...
- 检查点消息：`fix(miniprogram): exclude unused pages from production package`。下一批次唯一任务：完成提交推送和不可变体验版上传、核对实际产物；停止条件为任何内容门禁/版本血缘/上传失败。随后等待同构建小米 14 验收。未获 L4 授权，不连接或部署生产服务器、不改版本允许清单；不提审、不正式发布。
