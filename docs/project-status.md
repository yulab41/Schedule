...
- 轮次 46 手动排班起始日期只统计已发布记录（归档/删除不影响自动检测），且手动修改后保存/应用以手动日期为准；手动排班/模板应用检测已批准请假并阻断；再次点击当前班种可清除单元格；审批弹窗列出具体冲突班次；班种配置改为每班种一行紧凑布局。检查点提交 `fix(schedules): published-only next start, leave-aware manual apply, toggle-off paint, one-row shift config` 识别。
