...
1. 只做最终全局收尾审计：键盘焦点、`prefers-reduced-motion`、安全区/底栏避让和 1280×900、390×844、320×844 跨视口一致性；
2. 对全部已改页面运行最终角色与核心链路回归，不增加新业务能力，不修改 API、契约、权限或数据库；
3. 完成最终全局审计后停止并请求用户做整体验收；
4. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-13/14 实现与验证完成，本轮创建并推送 `feat(web): polish group config and guest states` checkpoint 后停止；下一实现对话只完成最终全局收尾审计。
