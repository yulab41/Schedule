...
- Web UI 2.0 Task UI2-03/04 已分别以 `11bb812`、`5b00fa7` 推送；UI2-05 触屏月历已完成实现和验证，等待独立 checkpoint。业务逻辑、权限、API 和数据结构未修改。
...
1. 当前批次下一任务只完成 UI2-06：实现选中日期值班轨道、班种/岗位/完整姓名、电话、变更状态和事件入口，并把班次事件对话框统一为响应式 Sheet；
2. UI2-06 完成浏览器与全仓库验证并独立提交后停止，不提前改成员、认领、请假、换班、加扣班等工作流卡片；
3. 原生产待办继续保留：人工复核 `D0796` 的群主/管理员页面，并在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能。

停止条件：UI2-05 只包含生产月历选中日期、密集月格、筛选 Sheet 和横滑意图；创建并推送 checkpoint 后才进入 UI2-06，工作流页面保持不变。
