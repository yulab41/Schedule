...
## 当前批次：手动排班一次性编辑、366 天应用与模板删除（生产已部署放行，待真机复核）
...
- 周期矩阵仍限制 30 天、20 人、600 格；应用日期独立允许含首尾最多 366 天。初次/重选开始日期默认 `+29` 天；保存模板保留用户当前开始/结束日期。一次性草稿按 `businessMonth` 拆分，完整快照和日期进入幂等指纹，并写 `manual_schedule_inline_applied` 事件，不伪造模板记录。
- 模板删除复用既有软删除端点；二次确认展示摘要。删除当前模板回到“新建模板”，删除其他模板保留当前编辑快照，接口失败保持可重试。
- 回归引入点：`591ccff6` 引入保存前禁预览/保存后重置日期，`d1e53646` 引入等宽两列；本轮先补 RED 再改行为。
- 验证：契约/client 12/12；真实 MySQL 手排 34/34；Mini 1248/16，根池工具 81/81，根 Vitest 1302/444。typecheck、format、lint、build、生成器、production verify/package/source/determinism、CI dry-run均通过；总包 4,794,894 B、主包 1,820,944 B，矩阵节点从 1507 增至 1510（三个字段行宿主），矩阵本体未扩大。
- 运行/浏览器验证：`pnpm smoke:browser` 以当前源码和本地测试库通过登录、管理员、成员、访客 vkey 与访问记录；合成管理员标记已恢复，临时服务已停止。
...
- 生产交付：备份 `18760517-efb4-45fa-9012-5b478e658db5`（54 表、297990 行、124304832 B，登记/文件/SHA 一致）后部署 `b17a4e57`，schema 63；可信 `ensure` 只追加 `.190` 并保留旧版。allowlist/ECS verifier 与公网 `.190/.189=200`、未知版 `=426`、新路由未认证 `=401` 通过。未提审、未正式发布、未写业务数据。
- 停止条件：小米 14 使用 `.190@eacfd752` 验收布局、删除、未保存预览和长范围草稿；取得同构建证据前保持“待真机复核”。
- 记录见 [手动排班一次性编辑改造](audit/manual-schedule-inline-editor-20260923.md)。
...
- 文档收口检查点：docs(release): record feedback9 trial 102 delivery。当时下一任务/停止条件：用户在小米14重开.102/e40c4f9，复核群组偏好、手排矩阵/日期/月历、补录和CSV下载后发送/取消；失败时保留新版分类提示。自动化交付完成，真实下载原因与原生效果待用户复核；不重复上传、放行、部署或同步服务器release标识。
