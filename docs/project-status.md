...
- 用户随后反馈新症状：150ms 快速左滑时动画正常但 5 次只前进 1 个月，且换月数据感觉有延迟。已复现（交错事件流仿真下`.174`从 09 倒退到 08）：根因是提交门禁按模 3 单格差算位移并丢弃中间 finish。修复为“按 change 事件累计有符号步数、settle 一次性 adopt、重放/取消不移动”，触摸路径不再回写`swiperCurrent`；周历 pager 与日期选择器同步。同时新增“只下发真正变化的键”，消除每次换月约 100KB 的无效整包刷新（3 次/约313KB → 2 次/约145KB）；实测目标月份滑动前已在内存缓存，延迟来自重复下发而非网络或服务器。
- 已交付体验版`0.1.0-p10.20260919.175`（候选`17b63aa2`，Manifest`680384e6…0131`，233 code files，远端tag同一SHA）；`ensure .175`+`verify`+公网探针`.175/.174`=200、未知=426+完整`ecs-verify.sh`（release`44034fcc`未变）通过；候选含血缘证明 blob 刷新并已推送 `main`。完整 Mini 1209 项、`tsc`、production verify 通过。未部署生产应用、未备份生产库、未提审/正式发布；`.173`仍被放行，退役需另行批准。
- 运维提示：开发者工具缓存编译产物，重建`dist`后需`debug_clear_cache --action cleanCompileCache`再刷新，否则模拟器跑旧代码。
- 用户要求继续做四条下发性能优化（UI/交互/视觉不变、避免堆砌）。已实施：①`createViewPatch` 按可见视图裁剪面板；②新增`features/workbench/panel-patch.ts`，面板环展开为路径级 patch（只发旋转槽位/变化格子）；③月格不再下发`day`/`isWeekend`，由`calendar-cell`从日期推导（`ariaLabel`因会改动无障碍文本而保留）；④月视图预取窗口±2→±3（后台读取）。
- 效果（同环境同一换月）：**3 次 patch/约313KB → 1 次 patch/16KB（-95%）**，重复刷新整包跳过；5 次150ms快速滑动仿真仍为+5个月、状态一致。回归：Mini 1209 项、`tsc`、production verify（包体4567118、manifest`32248585…`）通过；颜色/尺寸/交互未改。
- 已交付体验版`0.1.0-p10.20260919.176`（候选`9ac4a301`，Manifest`064658e3…ca59`，234 code files，远端tag同一SHA）；`ensure .176`+`verify`+公网探针`.176/.175`=200、未知=426+完整`ecs-verify.sh`（release`44034fcc`未变）通过。未部署生产应用、未备份生产库、未提审/正式发布；`.173`仍被放行，退役需另行批准；general-5 已释放。
- 唯一下一任务/停止条件：用户在小米14打开体验版`0.1.0-p10.20260919.176`，确认150ms连续快速左滑5–6次前进5–6个月、换月内容出现更快、观感与上一版一致；未取得与`9ac4a301`一致的真机结论前，不得写“小米14验收通过”。详情见`docs/debug/debug-feedback-log.md`2026-09-19第六条。
