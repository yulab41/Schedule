...
- Debug rounds: 1–31 已完成；最新验证基线 `pnpm verify` 356/356（60 个测试文件，隔离 MySQL）
- Next actions: 本地验收（Web `http://localhost:5173`，本地账号 `local-admin`/`local-member`；当前重点：添加成员后列表自动刷新、本地节假日标注恢复、换班/加扣班事件新文案与变更链、统计页切换后不残留、加入群组输入真实姓名直接生效）+ 手动部署 CloudBase（Deploy Development 已改为手动触发）；下一轮规划：月历式草稿/发布/应用预览、发布记录撤销/恢复/删除、发布前工作流清理、未来日期发布限制与既往排班模块
...
- 2026-08-03 调试期轮次 1–31 全部完成（用户反馈、根因、修复、验证详见 debug 日志；最新基线 356/356；CloudBase 部署改为手动触发，round 29 未自动部署）。
