...
## 2026-08-18 微信小程序迁移 P0/P1（当前批次，Minium 原生套件）

- 批次范围：在用户确认四项 Web 黄金基线，且基础控件、42 格月历与 7×7/20×30 Skyline 手排矩阵 PoC 完成后，只补齐 P1 MiniTest 安全 runner、可上传 Minium Python 套件、四页证据清单和自有视觉比较器；不接入业务 API，不实现 P2，不启动本地微信开发者工具，也不执行微信正式发布动作。
...
- 工具链：确定性 staging/production `src→dist`、TypeScript、WXML/WXSS/JSON、运行边界、密钥、Worklet 指令、包体和双构建一致性门禁继续生效；`miniprogram-ci@2.1.31`、`miniprogram-simulate@1.6.2`、`jsdom@26.1.0` 和仅供测试工具使用的 `pngjs@7.0.0` 精确锁定。CI/MiniTest wrapper 的 dry-run 不读取凭据，真实私钥和 token 仍必须在仓库外。
...
- 原生证据工具：`testing/p1-minitest-plan.json` 锁定基础控件、月历、7×7、20×30 四条原生路由、Android/iOS、唯一截图名与 98%/2%/2px 门槛。`testing/minium/p1/test_p1_native.py` 继承官方 `minium.MiniTest`，4 个 `test_*` 方法覆盖 18 个唯一截图状态；稳定 WXML selector 只增加 `id`/测试属性，不改变样式、事件或业务数据。`pnpm miniprogram:minitest:case:build` 生成只含根目录 `test_p1_native.py` 的确定性 ZIP，源码与产物均拒绝凭据/本地路径。`scripts/minitest.mjs` 只封装官方非破坏性的 `POST /thirdapi/plan` 与 `GET /thirdapi/plan`；`scripts/visual-compare.mjs` 对三项视觉门槛独立失败关闭。
...
- MiniTest 清单回归定位：`git log -S`/`git blame` 确认 `dialog-open` 与 7×7 `vertical-scroll` 均由 `c8d50f5` 首次写入。已确认用户通过的基础控件 Storybook/原生页不存在对话框，7×7 的 82px 表头 + 7×44px 行恰好等于 390px 视口，不存在纵向滚动；清单现改为实际可见的通知开启、联系方式取消核对、周视图选择，以及仅在 20×30 套件验证纵向滚动。该修复不增加产品状态或改变 UI。
...
- 证据工具验证：Minium helper 测试先因实现文件不存在失败；实现后测试方法/路由/交互/18 个截图名、稳定 selector、无凭据、ZIP 根结构和双构建确定性 4/4 通过，Python 3.12 AST 解析通过，生成 ZIP 5,247 bytes、SHA-256 `6bef4f99d6122803e2c5cf1f584e3c82add08a0dc50d1c000d425d289047de15`。Mini 全套 45/45；staging/production verify 为 101,976/101,969 bytes、11 个 Worklet，manifest 分别为 `76153d8fa9e56dc78d7d27c08bc6a7a04f754f4b89b399af3b33316788438dfb` / `b8cd241d963372a175ab23a647bdc7fd47ae485b03ada4c1fe2089da894dfc8d`。根 lint/build/typecheck、排除用户自有 `runtime/**`/`src/**` 后主工作区 119 文件/703 项（31 文件/261 项按环境跳过）、任务文件 Prettier、`pnpm smoke:check-core`、无凭据 CI/MiniTest dry-run 通过；根 `format:check` 仍只因用户自有 `project.config.json` 失败。
...
- 外部门禁：`WECHAT_CI_PRIVATE_KEY_PATH`、`WECHAT_CI_ROBOT`、`MINITEST_USER_TOKEN`、`MINITEST_GROUP_EN_ID`、`MINITEST_TEST_PLAN_ID` 与 `MINITEST_DEV_ACCOUNT_NO` 均未注入。仓库已能生成上传 ZIP，但官方平台仍需已登录用户在“Minium 用例管理”上传、确认解析出 4 个方法并建立正整数计划 ID；随后必须由同一 1–10 号 robot 生成开发预览再提交计划。本轮仅构建本地 ignored ZIP 和执行 dry-run，未预览、上传开发/体验版、提交云测或改变微信平台状态；全程未启动、唤醒或控制微信开发者工具。
...
- 当前状态：42 格月历和 7×7/20×30 手排矩阵 PoC、Minium 上传套件、MiniTest runner 与自有视觉门禁均已实现；MiniTest 原生视觉、双轴滚动手感和性能因仓库外凭据/平台注册尚未执行，不能标为 P1 原生完成。
- 下一批次：等待用户上传 `apps/miniprogram/.artifacts/minitest/p1-minium-cases.zip` 并提供计划 ID、用户 token、产品英文 ID、同号 robot/dev account 及仓库外代码上传私钥；随后只生成开发预览并执行 Android/iOS，下载截图/几何/性能证据、锁定设备与 mask，再等待用户实体 Android 确认。
- 本 checkpoint 识别消息：`test(miniprogram): add p1 minium native suite`。
- 停止条件：本 checkpoint 显式提交、推送、生产备份/部署/核验并使 Git `HEAD`、`origin/main` 和服务器 `current-release` 一致后，向用户交付 ignored ZIP 的本地路径和一次性平台配置步骤；不进入 P2，不以 Storybook/simulate 代替原生验收。
