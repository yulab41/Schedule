...
- 发布与停止条件：代码 checkpoint 识别消息 `fix(miniprogram): eliminate calendar recenter regressions`；production-profile 体验版计划为 `0.1.0-p4.20260823.71`，只上传体验版，不提审/正式发布。完成代码推送、体验上传、生产备份/部署/验证和最终状态 checkpoint 后转为“待实体微信复核”，下一批仍只允许处理本轮复核反馈并暂停，不进入 P5。
