...
- 微信网站管理员认证文件已加入 Web 公共根目录；本轮 checkpoint 识别消息为 `ops(web): add WeChat site verification file`，待推送后创建生产备份、部署并从公网核对响应正文。
...
- 微信认证文件：修改前公网 URL 返回 1136 字节应用首页 HTML；新增文件后 Web 生产构建产物为 41 字节且正文精确匹配。`pnpm verify`、`pnpm smoke:check-core` 与 `git diff --check` 通过；71 个测试文件、490 项测试通过，29 个数据库集成文件、252 项因本机没有测试 MySQL 跳过。核心链路校验确认仅新增公共静态文件，无需运行浏览器 smoke。
...
当前活动批次：先完成微信认证文件 checkpoint 的推送、生产备份、部署和公网纯文本核验；随后由已获授权的微信号完成网站管理员认证并申请临时恢复，再复核微信内访问。若备案同步后仍被拦截，检查阿里云接入备案状态并提交微信申诉。下一项仓库任务是把 `粤ICP备2026116116号-1` 及工信部链接加入正式页面并独立验证、发布。

原生产待办仅保留：在微信平台重置已暴露的小程序 AppSecret 后再验收通知功能；这不属于 Web UI 2.0 代码实现缺口。

停止条件：微信认证文件的 Git `HEAD`、`origin/main` 与服务器 `current-release` 一致，公网认证 URL 返回 200 且正文精确匹配；完成微信管理员认证/临时恢复所需的用户侧操作后，再进入 ICP 页脚 checkpoint。
