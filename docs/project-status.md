...
- 交付（用户授权"1，2，3都做"的第1项）：体验版`0.1.0-p10.20260917.153`（说明“Skyline 3.17.2 wheel data channel a18f869”）production/clean上传成功，Manifest`72b7dcb4…da79fd`，候选前置与上传后绑定检查PASS；可信ensure只追加`.153`，独立verify与`ecs-verify.sh`通过，公网`.153=200`、`.152=200`、动态未知`=426`。待小米14复核"数字按行跟着动/单位出现/中间项放大"，3.17.3不变。
- 第2、3项待办：② 修开发者工具模拟器白屏后补一轮 DevTools 验证（官方 AI 开发模式要求 Nightly Electron 版，装 Nightly 亦可作为白屏的修复路径）；③ 小程序 AI 开发模式 generate→validate（需公众平台申请"开发模式"+开发者工具服务端口，且该模式代码不得合入正式提审版本）。详情见`docs/audit/runtime-ui-compatibility-wheel-data-motion-trial-release-20260917.md`。
