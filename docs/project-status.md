...
- Debug rounds: 1–57 已完成；fix-progress 轮次 1–42 已完成（轮次 22 = #7.1 子步骤 3 批次 11：exports/platform + users/profile 收尾读模型 zod schema 替换 2 个手写守卫，子步骤 3 全部完成；轮次 23 = #7.4：前端 swap/duty 候选与 isFutureAssignment 重复合并为共享 workflow 逻辑；轮次 24 = #8.1：抽共享 toUserMessage 统一错误文案模板；轮次 25 = #3.7：框架 4xx 按状态码映射契约错误码；轮次 26 = #4.2：swap/duty 冲突断言统一为共享接口；轮次 27–30 = #4.3 入口骨架/依赖容器/入口迁移 + #4.4 软删除函数合并；轮次 31 = #4.5 仅未来日期守卫 + #7.6 移除“调”标记；轮次 32 = #3.5 idempotency 仅重复键走查重路径；轮次 33 = #5.1 retry skipped 如实计数；轮次 34 = #5.2 群组回收清单锁定 + scanned 语义；轮次 35 = #5.3 统计重建失败上下文；轮次 36 = #6.1/#6.2 迁移回填合并 + 序列校验；轮次 37 = #2.2 数据库客户端公开 connection 事件 + UTC 会话初始化；轮次 38 = #1.3 工作区清理 + #9.3 共享测试基建；轮次 39 = CloudBase 弃用清理 + 阿里云部署审计；轮次 40 = 第 6 节 TODO 三条 + web-push 声明缺失；轮次 41 = #N1 compose 重复 NODE_ENV 修复 + CI compose 校验；轮次 42 = #N2 试用期 Nginx 浏览器密码门禁（临时措施，微信登录上线前移除））；最新验证基线 584/584（72 个测试文件，隔离 MySQL）
- Deployment: CloudBase 已于 2026-08-07 弃用并清理；部署目标固定为阿里云 ECS 试用机（`8.148.183.46`，Docker Compose：Nginx + Fastify + MySQL，试用期 `NODE_ENV=development + AUTH_DEV_MODE=true` 开发模式认证，另加 Nginx 浏览器密码门禁作为临时访问保护）；本地部署继续维护（API `127.0.0.1:3000/3001` 与 Web `localhost:5173/5174` 保持最新构建，开发模式认证可用）；PWA shell/schedule 缓存 `v6`；正式上线（域名/ICP/HTTPS/微信账号认证/定时任务）待推进
- Next actions: 轮次 57（fetch 接收者修复，登录按钮无法进入页面）已完成并待用户强刷复核；fix-progress 轮次 41/42 已完成（#N1：compose.prod.yml 重复 `NODE_ENV` 修复 + CI compose 校验；#N2：试用期 Nginx 浏览器密码门禁，用户选择方案二，微信登录上线前必须关闭并移除）；下一目标：N7（#7.3 时区转换收尾）→ N3/N4 等快速清理项 → 按阿里云部署路径推进（自建/微信账号认证 → 域名/ICP/HTTPS → 定时任务 cron → 正式 MySQL/最小权限账号 → 重新生成 `runtime/api-flat` 后部署）；等待用户部署/验收
...
- 2026-08-07 fix-progress 轮次 42：完成复审新发现问题 N2——公网试用机开发模式认证存在“任意 Bearer token 冒充任意 UID（含管理员）”风险；按用户决策（方案二）加 Nginx 浏览器密码提示作为临时门禁：`nginx.prod.conf` 改为官方镜像模板（`auth_basic ${NGINX_BASIC_AUTH_REALM}`），compose 以环境变量开关并挂载服务器本地生成的 `.htpasswd`（不入库），`.env.production.example`/部署文档注明“仅限非正式测试阶段，微信小程序上线改用微信账号登录前必须关闭并移除”；真实 nginx 容器 + 无头 Edge 浏览器实测：开启=无凭据 401/带凭据 200，关闭=200；`pnpm verify` 584/584（72 测试文件）通过，compose config 与 prettier 检查通过，`pnpm smoke:check-core` 通过（未涉及 Web 核心链路）。
...
- 试用期门禁（2026-08-07 用户决策）：N2 选择方案二“浏览器密码提示”（Nginx Basic Auth）。当前为非正式测试阶段，门禁为临时措施；微信小程序上线、网页改为微信账号登录时必须关闭并移除门禁（`NGINX_BASIC_AUTH_REALM=off` → 删除 `.htpasswd` 挂载与 `auth_basic` 两行）。
