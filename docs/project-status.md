...
- 线上发布：代码 checkpoint `1725ee7a5e62a65237193d5cf95092f435cad16b`（`fix(miniprogram): support group settings large text`）已推送；从精确 release worktree 上传体验版 `0.1.0-p9.20260827.35` 成功，153 个代码文件、zip `1,404,131` bytes、上传 manifest `94c3e00e3577acc3639f54b8a36fb587455a98efcc847e1c27671dd348826d65`。生产发布前数据库备份 archive `7823ca56-7120-423e-a34d-b696bb2535ea`（54 表、176,331 行、81,527,260 bytes、SHA-256 `0574d8a6407416bd21aececfbdb870df26486fff21836148033a9f201beef9b1`）；`.35` 已原子加入版本白名单，能力值未新增开放。
- 线上验证：代码 release `1725ee7a5e62a65237193d5cf95092f435cad16b` 已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.35` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 与 Git 一致，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- 线上发布：代码 checkpoint `1725ee7a5e62a65237193d5cf95092f435cad16b`（`fix(miniprogram): support group settings large text`）已推送；体验版 `0.1.0-p9.20260827.35` 上传成功，153 个代码文件、zip `1,404,131` bytes、上传 manifest `94c3e00e3577acc3639f54b8a36fb587455a98efcc847e1c27671dd348826d65`。代码部署前数据库备份 archive `7823ca56-7120-423e-a34d-b696bb2535ea`（54 表、176,331 行、81,527,260 bytes、SHA-256 `0574d8a6407416bd21aececfbdb870df26486fff21836148033a9f201beef9b1`）；`.35` 已原子加入版本白名单，能力值未新增开放。最终状态 docs checkpoint `77b635741acdd050c59da161df4a671b9c3417ee` 也已推送并完成 docs-only 部署；该次备份 archive `757b2e0e-3036-4d39-b599-930cd6579710`（54 表、176,350 行、81,533,772 bytes、SHA-256 `4c4df576d5b59b13dc345f81f5ffc6e996983fed237cc53372f0fe97c282f313`）。
- 线上验证：代码 release `1725ee7a5e62a65237193d5cf95092f435cad16b` 与最终 docs-only release `77b635741acdd050c59da161df4a671b9c3417ee` 均已部署，预热首个健康请求 502 后恢复；迁移、privacy-retention（deletedRows=0/remainingRows=0）、健康、产物/控制面哈希、域名/IP 隔离、容器和依赖检查均通过。`.35` capability HTTP 200，`global/core/workflows/organization/guest=true`、`insights/externalMessages=false`；当前 `current-release` 为 docs-only release，`.env.production` 保持 `root:root/0600`，远端本轮临时目录已清理。
- checkpoint/下一步：代码与状态文档 checkpoint 均已提交、推送、体验上传并完成生产部署/验证；本轮用户已明确无需人工复核，自动测试、构建、能力探针与 verifier 为验收依据。下一活动批次为 P10/P9 剩余状态分支自动审计（只处理 1 项），停止条件是覆盖矩阵无未覆盖项且验证/发布闭环通过。
