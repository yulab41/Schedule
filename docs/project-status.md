...
## 当前批次：五类通知模板服务器已启用，新体验版待授权

- 用户提供并确认四个业务模板及五个独立授权按钮；换班完整姓名MMDD班种缩写紧凑摘要、原因状态前缀、超长查看提示均已实现。代码22707cf6已推送、部署API/Web，schema57；详见docs/audit/feedback12-template-integration.md。
- 四类模板8项配置原子写入，其余配置不变；运行API核对5个唯一模板、四类字段与微信实际字段全部一致。原值班模板保留，医护两群trial配置继续生效。完整生产verifier和版本策略检查通过，109/108=200、未知426。
- 部署备份a62363df-2e39-4bf6-95c3-8a6f057f8889已核验真实文件hash；无本地数据上传、新增迁移、历史重发或真实发送测试。自然cron正常，本轮sent0，不能宣称真实收信通过。
- pnpm verify完整通过：Mini1164/15跳过、根1251/434跳过、依赖保护81；隔离MySQL132、390/320五按钮CSS几何、本地完整浏览器冒烟通过。小米14原生和实际通知尚待验证。
- 唯一下一任务：用户确认已弹出的新体验版上传/追加放行请求，使五按钮可在手机中使用；当前未分配版本或上传，仍为109。二维码及导出真机诊断待复核，不重复护士数据导入。
- 文档检查点docs(ops): record five-template activation，文档release只允许可信hash-identical复用，应用内容仍22707cf6；最新服务器release见正式manifest/current-release。
