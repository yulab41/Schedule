...
- 回归先失败后通过；MySQL46项证明名称同步及原排班不变，Node运行时和真实WXML/生产CSS的390/320几何通过。主体逻辑不是等价重构；保留this接收者、异步错误范围和回调次数，明确修改当前周测量时机、选择更新范围及用户批准行为。
- 运行/浏览器验证：pnpm smoke:browser（同仓库node scripts/smoke-browser.mjs入口，本地API3105/Web4175，canonical本地env仅内存适配）通过登录/管理员/成员/访客及访问记录。最初生产preview不含开发登录按钮，改用文档要求的本地开发模式后通过；不是业务故障。未连接生产或发送真实通知。
- 全量检查首轮只失败于旧静态“单模板”断言；按批准行为更新，保留三模板上限/单按钮及权限检查。最终pnpm verify通过：Mini1184/16跳过、根1252/439跳过、依赖保护81。额外偏好保存失败RED1→通知35通过，最终Mini verify/Worklet2/2通过。smoke:check-core通过；本地管理员原标记回读恢复、临时服务停止，原生待同版本小米14。
...
- 运行验证：b618d938修复提交部署后完整线上verifier及既有allowlist verify通过，schema57不变。pnpm smoke:check-core按控制脚本范围检查；无Web核心改动，不重复浏览器冒烟。直接ESLint该既有.mjs超出项目lint配置报旧URL/process全局错误，改用项目pnpm lint通过，不改无关源文件。
...
- 交付收口：.104=b618d938官方上传被微信以-10008 invalid ip拒绝；已记录IPv4路线TLS通过，但同三元组重试及独立getrandstr请求均ECONNRESET。无成功receipt、未追加放行，.102保持200、.103/.104均426。部署和两份真实备份证据见[feedback10-release.md](../audit/feedback10-release.md)；下一步仅处理允许的微信上传出口并续传，不重复服务器部署。
...
- 引入点：82840db9轮询在请求返回后才检查超时；47733e68微信发送遗漏page；040f03b7本人测试幂等；ddd5c107邀请访客QR异步与常驻提示。QR具体风险与先失败后通过证据见[feedback10-qr.md](../audit/feedback10-qr.md)。
- 已建立前台悬挂、创建前检查悬挂、取消前未发出请求、创建结果未知、下载前置等待及失效身份回调的失败回归；修复独立截止和操作代次，不把服务器排队改为同步导出。保持四字段模板、原this/观察器绑定、鉴权、仅本人发送与限频。
- 运行/浏览器验证：本地隔离MySQL导出与通知12项通过；源CSS桌面headless Chromium在320/390和普通/大字体下覆盖CSV28种、二维码与通知8种布局。pnpm smoke:check-core未触及Web核心链路，无需运行核心浏览器冒烟；桌面证据不能证明微信原生验收。
- 四项代码和自动化运行验证完成，待用户复核。Mini1099/15跳过、root1234/420跳过、MySQL12通过；依赖保护首轮一项失败后未改实现复测81通过；最终Mini verify/Worklet2/2通过。检查点消息：fix: bound CSV exports and add QR saving and notification links。本轮未授权部署、上传、放行或真实通知，原生验收待用户复核。

## 2026-09-10 FEEDBACK9 群组偏好、矩阵、预览月历与 CSV

- 用户批准九项方案，并另行授权无活跃 general 槽位恢复和离线锁文件校准。执行范围、基线和全部证据见 [feedback9.md](../audit/feedback9.md)。
- 引入点经 log -S / blame 核对：dffef1f2 内层偏好卡片；591ccff6 固定七行、窗宽估算、今天后异步默认日期；50744302 双层预览卡片；8e68a480 的 10×14 标识；1f715c96 月历统一阴影；de710eaf 导出下载与打开。
- 新增回归首次 9 失败/1 通过，修复后扩充的日期竞态、节假日和几何测试通过。变更属于用户批准的行为修复，不声明等价重构；维持身份、权限、保存/发布和补录语义。
- 运行/浏览器验证：桌面 headless Chromium 使用当前源 CSS、合成 fixture 检查 320/390px 月历和矩阵；姓名裁切与矩阵边界检查通过。pnpm smoke:check-core 确认未触及 Web 核心链路；没有运行或宣称原生开发者工具验证。
- 状态：九项代码和自动化运行验证完成，待用户复核。已合入并行656a463d；pnpm verify通过（Mini1057/15跳过、root1226/418跳过、依赖保护81），最终Mini verify/Worklet及320/390桌面布局通过。真实CSV失败原因与下载/发送、原生交互尚未验证；本轮未上传或生产操作。
...
## 2026-08-27 小程序工作台导航与“更多”重组

- 反馈与引入点：用户要求底栏按 Web 手机版改为“日历、通讯录、换班、我的、更多”，并让群组管理等二级工具成为可侧滑返回的独立 Page。`git log -S`/`git blame` 定位原五项底栏由 `ad4cfb2c` 引入，群组管理内嵌由 `59300957`/`0d971de17` 引入。
- 测试先行：新增五项精确顺序、工作台内切换、三组“更多”、独立 Page 路由、共享嵌入态和构建可达性回归，旧实现 5/5 失败；实现后定向 12 files/77 tests、Mini 全量 92 files/440 tests 通过。
- 实现与视觉：工作台只保留 calendar/directory/swap/profile/more 五个一级状态；通讯录与个人中心首次打开后常驻 hidden，顶部与底部壳不卸载。群组管理、请假、加扣班通过现有 `navigateTo` Page 栈打开；“更多”按群组与排班、信息与通知、访问与平台三组展示。通讯录和个人中心新增共享 embedded 模式，独立 Page 继续静态 include/import，未重新引入 Skyline 大型 Panel 薄 Page。
...
## 2026-09-01 `.75@24a847f` 小米 14 第二批启动诊断

- 第二批 5 个 request ID 与 production API 日志逐一匹配，均 `/employee-directory` HTTP 200；完成
  618.4/667.7/164.4/291.2/406.2ms 与 Server-Timing 一致。1 条被新搜索替代，服务端仍完成但页面
  没有提交旧结果，竞态隔离有效。
- 新 App 启动真正首搜为倒序 `#5`：total/TTFB/server/main=685/649/617/412ms，较首批异常低
  93.2%/93.5%/93.8%/95.8%；服务端外 32ms、响应后 21ms、诊断序列化 0ms。App 启动到下一渲染
  约 6,634ms，页面加载到下一渲染约 4,590ms，首个目录页面在 App 启动后约 2,044ms 加载。
- `#4` 页面会话序号也为 1，但 App 启动累计从 5,949ms 增至 17,863ms，且同一内存报告保留 `#5`，
  证明它是同一 App 运行会话的第二次通讯录挂载，不是第二个独立 `App.onLaunch`；该次仍仅 725ms。
...
- 三次独立首搜为 10,016/685/799ms，中位 total/TTFB/server/main=799/741/705/505ms；两次连续复测
  ...
  main query >1.5s 时再收集单条并升级调查。未操作代码、数据库、上传、allowlist、production、备份
  或 release。

## 2026-09-01 `.75@24a847f` 小米 14 半屏原生验收

- 用户明确确认四项全部通过：弹层高度约半屏；横条下滑达到阈值关闭、短拖未达阈值正常回弹；列表
  内部滚动不误关闭；“清除全部筛选”固定可见。
- 证据层级为当前 `.75@24a847f` 在小米 14 Android 微信体验版上的用户人工原生交互验收；Web 黄金图
  未作为原生结论，仓库禁止代理操作微信开发者工具，故 Console/Network 输出仍未采集。结论不外推到
  iOS、全部安卓或全平台。
- 本轮纠偏审计由此收口，性能仅保留 total >2s 或 main query >1.5s 的长尾触发条件；不启动阶段 B
  优化。最终文档 checkpoint 以 `docs(audit): close .75 Xiaomi half-sheet acceptance` 识别，不触发上传、
  allowlist、production、数据库备份或服务器 release。

## 2026-09-01 `.75@24a847f` 性能最终口径与观察边界

- 三次独立 App 首搜为 10,016/685/799ms。当前证据排除稳定的客户端首次搜索退化，但确认首批发生过
  一次服务端主查询约 9.7 秒长尾；后续两次独立启动未复现。三次测试集中在十几分钟内，未覆盖服务端
  或数据库长时间冷态，长尾根因与发生率仍未知，不标记为彻底解决。
- 正常首搜约 0.7–0.8 秒，本轮不再优化。正常样本的精确计数查询约 180–200ms；若未来继续压缩正常
  延迟，先单独确认页面是否需要精确总数，不与 10 秒长尾混合归因。
- 保留 total >2s 或 main query >1.5s 观察阈值；自然使用再次超限时，按 request ID 调查查询计划、
...
- 最新主线整合：验证期间 `origin/main` 经 `fa10d5ba` 前进到 `765b5c09`；官方
  `scripts/codex/worktree-deps-*` 取代临时
...
## WECHAT-REBIND-20260908：解绑后换绑及网页微信登录退役

- 引入点：15ee912f的clearMatchingDetachment要求新账号等于原账号；Union旧关联也会重新解析回原账号。网页登录起点12e7f402；诊断无身份仍可发送起点040f03b7。
- 行为变化：显式目标校验+范围/账号锁+事务内凭证失效与审计；移除Union运行时决策与网页微信授权面；三个稳定错误码、失效表单清理、订阅后重查身份；无迁移或业务数据删除。并非等价重构。
- 先失败后通过：往返换绑2、网页旧会话2、诊断身份2、过期凭证4；真实MySQL65与最终verify（Node81/Mini936/root1185）通过。受条件环境影响的跳过数量见审计报告。
- 运行/浏览器验证：pnpm smoke:browser 原脚本完整流程通过；认证专项覆盖密码错误、登录、恢复、退出及回调清理。合成管理员标记临时调整后恢复，访客本地配置在内存提供，断言未更改。桌面320/390大字号代理通过。
- 完成状态：自动化及浏览器已完成，待新版本交付后用户复核；没有生产操作、真实发送或体验上传。完整记录docs/audit/wechat-rebind.md。

## 2026-09-08 feedback6 第一检查点

- 变更：08:00业务日期、班种展示色、模板实际可用性、共用自绘日期、跨月岗位补录草稿与格内差异；旧单条补录写事件，通讯录readiness启动预检/后台复核。引入点与语义差异见docs/audit/feedback6.md。
...
## FEEDBACK8-20260910：预览、邀请和微信提醒回归

- 基线6729ec0a，详情见docs/audit/feedback8.md。引入点：strict预览d7793719、Mini端点591ccff6；固定高度/颜色丢失/隐藏补录提示50744302；通知字段ef3d20ca、提醒批次52e9e1f4；邀请路径a50c4fce/cf453205、Mini文本ddd5c107。
- 行为变化：完整草稿快照可解码、DELETE合法JSON、月历拟排班叠加原排班删除线及自适应弹窗；邀请卡片接收/登录续接/会话及生命周期保护；四字段通知映射与实际模板核验、有效收件人校验及按成员去重。collectMarkers只增加导出，原调用行为不变。
- 运行/浏览器验证：pnpm smoke:browser已运行，通过本地配置封装保留全部原断言；管理员/成员/访客vkey/访问记录通过，无浏览器错误。先前本地模式未开启的启动失败已修正；合成管理员标记已恢复。
- 生产账户范围后来由用户改为3个具体账户并确认删除，已备份、事务回滚演练、正式删除及验证；剩32/24/8，排班4011及成员归属保留，见feedback8-account-cleanup.md。随后应用8e68a480已按用户根AGENTS自动部署规则完成备份、部署及核验；用户另行授权后ca634116已上传体验版.99并只追加放行，详见feedback8-trial-release.md；未主动真实发送。
- 最终验证：pnpm verify退出0（Mini996/15跳过、root1215/402跳过、依赖保护81）。独立复核发现本轮未提交裁切样式影响长确认弹窗，恢复591ccff6原非预览滚动，裁切只限月历；回归先失败后通过，21项定向及最终Mini verify通过。最终主包1764091/总包5324841，320/390桌面几何通过，pnpm smoke:check-core通过。真实MySQL15项同源码旧证据保留，本轮Docker故障未新增集成结果。状态：应用本地验证完成、待版本交付及用户真机复核。

## 2026-09-10 群组双向访客权限

- 引入点核对：群列表8e42afb8、登录访客日历1b1eebdb、群回收覆盖门禁9e4a6765；已执行git log -S与git blame。新功能不是语义等价重构。
...
## Feedback14 更多页位移与导出白屏（2026-09-12）
...
## 2026-09-14 头颈 DOCX 一值轮转矩阵纠正

- 用户导出的 9 月 DOCX 仅显示 11 个日期，且日期按人员分别聚合，未遵循“先上后下、先左后右”。`git log -S` 与 `git blame` 定位到 `c60aa523`：该实现按当前成员 ID 直接过滤并以个人最多排班数确定列数，历史成员 ID 会被静默漏掉，也无法表达月初/月末空白和中途轮空槽位。
- 测试先行：新增 30 天从末行开始的列优先夹具，旧实现不能得到首列前五行空白、末列末行空白；新增中途跳过一人必须显示 `-`、历史成员 ID 归一及未知人员拒绝生成断言。实现后相关 8 项通过。
...
- 状态：已实现待小米14复核；当时未上传、未放行、未部署生产。唯一建议下一任务：取得当次授权后上传体验版并 add-only 放行，复核 A/B/C；若滚轮仍不能滚动，请回复“点滚轮中间那一项有无反应”。详情见`docs/audit/runtime-ui-compatibility-wheel-pager-and-locate-20260916.md`。

## 2026-09-16 Skyline 3.17.2 滚轮/切月/定位当日 体验版149上传与放行

- 用户在当前消息授权「上传并放行」并重申最小改动原则。候选 = `7e215a28d7730fcf30a33504b0af6cd39583c567`（分支`codex/runtime-3172-wheel-and-pager-20260916`），独占warm槽`runtime/wt/general-3`（lease token`fc135866…`、RUN_ID`runtime-3172-wheel-pager-upload-20260916`、REUSE_ONLY、无安装）。
- 血缘与前置：`git fetch`后`origin/main@4179f05a`是候选祖先（0 个落后提交），远端最新体验版 tag`.148@c5f06e50`也是候选祖先；`prepare-release-worktree.mjs --purpose upload`冻结候选，`check-worktree-safety.ps1 -RequireReady -ExpectedCommit … -LeaseToken … -RunId …`返回`STATE=ready-clean-detached RESULT=PASS`。首次冻结因槽位分支头不在候选上被拒（`Candidate no longer belongs to its leased task branch/base`），用 fast-forward 把槽位任务分支从`origin/main`推进到候选后通过，未改写任何已推送历史。
- 上传（脚本在独占锁内分配版本、构建、建不可变 tag）：`0.1.0-p10.20260916.149`，说明「Skyline 3.17.2 wheel pan and month paging 7e215a2」，production，Manifest`442f89339f4c47ad5a83cbe5ca1d28a176b96e951c330f3c93628e052f236fc8`；分配`14:49:18Z`、构建`14:48:54Z`、上传`14:50:21Z`；上传后同一 checker 加`-ForMiniprogramUpload -MiniProgramVersion 0.1.0-p10.20260916.149`返回`VERSION_LOCAL=absent`、`MINIPROGRAM_PROFILE=production-clean`、`RESULT=PASS`。远端轻量 tag`miniprogram-trial/0.1.0-p10.20260916.149`指向同一 SHA；`runtime/audit/miniprogram-trials/0.1.0-p10.20260916.149.{allocation,manifest,}.json` 三份记录齐全且字段一致。
- 放行：`root@hosp.schedule.eylinhome.top`（`IdentitiesOnly=yes`、`StrictHostKeyChecking=yes`、仓库外密钥）执行可信`schedule-client-version-allowlist ensure 0.1.0-p10.20260916.149`——只追加1项、白名单共45项并保留`.146/.147/.148`，重建API/Web容器期间出现既有短暂502后按健康等待恢复；独立`verify`与`/usr/local/lib/schedule/ecs-verify.sh`（`[verify] complete`）通过。公网HTTPS：`.149=200`、`.148=200`、动态未知`=426`。本轮未部署应用制品、未备份或迁移数据库、未提审、未正式发布，也未声明 production live release（`LIVE_RELEASE_VERIFIED=false`）。
- 状态：已交付待小米14原生复核。唯一建议下一任务：双实例复核3.17.2滚轮可滚动/切月动效方向与3.17.3一致/请假弹窗定位当日一次到位，并确认3.17.3三项不变；若滚轮仍不能滚动，请回复“点滚轮中间那一项有无反应”。详情见`docs/audit/runtime-ui-compatibility-wheel-pager-trial-release-20260916.md`。

## 2026-09-17 Skyline 3.17.2 滚轮仍不滚动（位移通道被模板覆盖）与定位当日偶发无反应

- 用户真机复核`.149`：换班年月滚轮**仍无法滚动**——上一轮的`catchtouchstart/catchtouchmove`（手势隔离）没有效果，因此"手势被祖先滚动容器抢占"这条判断被证伪；同时请假左右切月**不再乱跳**（`duration: 0`+直接结算生效），定位当日**偶尔点了没反应**且月份离当月越远越容易遇到。
- 引入点/根因 A：`git log -S "wheelInitialOffset"` 确认位移的内联绑定与 WXS 滚轮同由`57e10cdc`引入。滚轮位移由 WXS 写在`#ui-wheel-track`的 transform 上，而同一节点在 WXML 里还挂着`style="transform:translateY({{wheelInitialOffset}}px)"`；拖动时每帧`handleWheelPreview`→组件`setData({internalSelectedIndex})`→该节点重渲染，3.17.2 会把内联样式整条重新下发，覆盖掉 WXS 刚写入的 transform（内部 offset、选中高亮、settle 都正常，只有像素不动）；3.17.3 对未变化的样式做 diff，所以旧行为在 3.17.3 与更早的基础库上看起来正常。这与"同款组件+遮罩在 gesture-probe 真机曾可用"的旧证据一致。
- 根因 C：`.149`的定位当日走"把今天的月份面板放进相邻槽位 + 等共享 pager 结算"，只要存在未结算位移（连续点箭头、快速连点、上一次`animationfinish`还没到）就被开头守卫吞掉 → 偶发无反应，越远点得越多越容易撞上。
- 修复：A 删除该内联`style`绑定，位移完全由 WXS 拥有（符合既有"WXS 独占像素样式"约定），组件侧不再计算`wheelInitialOffset`；并且手势在`touchStart`用节点自身 dataset（新增`data-item-count`/`data-selected-index`）自建基线并立即应用一次样式，避免运行时没交付 config observer 时滚轮直接失效。C 改为一步重定中心（`resetDatePager` + 一次`setData({...createDateDraftPatch(today), dateSwiperDuration})`），不再依赖待结算状态；删除已死的`_dateLocateTarget`机制与`formatMonthValue`。
- 测试先行：新增`seeds its own baseline when the config observer never reaches the wheel`，用`git stash push/pop`回退 WXS 后该用例失败（`expected undefined to be 'translateY(-264px)'`），修复后通过；同步改写滚轮模板断言（track 不得再有`wheelInitialOffset`内联绑定、必须暴露 dataset）与两条定位当日用例/契约（一步重定中心、无待结算状态）。
- 语义等价审计：定位当日不再经过`requestCalendarPeriodShift`/`finishDatePeriodShift`的组合，改为`resetDatePager`+`createDateDraftPatch`。横向滑动与左右切月仍只在`animationfinish`（3.17.2 为零时长跳变后的直接结算）提交，`_datePendingSelection`、队列续走、`commitCalendarPeriodSwipe` 语义未改；WXS 侧只新增"缺 state 时按 dataset 自建基线"，`configure` 正常到达时该分支不触发，`state.offset`、`animationToken`、轴判定与投影逻辑逐行未变。
- 验证结果：定向53项+新增用例、Mini完整174文件1216项通过/16跳过；typecheck、production build366文件、package（主包1744556B/总4618961B）、determinism`21cae2df…3e758`、format、lint、smoke:check-core、agent-context-policy通过。`pnpm miniprogram:verify`仍只被既有未改手排矩阵`1507>1506`阻断。运行/浏览器验证：本轮只改`apps/miniprogram/**`，不触发`smoke:browser`核心链路清单。
- 状态：已实现待小米14复核；本轮**未上传体验版、未放行、未部署**（当前消息未含上传授权）。唯一建议下一任务：取得当次授权后交付体验版并 add-only 放行，复核滚轮拖动与定位当日；若滚轮仍不动，请回复“拖动时中间那一项高亮是否跟着换”或“非中间项数字是否比中间小且更淡”，以区分样式通道与事件通道。详情见`docs/audit/runtime-ui-compatibility-wheel-style-channel-20260917.md`。

## 2026-09-17 滚轮位移通道修复 体验版150上传与放行

- 用户在当前消息授权「上传并放行」。候选 = `a0707b0ce1041d0281bcaacf3acbf07bf99bf043`（分支`codex/runtime-3172-wheel-and-pager-20260916`），独占warm槽`runtime/wt/general-3`（lease token`a872fb0b…`、RUN_ID`runtime-3172-wheel-style-upload-20260917`、REUSE_ONLY、无安装）。
- 血缘与前置：fetch 后`origin/main@4179f05a`与远端最新体验版 tag`.149@7e215a28`都是候选祖先；`prepare-release-worktree.mjs --purpose upload`冻结候选，`check-worktree-safety.ps1 -RequireReady -ExpectedCommit … -LeaseToken … -RunId …`返回`STATE=ready-clean-detached RESULT=PASS`（槽位任务分支同样先 fast-forward 到候选，未改写已推送历史）。
...
- **读图边界（修正）**：初稿曾把 3.17.2 截图读成"行间有明显渐变 → 行样式通道可用"，但该判断在截图分辨率下无法确证，且与用户文字"没有选中字体放大效果"冲突；用户随后澄清两张图分别对应 3.17.2 与 3.17.3。该条从"反证"降级为未证实假设，本轮修复**不依赖样式通道是否可用**：单位与条目总数走数据通道（组件自身 `data` 已被"初始定位正确"证明可用），选中强调另加 3.17.2 专属 CSS 兜底。
- 修复（全部走数据通道，最小改动）：① `createWheelOptions` 给每个条目带上 `unit`，模板渲染`wx:if="{{item.unit || unit}}"`（条目优先、组件属性兜底）；② `data-item-count` 由`{{items.length}}`改为组件自身 data `{{wheelConfig.itemCount}}`，与已验证可用的`data-base-index`同路径；③ 新增 `refreshItemCount`：同一打开周期内条目数只增不减，忽略 host 报出的瞬时更短列表，避免一次瞬时渲染把可滚动范围剪短；④ 仅在 3.17.2 给滚轮根节点加`is-skyline-3172-ui`（组件自己按`SDKVersion`判定），用 CSS 兜底选中行放大与不透明（`opacity:1; transform:scale(1.06)`、数字`font-size:30px`）——WXS 行内样式存在时优先生效，写不进渲染器时由 CSS 接管，3.17.3 不匹配该选择器、外观与动画不变。
...
## 2026-09-17 开发者工具默认授权 + 官方工具链接入

- 用户要求并授权：Schedule 仓库默认同意 LLM 驱动开发者工具。据此修改 `apps/miniprogram/AGENTS.md`（"must never start/control/automate" → "may ... by default"；同时把 Node 工具那条改为"亦允许按边界驱动 DevTools"）与仓库 guardrails skill 的 `SKILL.md`、`references/miniprogram.md` 两处禁令，保留边界：无当次授权只读；上传/提审/正式发布/生产凭证读取需当次明确批准；证据标注层级，DevTools 不替代小米14原生验收。`validate-project-skill.ps1` → `RESULT=PASS`；`agent-context-policy` 3 项通过。
- 工具链安装：① 官方 `wechatide-skill` 覆盖升级到 v0.3.11（25 文件，旧版备份 `wechatide-skill.bak-20260917-144202`）；② 把用户提供的 MCP 配置按 Codex TOML 形式注册到 `~/.codex/config.toml` 的 `[mcp_servers.wechat-devtools]`（`cmd /c wechatide mcp --token=…`，token 仅存仓库外，配置已备份 `config.toml.bak-20260917-1445`，`tomllib` 解析通过）；③ 状态检查（带 `--skill-version 0.3.11 --token`）返回 `success=true / versionRelation=equal / loginExpired=false / mcpTokenRequired=true`。安装脚本自检 `check-installation.mjs` 报 `cli_unavailable` 是它自身在含空格路径上的调用问题，CLI 实际可用，因此未进入下载/更新流程。
- 官方文档要点（已读并落地判断）：`小程序 AI 开发模式(beta)` 需要 Nightly Electron 版开发者工具 + 基础库 3.16.2 的"小程序 AI 编译"模式；SkillHub 的 `wxa-skills-generate` 基于源码生成原子接口/原子组件，`wxa-skills-validate` 依赖开发者工具渲染与执行做闭环校验（需先在"设置 → 安全设置"开启服务端口）；官方明确该模式**代码不得合入正式提审版本**。两个 skill 已装到 `C:\Users\eylin\.codex\skills\@tencent-adm\`（SkillHub CLI 走官方 latest kit + 本机 Python 手动落地，因官方 install.sh 是 bash+python3，在 Windows 会装进 WSL）。
- 状态：规则与工具链已就绪，下一步用 DevTools 对当前滚轮修复（`cf6fbf40`：3.17.2 像素改走数据通道）做一轮模拟器验证；注意模拟器的 Skyline 不会复现真机"丢弃 WXS setStyle"的行为，因此模拟器只能验证接线与视觉（单位、初始定位、数据驱动的位移样式串），设备级结论仍需小米14。

## 2026-09-17 开发者工具 fullMode 复现并验证滚轮修复（授权清单第2项）

- 用户当次授权：用 `fullMode` 重开项目窗口、以测试账号登录，重跑第2项。项目 `runtime/wt/general-3`（独占 warm 槽、REUSE_ONLY、无安装），分支 `codex/runtime-3172-wheel-and-pager-20260916`，验证对象 `.153`（`a18f8692`/`d3c9ca9f`）。
- 定位（此前"模拟器白屏"的真实原因，非版本问题）：`build-tools.mjs` 把 `WECHAT_CI_VERSION` 兜底为 `local`，于是客户端请求 `GET /api/client-capabilities?version=local` 被生产端以 `400 VALIDATION_FAILED` 拒绝，工作台永久停在"正在读取排班"。带 `WECHAT_CI_VERSION=0.1.0-p10.20260917.153` 重建后同一请求 `200`，真实生产数据全部加载。窗口必须用 `--window-mode fullMode`；基础库由 `project.private.config.json` 的 `libVersion` 决定，本轮据此在同一份构建上切换 `3.17.2` ↔ `3.17.3`。
- 验证方法（因工具限制而调整，见下）：向真实 `ui-wheel-column` 注入一次上报 `handleWheelPreview({index:3,offset:-132,generation:1,runtimeKey:'diagnostics-wheel-probe',sequence:1})`，它走的正是真机已验证可用的 `callMethod`+模板数据通道。
- 结果（A/B，同构建仅切基础库）：3.17.2 → `skyline3172UiCompatibility=true`、`wheelTrackStyle='margin-top:0px;transform:translateY(-132px)'`、`#ui-wheel-track` 渲染矩形 top `11642→11510`（−132px）、`.ui-wheel-unit` 8/8、选中数字高≈25.2px vs 未选中≈17.9px、选中项盒子 127.2×46.64。3.17.3 → `compat=false`、样式串 `margin-top:0px`、轨道 top 保持 `11642`。→ 修复在真实 3.17.2 生效（数据通道真的推动像素、单位出现、中间项放大），3.17.3 无副作用。
...
## 2026-09-17 原生分页闪烁/卡顿/队列修复与 `.158` 交付

- 用户回传 `.157`：滑动卡顿；松手后先闪滑动前月份再出现新月；高度自适应要等闪烁结束才开始；快速连滑被内容加载阻塞。
- 根因三条：① 归位跳动写在结算里，发生在**宿主新面板渲染之前** → 那一帧中间面板仍是旧月 → 先闪旧月；② 拖拽过程中每个 `bindscroll` 都可能因槽位变化写 `viewportHeight` → 逐帧 `setData` → 卡顿；③ 上一步数据未回时 `shiftPending` 会拒绝新的一步 → 快速手势被挡。
- 修复：① 归位改为**只在面板同步时**下发（`syncCompatPanes`/`syncCompatDatePanes` 同一次 `setData` 里给新面板 + 归位目标，同一渲染帧）；无步可提交或上一步未回时单独归位（此时面板本就该停在当前月），不再出现旧帧。② 拖拽期只记录状态，**高度只在结算时写一次**。③ 手势方向**入队**（复用 `requestCalendarPeriodShift` 的有界队列），数据到达后自动继续，并把滚动先归位避免停在侧面板。
- 验证（3.17.2 模拟器连续取样）：首页月历右滑两次再左滑一次 → **2026-09 → 2026-10 → 2026-11 → 2026-10**，每次 `target=month-pane-1`、`compatPanes.relative=[-1,0,1]`；请假选择器同样序列 **2026-12 → 2027-01 → 2027-02 → 2027-01**，随后按钮前进 → **2027-02**。
- 门禁：typecheck、Mini **1222 项通过/16 跳过**、package 总 **4646321B**、determinism`2df84730…`、format、lint、smoke:check-core 全通过。
- 交付与放行：候选 `8351312d`（`general-5` 新租约、`merge --ff-only`、冻结与 `check-worktree-safety` 两次 `RESULT=PASS`）；`.158` 上传成功（说明「Skyline 3.17.2 pager no stale frame 8351312」，Manifest`63414cb6…4cc0`）；可信 `ensure` 追加 `.158` 保留旧版并通过健康与策略验证；`ecs-verify.sh` `[verify] complete`；公网 `.158=200`、`.157=200`、未知`=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 状态：已交付待小米14复核。唯一下一任务：小米14 打开 **`.158`** 复核滑动是否跟手不闪、高度是否即时、快速连滑能否连续切月，并确认 3.17.3 无变化。

## 2026-09-17 原生分页"可见面板不变月"修复与 `.159` 交付

- 用户澄清 `.158` 的闪烁：切换后**短暂出现的是别的月份的单元格内容**，定位跳转同样。
- 机制（实测确认）：滚动跳转与内容更新在渲染器里不同帧，而提交那一下**可见面板自身也换了月份**（右滑后右侧槽由"下一月"变成"下下月"，实测 `panes[2]` 从 2026-10 变成 2026-11，而用户正看着它）。
- 修法（不变量）：提交时让**用户正看的面板与中间面板同月**（右滑→`[prev, cur, cur]`；左滑→`[cur, cur, next]`），随后归位；**干净的三面板映射等归位真正落地**（下一次滚动事件到达中间面板）再替换，那时被改动的面板已在屏外。任何一帧可见面板的月份都不变。
- 同时：高度提前到**滑动方向确定的那一刻**切换（实测拖拽中 `viewportHeight` 372→310）；归位与新面板同一次 `setData`；上一步未回时方向入队；两个原生滚动容器加 `bounces="{{false}}"`（无属性告警）以压制惯性末端的内容拉伸。
- 验证（3.17.2 模拟器取样）：提交后 `panes=[2026-09,2026-10,2026-10]`（`middleMatchesRight=true`、`cleanupPending=true`）→ 归位落地后 `panes=[2026-09,2026-10,2026-11]`（`cleanupDone=true`），月份 2026-10。
...
## 2026-09-18 程序化切月改轨道同帧过渡与 `.162` 交付（"高度仍两段式"）

- 用户回传 `.161`：滑动顺滑 ✓、连按不再卡死 ✓；但高度仍要等滑动结束才变，并问"3.17.3 为何能同步"。
- 分析（回答用户问题）：3.17.3 的同步来自"滑动与高度是**同一元素上的同帧动画**"——`swiper` 240ms 与高度过渡 240ms 在同一次 `setData` 下发；3.17.2 兼容路径用 `scroll-into-view` + `scroll-with-animation` 的**原生平滑滚动**，真机把随后的高度/布局变化压到滚动结束之后。模拟器逐帧测得下方内容在滑动期间已在位移（661→649.4→602.5→599），即本机不复现，因此按真机证据换掉动画通道而不是继续调写入顺序。
- 修法：程序化切月（按钮与定位）改成分页轨道的 `transform` 过渡，**高度与位移同一次 `setData`、同一时长与缓动**（240ms、`cubic-bezier(0.33,1,0.68,1)`）；提交时在同一个更新里换回干净环并把轨道归零（可见面板月份不变），程序化步骤不再需要"等滚动事件落位"的清理；手势路径与 `.161` 的兜底保障（兜底结算/清理定时/先归位再滑/提交后对齐 `gridHeight`）全部保留；3.17.3 的 swiper 分支逐字未改。
- 验证（3.17.2 模拟器）：一次 372→310 的切月，轨道 -365→-476→-593→-661→-712→-715 与高度 372→340.8→322.5→313.5→310.4→310 并行完成（≈250ms）；连按 4/6/10 次每步都有滑动且结束态干净（`cleanup=false`、`viewportHeight==gridHeight`）；连按后手势正好一月；滑动中落指先归零再交给手势、该步正常提交。
- 门禁：typecheck、Mini **1224 项通过/16 跳过**、package 总 **4650161B**、determinism `eb798495…`、format、lint、smoke:check-core 全通过；新断言在旧源码上先失败（RED）。未改 `workbench/index.ts`，血缘证明无需刷新。
- 交付与放行：`076c897f` 已推送；候选在独占 `general-5` 冻结（前后 `RESULT=PASS`）；`.162` 上传成功（说明「Skyline 3.17.2 pager height sync 076c897」，Manifest `ad51d1b8…cbbf`）；可信 `ensure` 追加 `.162` 保留 `.161`；`ecs-verify.sh` `[verify] complete`；公网 `.162=200`、`.161=200`、未知 `=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 状态：已交付待小米14复核。唯一下一任务：小米14 打开 **`.162`** 复核 ①切月时高度与滑动是否同时变化/同时落位；②连按不卡死且每步有滑动；③定位仍不推动整页；并确认 3.17.3 无变化。

## 2026-09-18 `.163`：提交期保住可见面板（修箭头/定位闪动）+ 单元格内联 px（修手势横向抖动）

- 用户回传 `.162`：高度基本同步（仍稍落后）✓；新增两个问题——①**箭头/定位切换时单元格内容闪动**（手势滑动不闪）；②**手势左滑（未来月份）单元格横向抖动**，右滑不抖。
- ① 定因：`.162` 在提交时把"换干净环 + 轨道归零"放在同一更新，但换掉的面板正是用户要看的：提交前可见的是位移后的右侧面板（新当月），提交后可见的是中间面板，而中间面板内容此刻才由旧当月换成新当月 → 单元格按 `businessDate` 整体重建 → 可见帧闪动。手势路径的换环发生在屏外侧面板，所以不闪。修法：提交改用**保住可见面板**的映射（右滑 `[prev,cur,cur]`、左滑 `[cur,cur,next]`），轨道**下一 tick 再归零**（中间面板内容与可见面板一致，看不出位移），干净环继续走既有清理兜底 → 可见面板单元格键全程不变。
- ② 定因（推断；实测合成触摸无法驱动 Skyline 滚动，度量停在中面板）：兼容分支单元格仍为百分比宽度，而 3.17.2 在原生滚动容器里解析百分比链不可靠；左滑常伴随新月份数据回填触发重排 → 百分比重新解析 → 横向抖动，右滑多为缓存月份无二次回填。修法：兼容分支单元格改**内联 px**（`width:52px;`，面板宽度/7，与 `.156` 的面板 px 同源），并镜像到请假页日期网格。验证：`selectAll('.calendar-cell-slot')` 112 项宽度直方图全部 52px。
- 高度再提前一帧：真机高度变化要先做布局，现在 `nextFrame(leadHeight)` 先发高度、下一帧再启动轨道过渡，抵掉这一帧开销；手指在半帧内落下时先归零轨道再交给手势（`_compatSlideActive` 守卫）。
- 验证（3.17.2 模拟器）：一次切月轨道 -351→-534.3→-641.8→-681.8→-703.8→-713.2→-715（≈250ms）→提交→归零，结束态 `trackStyle=''`、`cleanup=false`、`viewportHeight==gridHeight`；连按 4 次逐月推进不卡死；连按后手势仍正好一月。新断言在旧源码上先失败（RED）。
...
- 交付与放行：`76712d29` 已推送；候选在独占 `general-5` 冻结（前后 `RESULT=PASS`）；`.163` 上传成功（说明「Skyline 3.17.2 pager commit hold 76712d2」，Manifest `a25a9f58…36d8`）；可信 `ensure` 追加 `.163` 保留 `.162`；`ecs-verify.sh` `[verify] complete`；公网 `.163=200`、`.162=200`、未知 `=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 状态：已交付待小米14复核。唯一下一任务：小米14 打开 **`.163`** 复核 ①箭头/定位不再闪动；②手势左滑横向抖动消失且右滑不退化；③高度与滑动同时落位；并确认 3.17.3 无变化。若②仍在，下一步把拖动期的高度改写改为"滑动结束再改"。

## 2026-09-18 `.163` 回退与 `.164`（撤回延迟归零 + 排队手势就地提交）

- 用户回传 `.163`：**比上一版更卡**；高度要等活动结束约 1 秒才落；快速切月出现**"有切换动画但内容不变"的假死**；定位闪动更明显；手势横向抖动仍在。并补充：3.17.2 **必须等蓝色选中框出现才能触发下一次手势滑动**，之前滑会"落空"。
- `.163` 回归定因（代码级）：延迟归零靠 `_compatSlideResetPending`，而该标志在 `syncCompatPanes` 开头被**无条件消费**；`panels` 观察器可能因同批第二次 setData 先触发一次 → 标志提前消费 → 归零丢失 → 轨道停在侧面板、`_compatSlideActive` 不复位 → 清理被挡 → `gridHeight` 观察器一直让位 → **内容假死 + 高度迟到（约 1 秒）**；高度拆成两帧下发也让过渡更慢。
- 修法：撤回上述两处（回到 `.162`：高度与位移同一次 `setData`、提交时原子换环 + 轨道归零），保留单元格内联 px 宽度（含请假页日期网格）。"落空"：忙窗口内的手势原先在队列后立刻 `recenterCompatPanes` 弹回原位；现在**保留手指位置**并在上一步结束后**就地提交**该增量（`_compatQueuedInPlace` + `finishMonthSwipeAt`），左右按钮排队仍走滑动。
- 验证（3.17.2 模拟器）：一次切月轨道 -351→-641.8→-714.6 与高度 372→322.5→310.1 并行（≈250-300ms）；连按 4 次 12 月→2027-04 不卡死；切月途中补手势 → 月份再前进 1 且结束态干净。RED：新/恢复断言在旧源码上先失败。
- 门禁：typecheck、Mini **1224 项通过/16 跳过**、package 总 **4650884B**、determinism `153070fb…`、format、lint、smoke:check-core 全通过；未改 `workbench/index.ts`，血缘证明无需刷新。
- 交付与放行：`4bd30e5e` 已推送；候选在独占 `general-5` 冻结（前后 `RESULT=PASS`）；`.164` 上传成功（说明「Skyline 3.17.2 pager revert+queue 4bd30e5」，Manifest `a38c0e58…0e61`）；可信 `ensure` 追加 `.164` 保留 `.163`；`ecs-verify.sh` `[verify] complete`；公网 `.164=200`、`.163=200`、未知 `=426`。未部署应用制品、未备份或迁移数据库、未提审、未正式发布。
- 未解决：箭头/定位闪动、手势左滑横向抖动（内联 px 未消除）。取证边界：合成触摸驱动不了 Skyline 滚动、模拟器截图 35×75 不可用 → **请用户提供 5 秒手机录屏**（先按两次左右按钮，再左滑、右滑各一次）；若无法录屏，改用"拖动期不改高度、滑动结束再改"的单变量试验。
- 状态：已交付待小米14复核。唯一下一任务：小米14 打开 **`.164`** 复核是否恢复顺滑/不假死/高度不迟到、忙窗口手势不落空，闪动与抖动仍在则附录屏。

## 2026-09-18 `.164` 复测定因与 `.167` 交付（含 IPv6 烧号事故）

- 用户回传 `.164`（不便录屏）：滑动比 `.163` 顺滑 ✓；仍**轻微横向抖动**；高度**仍慢半拍**；**定位生硬无动画**且跳转后"正确本月内容 → 闪一下 → 又是正确本月内容"（＝可见面板节点重建，非别的月份）；并说明必须等蓝框出现才能触发下一次手势滑动。
- 定因：①定位从别的月份**有** 364px 轨道滑动（模拟器实测）；当前月即本月时走 `applyTodayLocation` 直接重渲染 → 无动画（"生硬跳转"）。②"闪一下"＝可见面板的单元格节点在提交时被重建：`.164` 把"换干净环 + 轨道归零"放在同一更新，换掉的正是用户要看的中间面板；`.163` 的"保住可见面板"思路是对的，失败原因是标志 `_compatSlideResetPending` 被任意 `panels` 更新提前消费。
- 修法（3.17.3 swiper 分支逐字未改）：①标志只在真提交（`delta !== 0`）时消费 → 提交出 `[prev,cur,cur]`、轨道下一 tick 归零（两面板同一对象，可见帧不变）；②高度过渡 240→**200ms**（滑动 240ms）让两者同时结束；③手势高度改在结算时一次（用户提出的单变量试验，针对横向抖动）。
- 验证（3.17.2 模拟器）：连按 6 次 → 6 次归零、Sep→2027-03 逐步推进、结束态 `cleanup=false`/`track=''`/`viewportHeight==gridHeight`；一次切月轨道 -351→-641.8→-714.6 与高度 372→322.5→310.1 并行；定位 2026-11→09 有完整轨道滑动。RED：新断言在旧源码上先失败。
- 门禁：typecheck、Mini **1224 项通过/16 跳过**、package 总 **4650981B**、determinism `889cfc96…`、format、lint、smoke:check-core 全通过。
