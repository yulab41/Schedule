...
- 状态：已实现待实体复核。packager 写 LF `components.d.ts` 后内容 hash 与 HEAD 相同但 status stat-only M；helper 第 8 项回归先红，改用 content diff/cached diff/untracked 三项后转绿，真实差异仍阻断。checkpoint=`fix(release): ignore stat-only worktree noise`；推送后所有后续 release 均先显式备份，再部署/验证并收口状态。
