...
- 运行验证：Mini production verify 通过（2/2 Worklet、4,343,850 bytes、manifest `83591d359ebf44b553796f07fb8a2d5b08854c2e0610d155cf07fb2019a2dad6`），CI dry-run、包审计、任务 Prettier/ESLint、根 build/typecheck、根 Vitest 233 files/1,113 tests（37 files/352 tests 按环境跳过）、`git diff --check` 与 `pnpm smoke:check-core` 通过；未触及 Web 核心链路，无需 `pnpm smoke:browser`。checkpoint 识别消息为 `feat(miniprogram): align workbench navigation with web`，待推送、体验上传和生产同步后补充发布证据。
