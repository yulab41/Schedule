...
- 交付（用户当次授权“上传并放行”，并提示最新版已到`.140`）：候选先`merge c6cbb6a6`（`.140`来源，其祖先为`.138`的`09c100d5`）保证血缘连续，`.139/.140`修复随候选一并交付。首次候选`46eee448`已分配`.141`并创建远端tag，但微信官方`summer-wxss`编译器拒绝WXSS中的`//`行注释（`Unexpected '/'`，code 10037），上传失败，该号码按仓库规则永久占用不复用；随后把注释改为`/* */`，并在`scripts/build-tools.mjs`的`validateWxss`新增“WXSS不得使用行注释”的守卫及单元用例，使同类缺陷在`check:source`/测试阶段失败。修复后源码`8988afe6`已推送，体验版`0.1.0-p10.20260915.142`（说明“Skyline 3.17.2 picker inline 8988afe”）production/clean上传成功，Manifest `2b8d0f3f…d4317a`；候选前置与上传后绑定检查`RESULT=PASS`。合并后复测Mini1211项通过/16跳过及typecheck/build366/source/package/determinism/format/lint/smoke:check-core通过，主包1742411B、总4610065B。可信`schedule-client-version-allowlist ensure 0.1.0-p10.20260915.142`只追加并保留`.140`；allowlist verify与`ecs-verify.sh`通过，release仍`44034fcc`，无应用部署或数据库操作；公网`.142=200`、`.140=200`、`.141=426`、动态未知`=426`。未使用微信开发者工具，未提审或正式发布。详见`docs/audit/runtime-ui-compatibility-picker-inline-trial-release-20260915.md`。
