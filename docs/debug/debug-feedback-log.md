...
- checkpoint/下一批：checkpoint 识别消息 `feat(auth): harden p8 platform identity mutations`；计划上传 production-profile `.99-identity` 但不 allowlist/提审/正式发布，再完成生产备份/部署/full verifier。随后进入 P8-B Web 黄金，不提前写 Mini 原生页面或开启 organization capability。
