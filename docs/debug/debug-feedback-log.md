...
### API integration test runtime investigation (2026-08-12)

- Reproduction: `pnpm vitest run` for leaves, swaps, and duty adjustments reported 3 files / 78 tests skipped. The files select `describe.skip` whenever `NODE_ENV !== test` or required `TEST_MYSQL_DATABASE`, `TEST_MYSQL_USER`, and `TEST_MYSQL_PASSWORD` are absent; the local Vitest command does not load `.env`.
- Environment evidence: the dedicated `medical-schedule-test-mysql-1` MySQL 8.4 container is running and healthy on the test port; `infra/docker/compose.test.yml` defines a `tmpfs` database and is separate from the persisted development service on 3306. No credential values or production/development data were read.
- Provenance: `git log -S 'describeWithDatabase'` and `git blame` identify leave `0d5ec55c`, swap `b20ff9b`, duty `5d8b205a`; `vitest.config.ts` has `fileParallelism: false` from `0a794d9a` because every integration file resets the shared test schema.
- Approved design: `docs/superpowers/specs/2026-08-12-api-integration-test-runtime-design.md` limits the follow-up to a guarded explicit runner and real execution of the three workflow suites. Task 10 remains out of scope.

### API integration test runtime completion (2026-08-12)

- TDD: `scripts/run-api-integration.test.mjs` first failed because its runner module did not exist. After the guard implementation, tests passed. First real execution failed before tests because Node `spawnSync` cannot execute the Windows pnpm `.cmd` shim (`EINVAL`); direct Node execution of the local Vitest module was verified. That then exposed Vitest's transformed test context lacking `import.meta.resolve`; the runner now uses the stable repository `node_modules/vitest/vitest.mjs` URL. Independent review identified untested child-process behavior, so spawn-stub cases now lock the Node/Vitest launch, exact suite list, child-only `NODE_ENV`, exit propagation, and no spawn on rejected input (5/5).
- Safety/runtime: `pnpm test:api-integration` loads `.env` only for this explicit command, rejects any database other than `schedule_test`, missing TEST credentials, nonlocal hosts, and invalid ports; it passes `NODE_ENV=test` only to the spawned Vitest process and never prints secrets. It invokes exactly leave, swap, and duty-adjustment integration files serially under the existing global `fileParallelism: false` contract.
