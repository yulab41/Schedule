...
- RED旧实现4失败；仅3.17.2解除220px宽度上限、使用带同源token的root-portal菜单、移除周格灰色hover类、月格松手后0ms清除蓝色反馈。3.17.3保留原菜单分支、周格按压类和月格70ms；分页状态机和动画时长未改。
- GREEN定向30项、Mini完整1199项通过/16跳过；typecheck/build/source/package/determinism/format/lint通过。主包1738379B、总4606033B，较`.135`总包增1766B。Mini verify仅被未改手排节点预算1507>1506阻断。
- 运行/浏览器验证：本轮不触及Web核心，`pnpm smoke:check-core`执行并通过。未调用微信开发者工具，root-portal的原生合成和四项真机结果仍待同SHA体验版复核；当前未上传或放行。
