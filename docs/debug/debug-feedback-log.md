...
- 反馈：用户指出（原）（拟）徽标出现的人员/班种没有规律、要求「点击已有人员同一班种＝消除该排班（确认后才生效）」、终结点不再即时生效、取消手排预览的单元格点击详情，并追问月视图/访客/补录/预览的选中圆弧是否都已统一。
- 根因：周视图把「当天第一条」排班标为（原）而不按班种匹配（[backfill/index.ts:842-855](E:/AItools/Schedule/apps/miniprogram/src/subpackages/scheduling/pages/backfill/index.ts:842)），`alreadyMatchesCurrentAssignment` 同样只看第一条 → 会暂存重复人员；草稿键为「岗位+日期」，一天只能一条；月视图的删除线只是显示、服务端从不删除。
...
- 反馈：用户人工观察周视图左下/右下角选中蓝色框圆弧比直线细，怀疑被遮挡或本身更细；授权做最小统一改动。
- 归因：直线与圆弧是同一支 `inset box-shadow: 0 0 0 2px`；开发者工具（allowlist 通过的 `.198` 构建、真实排班）实测圆弧外缘半径 16.96–17.21px 等于设计值 17px，无遮挡；圆弧峰值不透明度 0.83–0.92 低于直线的 0.94–1.00，差异来自抗锯齿。
- 结构性不一致：首页 `.week-day-grid` 裁剪圆角 18px（`var(--ui-radius-large)`）与格子 17px 不匹配，隔离 CSS 复现会把圆弧外层削掉约 0.3px；补录/预览共用组件为 17/17。
- 改动与测试：只把首页网格圆角改为 17px；`calendar-week-regression.test.mjs` 新增圆角一致断言先失败后通过。Mini 全量 183 文件/1280 通过/18 跳过，Mini verify 通过（总包 4,817,612 B），`smoke:check-core` 通过。
- 复测：开发者工具左下圆弧 2.131 → 2.259 CSS px、峰值 0.92 → 0.965；右下 1.876 → 1.997、0.826 → 0.887。证据在 ignored `runtime/audit/week-arc-20260925/`。
- 交付：用户授权后上传体验版 `0.1.0-p10.20260925.199`（候选 `e5305bd0`、Manifest `14b8aeb2bb4e0f038a29428f2498ef38a471bcf73493705e9f048aac332104df`、239 文件、ZIP 2,703,859 B）；受信 add-only `ensure` 只追加 `.199`，独立 verify 与完整 `ecs-verify.sh` 通过（一次 502 后恢复），线上 release 仍 `ad6c09fe`；公网 `.199`/`.198`=200、未知版=426。
- 边界：未部署生产、未备份或迁移数据库、未提审或正式发布；小米 14 同构建复核仍待用户证据。
...
## 2026-09-19 月历修复体验版 `.173` 已上传并放行（用户选择 B）

- 解除阻塞：用户确认并行 `runtime-3172` 线的改动已下令撤回，选择“撤回式合并”。在 main 上执行 `git merge --no-ff -s ours 44a25885 -m "merge: retract runtime-3172 trial line, keep main content"`（`48102b9a`）：内容保持 main 现状、`.172` 成为祖先。合并前后逐文件核对本批 11 个文件 blob（`runtime/codex/calendar-fix-blobs-premerge.json`），**mismatch=0**，滑动修复未被抹掉。
- 上传：候选 `48102b9a0288a9771cd5535f015198e66c2ff1e`，独占 general-5（lease `2e768281`、upload 用途、`check-worktree-safety.ps1` = `PASS/ready-clean-detached`，依赖 REUSE_ONLY 未安装）。`miniprogram:upload-experience` 动态分配 `0.1.0-p10.20260919.173`，description `calendar swipe fix 48102b9`，production profile，Manifest `5a31d0535f422ba56ee547cb49eda56a33cc67e658a04ff85119de38fe946522`，ZIP 2,625,730B，237 code files；远端轻量 tag `miniprogram-trial/0.1.0-p10.20260919.173` 绑定同一 SHA，receipt 写入 ignored `runtime/audit/miniprogram-trials/0.1.0-p10.20260919.173.json`。
- 放行（L4，用户当次授权）：按 ignored 运维笔记的物理路线校验后连接——正式域名、known_hosts 中唯一匹配的 IPv4 候选、三家 DoH 一致、HTTPS 证书有效且 health 200、SSH `HostKeyAlias`+`StrictHostKeyChecking=yes`+`BatchMode`+`IdentitiesOnly`。可信 `schedule-client-version-allowlist ensure 0.1.0-p10.20260919.173` 只增不删、重建 API/Web（预热 7 次 502 后恢复），随后 `verify` 通过；公网探针 `.173`/`.172`=200、动态未知=426；`ECS_PUBLIC_IP` 完整 `ecs-verify.sh` 通过（release `44034fcc` 未变，未知 Host 421 拒绝、artifact 哈希一致、容器健康、迁移计数正常）。
- 边界：本次为 Mini-only + 文档批次，未部署生产应用代码、未做生产数据库备份、未同步服务器 release 标识；未提审、未正式发布、未退役旧体验版。general-5 已释放。
- 唯一下一任务/停止条件：用户在小米 14 微信客户端打开体验版 `0.1.0-p10.20260919.173`，在日历首页月视图连续快速左滑（含一次不松手连续左滑两次），确认月份单调向前、内容不回退、无“跟不上”的旧月份闪现；未取得与 `48102b9a` 构建一致的真机结论前，不得写“小米 14 验收通过”。

## 2026-09-19 `.173` 交付错误：漏掉 2026-09 运行线内容，改用 `.174` 重发

- 用户复核反馈：`.173` 完全错乱、没有继承 `.172` 的内容、版本像回退了很多。复核确认：`.172` 与 `.173` 的**内容差异达 199 个文件**，而生产 live release `44034fcc` 明确属于 `.172` 那条线（`runtime-3172` 运行线，含 WebView-only 渲染、PoC 页面删除、日期选择器/覆盖层/滚轮与 API 改动）。我的 `.173` 用的是 main 内容，等于把那条线整套客户端工作丢掉，与服务器 API 不匹配。
- 修正：从那条线内容重建候选。做法是在 main 上 `git read-tree -u --reset 44a25885` 把工作树切到 `.172` 内容，再把这批滑动修复**移植**到该线上（pager 状态机、月组件 `finishMonthSwipeAt`/`finishPeriodShift`、`ui-date-picker` 的 `finishDateSwiperAt`/`finishDatePeriodShift`、以及该线独有的**周历环形 pager**（workbench/guest 各一处）同步 adopt；页面仅放宽 `MonthChangeEvent.delta` 类型）。该线没有 PoC 页面，故原 `calendar-poc` 回归测试按其 `calendar-month.test.mjs` 重写；刷新了该线 `trial-lineage-policy` 中 `pages/workbench/index.ts` 的等价证明 blob（其余 8 个 pin 逐一 MATCH）。
- 验证：`tsc`、完整 Mini 174 文件/1205 项通过（16 跳过）、Mini production verify 通过（包体 4566020、manifest `7888bd41…`）；候选 `697795eb6fb039b839b2d393218337d4e97fda70` 已推送到 `codex/schedule-65704-00e7c79fd459`。
- 交付：候选经 general-5 upload 用途 lease 与 `check-worktree-safety.ps1`（`PASS/ready-clean-detached`）后上传 `0.1.0-p10.20260919.174`（description `swipe fix on 172 line 697795e`、production、Manifest `80967ffa7e43ca5158c38ccea15915e9bfde833ab8ae8c44920a0ccdf033d251`、233 code files、ZIP 2,610,719B、远端 tag 同一 SHA、receipt 在 ignored 目录）。放行按运维笔记物理路线校验后 `ensure .174` 通过（重建 API/Web，预热 11 次 502 后恢复），`verify`、公网探针 `.174/.173/.172`=200、未知=426、`ECS_PUBLIC_IP` 完整 `ecs-verify.sh`（release `44034fcc` 未变、未知 Host 421、artifact 哈希一致、容器健康、迁移计数正常）全部通过。
- 边界与遗留：`.173`（错误内容）仍被 allowlist 放行，退役需用户另行明确批准；本次未部署生产应用代码、未备份生产库、未提审/正式发布。候选内容目前只在 `codex/*` 分支与 `.174` tag 上，尚未并入 main——是否需要把这 199 文件的内容线正式落到 main，等用户决定。general-5 已释放（REUSE_ONLY，未安装依赖）。
- 唯一下一任务/停止条件：用户在小米 14 打开体验版 `0.1.0-p10.20260919.174`，确认（1）整体内容与 `.172` 一致（不是回退版），（2）月视图连续快速左滑月份单调向前、内容不回退、无旧月份闪现，（3）周视图连续快速左右滑同样不回退；未取得与 `697795eb` 一致的真机结论前，不得写“小米 14 验收通过”。

## 2026-09-19 快速滑动只前进一个月 + 换月加载延迟（`.175`）

- 用户反馈：150ms 一次快速左滑，一秒内滑 5–6 次，动画都正常，但月份一直停在 9 月，全部停下后才变成 10 月——5 次滑动只前进 1 个月；并询问是否应优化月历数据的提取/加载/显示。
- 复现：在开发者工具上用交错事件流仿真（`change(2)`,`change(0)`,`change(1)`,`change(2)`,`change(0)` 与各自的 `animationfinish` 交错）复现 `.174`：`businessMonth` 从 09 变成 **08**（倒退），环锚点与原生索引分离。根因是提交门禁只接受“与最后一次 change 相同槽位”的 finish，并把位移按模 3 的**单格差**计算：中间几次 finish 被丢弃、最终只算一格。
- 修复：pager 改为**按 change 事件累计有符号步数**（每格 ±1，上限 ±6），finish 只提交/排队累计值，settle 时一次性 adopt；重放/取消（步数为 0）仍被忽略。同时触摸路径**不再回写 `swiperCurrent`**（只在程序化换月时写），避免手指还在滑时被强制跳回；日期选择器与该线独有的周历 pager 同步采用新语义。
- 验证（Agent 操作开发者工具，非实体设备）：同一 5 次快速滑动仿真，修复后 `2026-09 → 2027-02`（正好 +5）；仓库回归测试新增“5 次快速滑动=5 个月”“累计步数上限”“重放 finish 不移动”等，RED→GREEN；完整 Mini 174 文件/1209 项通过（16 跳过）、`tsc`、Mini production verify 通过（包体 4565056、manifest `637a0492…`）。
- 加载延迟测量与优化（同一换月，开发者工具）：目标月份在滑动前**已在内存缓存**（无需网络），但每次换月触发 3 次页面 patch ≈104KB/143ms、104KB/98ms、104KB/88ms（共约 313KB、约 330ms 视图工作）——瓶颈是重复下发而非网络或服务器。新增“只下发真正变化的键”后：第三次整包刷新被整包跳过、第二次从 104KB 降到 41KB，单次换月从 3 次/约313KB 降到 2 次/约145KB。
...
## 2026-09-19 预取窗口重复读取修复 + 缓存策略评估（`.177`）

- 用户提问：切换月份后是否会重复预取已取过的窗口；节假日/补班能否常驻缓存；已过排班能否常驻缓存、只在补录或后台写库时静默更新。
- 实测（DevTools，钩 `wx.request` 统计）：窗口内的月份**不会**重复请求（9→10 时 11/12/9/8/7 全部命中内存），但**滚出窗口的月份会被丢弃**——11→10→11 往返时刚取过的 `2027-02` 被重新请求；且**每次换月都会重新请求 `/api/holidays?year=`**（同年数据不可能变）。修复后同一往返：回退段 1 次请求（真正新进入的月份）、**返回段 0 次请求**、**holiday 请求 0 次**。
- 已实施：①页面级有界 `monthCache`（LRU，12 条，上下文切换时清空）让离开窗口的月份仍可命中；②`holidayReads` 会话内按年去重 + 新的持久化节假日缓存（`schedule.workbench.holidays.v1`，**TTL 24h**，跨会话复用，强制刷新路径绕过）；③回归测试新增“跨会话复用节假日、过期后只重新校验一次”，完整 Mini 1210 项、`tsc`、production verify（包体 4568881、manifest `fc84d9ba…`）通过。
- 缓存策略评估（回答用户的第二问）：节假日适合常驻（按年发布、体积小），但必须有 TTL 或版本校验，否则服务端修订不会被发现。**已过排班**同样适合常驻——`past-schedule-limits` 已限制历史编辑，实际改动只剩补录/换班/后台导入——但关键前提是**需要可靠的失效信号**：实测服务端 `groups.version` 只在换班、职责调整、日历偏好、访客码、发布模式时自增，**手工排班、补录、排班周期修改并不 bump 它**，因此现在**不能**用它做“已过排班”的缓存键。推荐分三步：A 会话内缓存（本批已完成）；B 持久化节假日 + TTL（本批已完成）；C 已过排班常驻缓存，需先给排班读接口加一个轻量 `calendarRevision`（如该群 assignment 的最大 `updated_at`/版本），客户端按版本缓存并在进入/回前台时静默校验，任何 DB 写入都会自然推进该版本。弊端需同时接受：越界写入后的短暂陈旧、存储增长（需上限与清理）、双缓存不一致风险、以及访客/个人数据不得进入该类缓存。
- 交付：候选 `44b98811e88639d915ed7dabbcf2944c992ef75a` 已推送 `main`；体验版 `0.1.0-p10.20260919.177`（description `calendar read cache 44b9881`、Manifest `10276baea9a71fe5d73353c0e8610029d4bc2d6855f199ac0362b8b7ded22eb7`、234 code files、ZIP 2,612,573B、远端 tag 同一 SHA）。放行 `ensure .177`、`verify`、公网探针 `.177/.176`=200、未知=426、完整 `ecs-verify.sh`（release `44034fcc` 未变）全部通过；`.173` 仍待用户批准退役。
- 唯一下一任务/停止条件：用户在小米 14 打开体验版 `0.1.0-p10.20260919.177`，确认快速滑动与换月手感、并留意跨会话首次进入时是否明显更快；若要继续做“已过排班常驻缓存”（C 步），需先批准为该群排班读接口增加 `calendarRevision`。

## 2026-09-19 极致读缓存与增量同步：群级变更日志 + 客户端游标（`calendar-delta-sync`）

- 用户批准方案（原文要点）：切月/切周**永不等待网络**；进入页面即可用缓存渲染；数据变更由**服务端版本号驱动静默增量更新**；节假日/补班按版本常驻；已过排班常驻但必须有可靠失效信号。
- 本轮契约偏差（已在上方 project-status 记录，此处给技术原因）：`calendarReadModelSchema` / `holidayReadModelSchema` 使用 `.strict()`，`schema-generation.mjs` 生成 `additionalProperties:false` 的紧凑解码器（`json-decoder.ts` 的 `compileObjectSchema` 遇到未知键直接返回失败）。给**已发布**响应加字段会让 `.177` 及更早体验版与线上 Web 的 `safeDecode` 失败并抛 `INVALID_RESPONSE`。所以 `revision` / `version` **只出现在新端点** `GET /api/groups/:groupId/calendar-changes?since=<seq>`（`{revision, resync, changes[], holidayVersions[]}`，成员鉴权，访客 403/401）。同时把 `businessMonth` 设计为**可选字段**而非 `nullable`——`sanitizeJsonSchema` 不支持 `anyOf`/`null`，可选项天然表达"影响全部已缓存月份"。
- 服务端实现：迁移 `0062_group_calendar_changes`（`groups.calendar_revision bigint unsigned` + `group_calendar_changes` 表，唯一键 `(group_id, seq)`、`(group_id, changed_at)` 索引、`ON DELETE CASCADE`、含 rollback）；`recordCalendarChange()` 在同一事务内 `UPDATE groups SET calendar_revision = calendar_revision + 1` → 读回新值 → 以该值作 `seq` 插入日志 → 保留窗口（每群 500 条 / 90 天，剪枝条件仅由游标值表达，避免 `DELETE` 自引用子查询的 MySQL 1093 限制）。
- 插桩点：`EventWriter.append` 是发布/撤稿/草稿增删、补录、换班、休假替班、加班、手排模板应用、工作流自愈的**共同事务汇点**，在此统一记录并按 `schedulePeriodId` 解析 `businessMonth`（`schedule_period_*`/`assignment_*` 记为 `schedule`，其余记为 `event`）；`runOrganizationMutation` 新增 `calendarChange` 声明，已接到岗位/班种增改删（`config`）与成员删除/改名/改角色/交接（`member`）；`setAccountMobilePhone` 对每个受影响群显式记录（日历里手机号取自账号手机号，仅联系人行相同也要算变更）。
- 兜底与自愈（关键）：`calendarLedgerDiverged()` 用一条 SQL 把 `schedule_periods / shift_assignments / schedule_events / shift_types / schedule_roles / group_memberships / group_member_contacts` 的 `MAX(GREATEST(updated_at, COALESCE(deleted_at, updated_at)))` 与 `group_calendar_changes.MAX(changed_at)` 比较（删除也计入，纯 `max(updated_at)` 会漏掉软删）。发现未入账写入时**当场补记一条 `config` 变更**：`resync: true` 让客户端整窗重读一次，同时让账本时间戳越过该写入，下次校验即恢复空增量。这样"漏 bump"只会退化为一次整窗读取，而不会永久陈旧——也正因如此，本轮没有逐个写路径手工插桩全部 30 处调用点。
- Mini 端实现：`workbench-read` 新增游标存储 `schedule.wechat.workbench.calendar-cursor.v1:<owner>:<group>`（含形状校验、账号/退群整体清除，已加入 `privateBusinessPrefixes` 与 `clearPrivateBusinessStorageForGroup`）；`readWorkbenchCache(..., { ignoreAge: true })` 让已被服务端确认未变的月份绕过 24h TTL；页面在 `/groups` 返回**之前**就用本地群 ID 同步铺一帧缓存（新增 `prerendered` 标记，避免把"首帧缓存"误当成"已验证数据"），随后只发一次 `calendar-changes`：空增量立即结束，有增量只重取受影响月份，`resync` 或缺省 `businessMonth` 才降级为可见窗口 ±3；游标仅在**所有需要重读的月份都成功**后才前进（失败时保留旧游标，宁可下次重复校验）。节假日按 `(year, version)` 校验，版本前进才重取。端点 404/离线时返回 `UNAVAILABLE_CALENDAR_WINDOW_PLAN`，行为与 `.177` 完全一致。
- 明确不做（避免屎山且避免负优化）：不做"本机写入后清缓存"的本地失效标记。写成功后清掉游标会让 `since=0` 触发整窗 ±3 重读，**比**服务端日志驱动的单月增量更慢；而 `seq` 严格单调，回前台必然取到该变更。
...
## 2026-09-19 逐条复核"极致方案"落地情况（发现并补上 1 个真实缺口）

- 复核方式：按批准计划的每一条在**当前 main 代码**上只读核对，而不是凭记忆回答。
- 已核实落地（附证据口径）：①服务端账本与游标（迁移 0062、`recordCalendarChange`、保留窗口 500/90 天、指纹自愈、`resync` 判定）；②端点 `GET /groups/:groupId/calendar-changes?since=` 为成员鉴权——核对 `permission-service.ts` 的权限表，`viewScheduleConfiguration` 只授予 member/administrator/owner，`guest` 仅有 `viewGuestCalendar`，故访客必然 403；③写入插桩完整性：`updateShiftAssignments` 的全部 12 个调用点逐一反查，全部位于会记录变更的事务内（duty-adjustment 1034→1052、1130→1149；swap 737/741→765、1372/1380→1409；workflow-invalidation 144→76/112；leave-assignment 251/417→224/258；schedule-repository 771（deleteDraft 已记账）、850（preservePastAssignments 由 publish/withdraw 记账）；membership-service 702/710（hardDeleteMembership 由 deleteMember 的 `calendarChange` 声明记账））；④节假日版本：核对 `holiday-service` 只在导入时插入 `version=latest+1` 的新行、确认时仅改 status，故"已确认内容变化必伴随版本前进"成立；⑤Mini 侧缓存优先渲染、单次校验、按受影响月份增量重取、游标仅在全部必需读取成功后前进、端点不可用回落。
- **缺口（本轮修复）**：计划的"缓存保留最近 24 个月（LRU）"没有实现。原持久月份缓存只有 24h TTL；引入 `ignoreAge` 读取后 TTL 不再生效，等价于**无上限增长**（小程序单账号存储配额有限）。修复：新增 `WORKBENCH_MONTH_CACHE_LIMIT = 24` 与 `pruneWorkbenchMonthCache()`，在每次写月份后按 `savedAt` 淘汰最旧条目，保留最新 24 个月；并新增用例（写 26 个月 → 只剩 24，最旧两个月被淘汰、最新保留）。取舍与说明写在代码注释里：被命中的缓存不刷新时间戳（否则每次回前台要写 7 个键），因此"最近"= 最后成功拉取时间。
- 有意偏差（不是遗漏，逐条说明理由）：①`revision`/`version` 不放进既有 `/calendar`、`/holidays` 响应，改由新端点承载——`.strict()` + `additionalProperties:false` 会让已发布客户端解码失败；本轮补了一个契约测试把这条钉死。②不做"本机写入后清本地缓存"：清缓存等于把游标归零，会触发**整窗 ±3 重取**，而服务端 `seq` 严格单调、回前台必然收到该变更并被精确到月，本地标记在这里是负优化。③部署顺序改为"先 API 后体验版"（计划写的是先客户端）：客户端遇到 404 会完整回落到旧行为，故两种顺序都安全。
- 验证：Mini production verify 通过（主包 1696330 / 总包 4594510 字节）；新增 Mini 缓存上限用例后 `workbench-calendar-sync` 6/6、新契约用例 `calendar-changes-client.spec` 3/3；根 `vitest` 复跑 1288 通过 / 448 跳过。
...
## 2026-09-20 LOAD-01–10：缓存、静默同步与服务器读开销

- 引入点经 git log -S / blame：9e3a966c（旧格式扫描/预取）、7470efb1（增量窗口/游标）、0fa49d8d（每会话补号码）、99f222d1（筛选 Map）。用户批准持久联系人和推送优先。
- 行为变化及 RED→GREEN、生产只读基线、回滚和验证层级见 [本轮审计](../audit/loading-cache-server-20260920.md)。不是等价重构；wx.request 保留接收者，旧账号/群组/请求结果不得写回缓存，401/403 不当作离线成功。
...
- 运行/浏览器验证：`pnpm smoke:check-core` 判定未涉及 Web 核心链路；Mini 全量及 production verify 结果见[轮次记录](../audit/mini-package-slim-20260926.md)。`.202@5b12c724` 体验版已上传，Manifest/receipt/tag 一致，正式产物内容审计通过；小米 14 同构建验收仍待证据。
