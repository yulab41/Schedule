...
- checkpoint/下一批：checkpoint 识别消息 `feat(organization): harden p8 invite visitor mutations`；上传/部署前 production `organization` 保持关闭。下一批只做 P8-A2-4 平台用户名分配和管理员绑定链接危险写入，不提前进入 Web 黄金或 Mini 原生页面。
