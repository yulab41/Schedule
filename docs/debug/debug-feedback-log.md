...
- 运行/浏览器验证：本轮改动仅`apps/miniprogram/**`，不触发`smoke:browser`核心链路清单；`pnpm smoke:check-core`通过并确认既有“运行/浏览器验证：pnpm smoke:browser”记录仍然有效。Windows/PowerShell全程`$PSNativeCommandUseErrorActionPreference=$true`，RED与基线对比用`git stash push/pop`在独占租约槽内完成并核对diff一致。
- 验证结果：定向`scripts/workbench.test.mjs`+`scripts/calendar-simulate.test.mjs` 29项通过，日历/访客/性能联合123项通过/1跳过；Mini完整174文件1207项通过/16跳过；`determinism.mjs`通过；`pnpm --filter @schedule/miniprogram verify`改动前后同样只被既有未改手排节点1507>1506阻断；主包1739148→1739424B（+276B），无新增依赖。
- 状态：已实现待体验版复核；未上传、未放行、未部署生产、未控制微信开发者工具。详情见`docs/audit/runtime-ui-compatibility-past-month-gray-20260915.md`。
