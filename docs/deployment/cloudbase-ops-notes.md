...
- 迁移已应用：20 条迁移记录、37 张业务表 + `__drizzle_migrations`（2026-08-03 round 12 补跑 `0018`/`0019`，round 13 补跑 `0020`；`leave_requests.starts_at` 的隐式 ON UPDATE 已在线上修正，请假数据已按用户要求清空重置）。
