...
- 接受邀请时若目标成员已被其他账号认领（典型场景：网页老用户）：若当前微信账号是刚登录的新账号且尚无任何群组关系，则把 openid 合并到已认领账号（只写入 `users.wechat_openid`，不改写既有 `cloudbase_uid`），删除空的新账号，并在接受响应中签发新会话；若当前微信账号已有其他群组关系，返回 409 提示先退出或联系管理员，避免静默合并数据。
- 合并/绑定均写审计日志。
...
4. 确认姓名一致后 `POST /invites/accept { token, confirmRealName }`：
   ...
   - 目标已被其他账号认领：按 5.4 规则合并 openid 到已认领账号并签发新会话（空新账号场景）；否则 409。
   - 无需再填真实姓名、无需认领审批。
...
- 邀请链接同时是身份绑定通道：接受已认领成员邀请时只允许空账号合并，防止跨账号静默合并数据。
