...
- 后端：微信登录/绑定、会话令牌、绑定码、群组访客 key、群组小程序码、邀请链接、订阅消息投递渠道。
- 小程序：登录注册、访客日历、群组日历（月/周/列表）、成员与联系方式、请假、换班、加扣班、审批中心、通知与提醒设置、群管理（成员/认领/角色/班种/排班配置/群码/邀请/转让解散）、统计、我的。
- Web：关闭公开群组列表、访客页改为凭访客 key 访问、个人资料页生成绑定码、登录体验适配微信。
...
- 绑定已有账号时不改写既有 `cloudbase_uid`，只写入 `wechat_openid`；微信认证端口先按 openid 查 `users.wechat_openid`，找不到再按 cloudbaseUid 兼容开发令牌。
...
- 网页端个人资料页 `POST /users/me/bind-code` 生成 6 位数字码，10 分钟有效，5 分钟内最多 3 次，服务端只存 SHA-256 哈希。
- 小程序“绑定已有账号”页输入绑定码，`POST /auth/wechat/bind` 校验后把当前 openid 写入该用户，并返回会话。
- 绑定后历史成员身份、认领、排班数据继续使用同一 UID。
- 绑定码生成、使用、失败均写审计日志。
...
- `POST /auth/wechat/bind` `{ code, bindCode }` → `{ token }`
- `POST /users/me/bind-code`（auth）→ `{ bindCode, expiresAt }`
...
- 统一复用现有 ApiError 协议；新增错误码：`WECHAT_LOGIN_FAILED`、`BIND_CODE_INVALID`、`INVITE_INVALID`、`INVITE_USED`、`INVITE_EXPIRED`、`VISITOR_KEY_INVALID`、`WECHAT_MESSAGE_SEND_FAILED`。
- 限频：绑定码尝试、访客解析、邀请创建、群码生成。
- 审计：绑定、邀请创建/使用/撤销、访客 key 重生成、群码生成。
- 日志脱敏：AppSecret、绑定码、token、openid 不进日志（扩展 `redact.ts` 路径）。
...
- 后端：微信网关 mock 单测；认证/绑定/邀请/访客 key/群码/订阅投递集成测试；`pnpm verify` 全绿；涉及 contracts 改动运行 `pnpm smoke:check-core`。
...
任务 1–3（数据库 + 微信网关 + 认证）→ 任务 4–6（绑定码 + 访客扫码 + 邀请）→ 任务 7（订阅消息投递）→ 任务 8–9（小程序工程 + 登录/API 基础）→ 任务 10–14（页面与功能）→ 任务 15–16（Web 配套 + 部署验收）。
