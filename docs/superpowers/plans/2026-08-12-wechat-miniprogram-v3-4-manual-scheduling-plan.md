...
- 文档日期：2026-08-12
- 状态：**Task 11–13 已实现待 Android/iOS 设备复核**
...
| Task 13 | 应用、草稿、发布/撤回、409                | 已实现，待 Android/iOS 复核 | 不多/少日期、不丢事件、不覆盖更新后的排班          | `f17cad4 feat(miniprogram): add template publishing and conflict protection` |
...
- 自动验证已通过：定向 17 项、小程序 181 项、config audit、typecheck、lint、Prettier、diff check、`pnpm smoke:browser`、`pnpm smoke:check-core`。DevTools CLI 已重试通过：build-npm 无 warning、preview 成功（351.2 KB）、显式 `auto --auto-port 9423` 后复用会话 smoke 打开 13/13 注册路由且无脚本错误；冷启动 smoke 无输出属于已隔离的 transport 边界。需 Android/iOS 复核后才可将状态改为已完成。
...
- [x] Task 13 已依 Task 12 真实代码展开并完成实现、真实数据库 allowlist、自动化检查与 DevTools CLI 复核；仅待 Android/iOS 设备复核，然后在 checkpoint 后停止。
