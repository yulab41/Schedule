...
3. 逐行审查 diff，只显式暂存本任务文件并提交、推送。纯小程序/文档 checkpoint 不自动触发 production 备份、部署或 release 同步；这些操作需要用户当次明确授权。
4. 体验版上传前另行报告短 SHA、版本描述、脏树和测试页面，并取得用户当次明确同意。

停止条件：Git checkpoint 与 clean 构建一致；未获上传批准前保持“小米 14 待实测”，不上传、不操作 production。
