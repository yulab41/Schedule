...
为降低长上下文带来的遗漏和误判，每轮新对话只实施 1–3 个任务，默认 1–2 个任务：
...
3. 首次注册要求唯一用户名、密码和真实姓名。
4. 业务数据库只保存 UID 和资料，不保存密码。
5. 提供获取当前资料和修改真实姓名的受控接口。
6. 为未来微信身份关联保留外部身份映射，不提前实现小程序页面。
...
2. 建立注册和登录表单。
3. 页面启动时恢复 CloudBase 会话，不保存明文密码。
4. 统一处理 401、403、409 和网络错误。
5. 受保护路由在会话确认前显示稳定加载状态，避免闪现业务数据。
6. 增加退出登录。
...
Web 1.0 阶段不开发小程序页面，但必须验证：

- API 不依赖浏览器 Cookie 才能工作。
- 共享协议包不引用 Vue 或 DOM。
- CloudBase UID 与业务用户映射支持增加微信身份。
- 已有用户未来绑定微信时需要原账号或一次性绑定码验证。
- 新用户仍通过真实姓名和群组码认领。
- 通知模块预留微信订阅消息渠道，但不提前发送。

Web 1.0 发布后，微信小程序另建并审阅独立实施计划。
