...
验收记录（2026-08-09）：第 26 行已完成——用户手动导入工程后，修复 `pages/requests/requests.wxml` 的 WXML 不支持表达式（`(三元).length` → JS 预计算 `activeRows`/`emptyRequests`），并把模拟器 `apiBaseUrl` 覆盖为本地 API（`http://127.0.0.1:3000`）后，`pnpm miniprogram:smoke` 两轮通过：未登录轮 31/31 页面打开无脚本错误；登录轮（真实 `wx.login` → 本地登录 200 → 写入会话令牌）31/31 页面均打开真实页面无脚本错误，截图 `.tmp-miniprogram-preview/screens/`。开发者工具冷启动自动化此前 `routeTo appLaunch timeout` 的阻塞已随工程信任/编译缓存与 WXML 修复解除。
