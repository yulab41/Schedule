...
- `READY_REUSE`/`MATCH` means no install. `MISS` reports exact fields and may use the existing official
  `dependency-maintenance.ps1` once per complete fingerprint with `--frozen-lockfile --offline`; it must
  reach `READY_REUSE`, keep the tracked tree unchanged, and record its own ignored audit evidence. This is
  the L2 local frozen reconciliation permitted only by an explicit current-message authorization; it never
  authorizes upgrades, force options, store repair/deletion, production work, or formal publication.
