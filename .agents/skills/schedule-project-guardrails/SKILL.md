...
For `L3`, use the existing release helper to freeze the already-owned clean warm slot with an explicit upload purpose, RUN_ID, SHA and expiry; then run [worktree safety](scripts/check-worktree-safety.ps1) with that same lease before building and again against the version-bound output. See [release candidate](references/release-candidate.md). Run [skill validation](scripts/validate-project-skill.ps1) after changing this skill. The reusable dependency wrapper is `scripts/codex/ensure-worktree-deps.ps1 -Mode ReuseOnly`; the bootstrap wrapper is `scripts/codex/ensure-workspace-bootstrap.ps1 -Profile <mini|api|web|root|release>`. These helpers do not authorize upload, production access or dependency installation.
