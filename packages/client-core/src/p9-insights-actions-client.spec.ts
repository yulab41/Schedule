...
describe('P9 notification and export job client', () => {
  it('encodes notification filters, read actions, and export job paths', () => {
    ...
    expect(p9InsightsActionsEndpoints.getExportJob.path({ groupId: 'group-1', exportJobId: 'job-1' })).toBe(
      '/groups/group-1/exports/job-1',
    );
    expect(p9InsightsActionsEndpoints.createExportJob.body?.({
      groupId: 'group-1',
  ...
  it('decodes strict notification and export payloads', () => {
    ...
    expect(notificationPageSchema.safeParse({ ...notificationPage, extra: true }).success).toBe(false);
  ...
  it('delegates reads, read markers, and export jobs once', async () => {
    ...
    await expect(client.createExportJob('group-1', { exportType: 'statistics', period: '2026-08' })).resolves.toEqual(exportJob);
