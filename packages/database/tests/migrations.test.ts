...
describeWithDatabase('identity and group migrations', () => {
  ...
  it('creates the visitor access retention aggregate and expiry index', async () => {
    ...
    const [indexes] = (await client.database.execute(sql`
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        ...
        AND INDEX_NAME = 'visitor_access_logs_created_idx'
    `)) as unknown as [readonly { indexName: string }[], unknown];
    expect(indexes).toEqual([
      { indexName: 'visitor_access_logs_created_idx' },
      { indexName: 'visitor_access_logs_created_idx' },
    ]);

    const [aggregateTable] = (await client.database.execute(
      sql`SHOW CREATE TABLE visitor_access_monthly_aggregates`,
    )) as unknown as [readonly Record<string, string>[], unknown];
    const createStatement = Object.values(aggregateTable[0] ?? {}).join('\n');
  ...
  it('adds nullable mobile phone consent evidence without creating another table', async () => {
    ...
    const [columns] = (await client.database.execute(sql`
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'group_member_contacts'
        AND COLUMN_NAME IN (
          'mobile_phone_consent_fingerprint',
          'mobile_phone_consent_notice_version',
          'mobile_phone_consented_at',
          'mobile_phone_consent_revoked_at'
        )
    ...
    expect(columns).toEqual([
      {
        columnName: 'mobile_phone_consent_fingerprint',
        columnType: 'char(64)',
        datetimePrecision: null,
        isNullable: 'YES',
      },
      {
        columnName: 'mobile_phone_consent_notice_version',
        columnType: 'varchar(32)',
        datetimePrecision: null,
        isNullable: 'YES',
      },
      {
        columnName: 'mobile_phone_consent_revoked_at',
        columnType: 'timestamp(3)',
        datetimePrecision: 3,
        isNullable: 'YES',
      },
      {
        columnName: 'mobile_phone_consented_at',
        columnType: 'timestamp(3)',
        datetimePrecision: 3,
        isNullable: 'YES',
      },
  ...
  it('creates directory snapshot, prefix, and Chinese ngram search constraints', async () => {
    ...
    expect(createStatement).toContain('`employee_code` varchar(64)');
    expect(createStatement.toLowerCase()).toContain('with parser `ngram`');

    const [aliasTable] = (await client.database.execute(
      sql`SHOW CREATE TABLE directory_search_aliases`,
    )) as unknown as [readonly Record<string, string>[], unknown];
    const aliasCreateStatement = Object.values(aliasTable[0] ?? {}).join('\n');
    expect(aliasCreateStatement).not.toContain("'t9'");
  ...
  it('maps persisted group defaults and nullable member calendar overrides', () => {
