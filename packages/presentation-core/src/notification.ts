...
export type ReminderHoursMode = 'custom' | 'default' | 'off';

export interface NotificationRecordLike {
  readonly id: string;
  readonly isRead: boolean;
  readonly shiftAssignmentId?: string | undefined;
}

export interface NotificationSettingsRoleLike {
  readonly isDeveloperAdmin?: boolean | undefined;
...
export function findNewUnreadNotifications<Notification extends NotificationRecordLike>(
...
export function getNotificationTargetUrl(notification: NotificationRecordLike): string {
...
export function getReminderHoursMode(hours: readonly number[] | null): ReminderHoursMode {
  return hours === null ? 'default' : hours.length === 0 ? 'off' : 'custom';
}

export function resolveReminderHours(
  mode: ReminderHoursMode,
  customInput: string,
): readonly number[] | null {
  return mode === 'default' ? null : mode === 'off' ? [] : parseReminderHoursInput(customInput);
}

export function canManageNotificationSettings(group: NotificationSettingsRoleLike): boolean {
  return (
