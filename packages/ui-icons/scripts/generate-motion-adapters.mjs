...
function renderContextBinding(binding) {
  ...
  const width = `${cssNumber(context.containerWidthPx)}px`;
  const height = `${cssNumber(context.containerHeightPx)}px`;
  const stroke = cssNumber(context.strokeWidth);
  if (binding.mode === 'action-variables') {
    return `${binding.selector} {\n  --action-motion-icon-size: ${width};\n  --action-motion-icon-stroke-width: ${stroke};\n  --action-motion-icon-transform-origin: ${context.transformOrigin};\n  --action-motion-icon-optical-offset-x: ${cssNumber(context.opticalOffset[0])}px;\n  --action-motion-icon-optical-offset-y: ${cssNumber(context.opticalOffset[1])}px;\n}`;
  }
  const declarations = [
    `width: ${width};`,
    `height: ${height};`,
    `transform-origin: ${context.transformOrigin};`,
  ];
