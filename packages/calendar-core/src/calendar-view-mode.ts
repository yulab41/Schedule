import { addBusinessMonths, parseBusinessDate, parseBusinessMonth } from './calendar-logic.js';
import {
  addWeeks,
  getBusinessMonthOf,
...
function validateCalendarViewModeState(state: CalendarViewModeState): void {
  parseBusinessMonth(state.businessMonth);
  parseBusinessDate(state.weekStart);
}
...
export function goCalendarToToday(
  state: CalendarViewModeState,
  today: string,
): CalendarViewModeState {
  validateCalendarViewModeState(state);
  return {
    ...state,
    businessMonth: getBusinessMonthOf(today),
    weekStart: getWeekStartDate(today),
...
export function goCalendarToThisWeek(
  state: CalendarViewModeState,
  today: string,
): CalendarViewModeState {
  validateCalendarViewModeState(state);
...
export function goCalendarToBusinessMonth(
  state: CalendarViewModeState,
