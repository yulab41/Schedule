...
export function readBenchmarkDatabaseConfig(environment = process.env) {
  const config = {
    database: environment.DIRECTORY_BENCHMARK_DATABASE ?? 'schedule_directory_benchmark',
    host: environment.DIRECTORY_BENCHMARK_HOST ?? '127.0.0.1',
    password: environment.DIRECTORY_BENCHMARK_PASSWORD ?? 'local-benchmark-app',
    port: Number(environment.DIRECTORY_BENCHMARK_PORT ?? 3317),
    user: environment.DIRECTORY_BENCHMARK_USER ?? 'schedule_directory_benchmark',
  };
  if (!['127.0.0.1', '::1', 'localhost'].includes(config.host)) {
    throw new Error('Directory benchmark database host must be local.');
  }
  if (config.database !== 'schedule_directory_benchmark' || config.port !== 3317) {
