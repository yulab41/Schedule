...
6. 验证回归：`pnpm verify` 全通过；涉及核心链路（API client、认证/会话、路由、PWA/离线、数据库迁移）追加浏览器/运行环境冒烟验证；测试失败时不得改测试来掩盖，要报告失败详情。
...
4. **真实运行环境验证**：涉及 API client、认证/会话、路由、PWA/离线、数据库迁移等核心链路时，单测之外必须用真实浏览器（Playwright / 无头 Edge / Chrome，或手动点击）走一遍受影响路径，并把验证命令与结果写入轮次记录；未执行浏览器验证的轮次只能标“已实现待浏览器复核”，不得标“已完成”。
