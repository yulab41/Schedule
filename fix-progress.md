...
> 本文件由 2026-08-06 全库合规审查报告整理生成，是 Web 1.0 合规修复阶段的**新对话唯一切入点**。
> 审查报告全文（每个问题的四项铁律、文件、行号、原文描述、严重度与建议）已并入第 7 节附录；本文件不再依赖对话记录。
> 证据行号以审查时 HEAD `69e843e`（`main` 与 `origin/main` 同步）为准；修复轮中行号会变化，请以第 7 节原始证据 + `rg` 定位为准，不要凭旧行号臆断。
> 修复轮只做第 2/3 节编号对应的问题簇；清单外发现记入第 6 节，不顺手改。

## 0. 基线

- 审查基线（2026-08-06）：`git status` 干净、`main` 与 `origin/main` 同步；测试基线 `pnpm verify` 420/420（62 个测试文件，隔离 MySQL）；无已提交密钥；ESLint/Prettier/类型检查均严格开启。
- 跟踪文件数：审查报告记 384；复核 `git ls-files`（HEAD `69e843e`）为 **383**，本文件提交后为 384。附录保留报告原文表述，实际计数以复核为准。
- 审查结论：致命问题 0 项；严重 6 项；轻微约 20 项；健康度 6.5/10。严重等级汇总见 7.11，健康度评分见 7.12。
...
2. 理解目标：读第 3 节对应编号卡片（含证据位置与原文问题），再对照第 7 节附录同一编号的完整描述，明确文件与预期行为。
3. 制定方案：不超过 5 句话说明修改计划。
4. 执行修改：只改目标问题；**发现其他问题不要顺手改**，记入第 6 节 TODO（新发现）回报。
...
|---|---|---|---|---|---|---|
| P0 | #3.1 | starts_at 环境补丁散落 12 处 | 严重 | 中 | 极高（防线上数据损坏复发） | 用户点名，先写锁定测试 |
| P0 | #4.1 | swap/duty 服务复制（loadMembers/loadRoleNames） | 严重 | 中 | 极高 | 用户点名，先写锁定测试 |
| P1 | #7.3 | 中国时区算法在 9 个文件重复 | 严重 | 中低 | 高（时区事故反复出现） | 抽单一工具后机械替换；#2.3/#8.2 并入 |
...
| P1 | #3.4 | AUTH_DEV_MODE 无 NODE_ENV 防线 | 轻微 | 低 | 中（安全） | 显式双条件 + env schema；#1.2 随行 |
| P1 | #7.5 | event-timeline 死代码（含 spec 续命） | 轻微 | 极低 | 中 | 删函数/字段/样式并同步 spec |
| P1 | #2.1 | ui-tokens CSS/TS 双份维护 | 严重 | 低 | 中高 | 单一来源生成 |
| P2 | #7.1 | client.ts 2200 行手写校验器 + 重复请求函数 | 严重 | 高 | 极高 | 必须拆子步骤，见第 3 节；#7.2 并入 7.1d |
...
| P2 | #2.2 | 数据库客户端 mysql2 内部属性 + 异步设时区 | 轻微 | 低中 | 中高（防时区事故复发） | 审查补充项；先写“连接后时区已生效”锁定测试 |
...
| P3 | #5.2 | group-recycle 27 条裸 SQL 手写级联 | 轻微 | 低中 | 低中 | schema 元数据生成或外键级联；**需用户确认** |
...
| P3 | #7.2 | 客户端错误码表与契约联合类型双份维护 | 轻微 | 低 | 低 | 并入 #7.1d |
| P3 | #2.3 | 领域层时区常量与 Web 端重复 | 严重（与 #7.3 合并计） | 零 | 低 | 并入 #7.3 |
| P3 | #8.2 | 视图层裸写时区/日期魔法数 | 轻微 | 零 | 低 | 并入 #7.3 |

### 2.1 非代码待办 / 外部阻塞（记录在案，不属于修复轮）

- 草稿过期提醒（设计 16.2）未实现：project-status.md 已标记“留待后续轮次”，代码中无对应实现（附录 7.10）。
- CAM 密钥轮换 TODO：运维残留、非代码；见 project-status.md 与 `docs/deployment/production-readiness.md`（附录 7.10）。
- CloudBase 余额不足导致 `/api/health` 不可用：外部阻塞，需用户充值后重新触发 Deploy Development 并验证（附录 7.10）。
- “Fastify 415”已在轮次 56 修复，仅剩文档残留，归入 #1.1（附录 7.10）。

## 3. 问题簇详情（执行时按此卡片理解目标；完整原文见第 7 节附录）

### #3.1 starts_at 环境补丁收敛（P0）
- 严重度：严重（报告 3.1）；违反原则：①不打补丁 ②代码自解释。
- 证据（审查时 HEAD `69e843e`，12 处）：
  - `apps/api/src/modules/schedules/schedule-repository.ts:770`（770/824/888）
  - `apps/api/src/modules/swaps/swap-service.ts:633`（633/642/1269/1278）
  - `apps/api/src/modules/duty-adjustments/duty-adjustment-service.ts:1097`（1097/1193）
  - `apps/api/src/modules/leaves/leave-service.ts:622`
  - `apps/api/src/modules/groups/membership-service.ts:1005`（1005/1018）
- 问题：为绕开 CynosDB `explicit_defaults_for_timestamp=OFF` 导致的 `ON UPDATE CURRENT_TIMESTAMP` 漂移，每个 `shift_assignments` 更新都手写 `` `startsAt: sql`${shiftAssignments.startsAt}` ``。这是环境特判补丁，5 个文件 12 处重复且无任何注释；迁移 0018/0020 已修库结构，这些补丁仍留在代码里。任何“精简”该字段的后续重构都会静默重演轮次 10/13 的线上数据损坏。
- 先做：新增“批量更新 shift_assignments 不改变 starts_at”锁定测试（锁定当前行为）。
- 做法：收敛为一个 `updateShiftAssignmentsPreservingTimestamps` 仓储助手（或统一在 schema 层处理），加“为什么”注释与回归测试说明为何必须保留该列，删除散落补丁。
- 症状预警：排班时间漂移 8 小时/被改成当前时间、唯一键冲突 500。

### #4.1 换班/加扣班服务复制（P0）
- 严重度：严重（报告 4.1）；违反原则：①不打补丁。
- 证据：
  - `apps/api/src/modules/swaps/swap-service.ts:1642`（loadMembers 1642–1692、loadRoleNames 1694–1711）
  - `apps/api/src/modules/duty-adjustments/duty-adjustment-service.ts:1425`（loadMembers 1425–1475、loadRoleNames 1477–1493）
- 问题：两个文件里的 loadMembers 几乎逐行相同（唯一差异是 autoAcceptSwaps 默认值 1 vs 0），loadRoleNames 完全一致；该差异无注释说明是否为刻意设计，看起来就是“复制后微调参数”。两文件各 1600–1900 行，任何一处修复都容易漏改另一处。
- 先做：为“成员读取 + autoAcceptSwaps 默认值（swap 默认 1 / duty 默认 0）+ 角色名读取”各写一条锁定测试。
- 做法：把成员/角色读取提升到共享服务（WorkflowConflictService 旁或新 GroupMemberReader），差异字段用显式参数表达并注释原因。
- 症状预警：换班/加扣班自动接受逻辑翻转、成员资格判断不一致。

### #7.3 中国时区算法在 9 个文件重复（P1）
- 严重度：严重（报告 7.3；#2.3/#8.2 合并计）；违反原则：①不打补丁 ②代码自解释。
- 证据：
  - `apps/web/src/features/calendar/calendar-logic.ts:7`、`apps/web/src/features/calendar/calendar-views.ts:10`
  - `apps/web/src/features/swaps/swap-logic.ts:12`、`apps/web/src/features/duty-adjustments/duty-adjustment-logic.ts:12`
  - `apps/web/src/features/leaves/leave-logic.ts:9`、`apps/web/src/features/events/event-timeline.ts:10`
  - 裸写 `8 * 60 * 60 * 1000`：`apps/web/src/views/statistics/StatisticsView.vue:34`、`apps/web/src/features/exports/ExportDialog.vue:112`、`apps/web/src/views/schedules/ManualScheduleView.vue:855`
  - 领域包已有正确实现：`packages/scheduling-domain/src/time.ts:1`（`getChinaStandardTimeBusinessDate`）
- 问题：同一“中国业务日期”算法在 9 个文件重复，部分写成裸 `8 * 60 * 60 * 1000` 魔法数；后端领域包已有正确实现却不被复用。轮次 12/18 的时区误判事故正是这类重复逻辑产生的。
- 先做：对“今天/本月/事件时间格式化”现有单测即为锁定测试；替换后全量回归。
- 做法：新建共享纯函数包（如 `packages/business-time`，或让 web 依赖 scheduling-domain 的纯函数），机械替换，不改任何业务语义；视图层裸写（#8.2）一并消除。
- 症状预警：日期/时区判断整体偏移 8 小时；若改动语义则“服务返回了无效资料”类提示回归。

### #3.2 日志/审计脱敏清单双份且已漂移（P1）
- 严重度：严重（报告 3.2）；违反原则：①不打补丁 ③不留残渣。
- 证据：`apps/api/src/app.ts:57`（57–70、217–247）；`apps/api/src/modules/audit/audit-writer.ts:6`（6–18、44–77）。
- 问题：sensitiveLogFields 与 sensitiveFields 是两套独立清单 + 两套几乎相同的 normalizeFieldName/递归脱敏实现，且清单已不一致（审计含 telephone，日志不含）。新增敏感字段需同时改两处，漏改即可能造成日志泄露。
- 做法：把敏感字段清单与脱敏工具抽成单一共享模块（如 packages/contracts 或 api 内 `security/redact.ts`），日志与审计共用，并加一致性测试。
- 症状预警：新增敏感字段后日志仍输出明文（泄露）；或误脱敏业务字段。

### #1.1 文档过期/自相矛盾（P1）
- 严重度：轻微（报告 1.1）；违反原则：③不留残渣。
- 证据：`docs/debug/debug-feedback-log.md:660`（660、663 行）；`docs/project-status.md:17`（17、31 行）。
...
- 做法：以当前代码和 Git 历史为准重写“待办/下一步”，删除已解决条目（415、既往排班模块等）；project-status 轮次与状态同步修正。

### #3.3 任务分派默认分支静默落到导出任务（P1）
- 严重度：轻微（报告 3.3）；违反原则：①不打补丁 ②代码自解释。
- 证据：`apps/api/src/jobs/runner.ts:38`（38–59 行）。
- 问题：JobName 联合类型有 7 个值，if 链覆盖 6 个，最后一个 `return new ExportJobProcessor(client).run()` 作为“兜底”。未来新增任务类型时，忘记加分支会静默执行导出任务而不是报错。
- 做法：改为 switch 穷举并在 default 抛错，或用映射表 `Record<JobName, Factory>`。
- 症状预警：新增任务类型后实际执行导出任务（数据/通知行为错误）。

### #3.4 AUTH_DEV_MODE 无环境防线（P1）
- 严重度：轻微（报告 3.4）；违反原则：④部署一致。
- 证据：`apps/api/src/runtime.ts:30`（30–33 行）；`apps/api/src/adapters/auth/dev-auth.ts`。
- 问题：`process.env.AUTH_DEV_MODE === 'true'` 直接选择“任意 Bearer token 即身份”的认证端口，没有 `NODE_ENV !== 'production'` 防线，也没有 schema 校验。当前 CloudBase 网关路径恰好绕开它，但这是“一旦重构路由就可能上线”的隐形后门。
- 做法：显式加 `NODE_ENV === 'development'` 双条件，把该变量纳入 env.ts 校验，并同步 `.env.example`（#1.2 随行）。
- 症状预警：线上环境一旦路由重构，任意 token 可通过认证。

### #7.5 事件时间线死代码（P1）
- 严重度：轻微（报告 7.5）；违反原则：③不留残渣。
- 证据：`apps/web/src/features/events/event-timeline.ts:133`（getEventRelationLabel 133–135；buildSwapChainSummary 399–429；buildDutyAdjustmentChainSummary 431–468；isCorrection 字段 20/491 行）；`apps/web/src/features/events/EventTimeline.vue:188`（188–200 `.entry-relation` 样式）；另有 NoopPushDispatcher、`apps/web/src/api/client.ts:4203` isUndefined。
- 问题：轮次 19/21 把“更正/撤销”标签从界面移除后，相关函数、isCorrection 计算、.entry-relation 样式全部保留，仅被 spec 引用“续命”；getEventMarker 也只在 spec 中被直接调用。典型“留一手”。
- 做法：删除生产无引用导出与样式（含 NoopPushDispatcher、isUndefined），同步删除/改写对应 spec；`rg` 验证无引用。

### #2.1 ui-tokens CSS/TS 双份维护（P1）
- 严重度：严重（报告 2.1）；违反原则：①不打补丁 ③不留残渣。
- 证据：`packages/ui-tokens/src/tokens.css`（1–56 行）；`packages/ui-tokens/src/index.ts`（3–70 行）。
- 问题：同一组颜色/间距/字号同时存在于 CSS 自定义属性和 TS 常量两份文件，靠人工保持一致；没有生成器或单一来源。
- 做法：从一份 token 定义（如 JSON/TS）生成 .css 与 TS 导出，或删除其一；保留现有对比测试并改为“生成结果”断言。

### #7.1 client.ts 2200 行手写校验器 + 重复请求函数（P2，必须拆子步骤）
- 严重度：严重（报告 7.1）；违反原则：①不打补丁 ②代码自解释。
- 证据：`apps/web/src/api/client.ts:1903`（1903–1942、1944–2002、2004–2057 三个几乎相同的请求函数；2059–4201 约 113 个手写 isX 守卫；4203–4205 从未使用的 isUndefined）。
- 问题：client.ts 共 4221 行，其中约 51% 是手写类型守卫样板。契约层加字段、改字段时，这里必须人工同步，否则线上表现为“服务返回了无效资料”（本项目已多次出现）；三个请求函数也只是复制后微调。这是整个代码库最大的“复制粘贴后微调参数”实例。
- 子步骤（每轮一个，保持 `pnpm verify` 全绿；任何一步失败即回滚该分支）：
  - 7.1a：引入共享运行时校验（zod schema 或代码生成），先支持 1–2 个响应类型并加测试。
  - 7.1b：批量迁移响应校验到共享 schema。
  - 7.1c：删除手写 `isX` 守卫与 `isUndefined`，合并三个重复 request 函数。
  - 7.1d：`knownApiErrorCodes` 改由 contracts schema 派生（即 #7.2）。
- 症状预警：迁移中某响应校验过严/过松，表现为“服务返回了无效资料”或放行畸形数据。

### #7.4 前端 swap/duty 候选逻辑重复（P2）
- 严重度：轻微（报告 7.4）；违反原则：①不打补丁。
- 证据：`apps/web/src/features/swaps/swap-logic.ts:109`（isFutureAssignment 109–112）；`apps/web/src/features/duty-adjustments/duty-adjustment-logic.ts:47`（47–50）。
- 问题：isFutureAssignment 完全相同，buildXxxCandidates 的“过滤未来班次/按当前值班人分组”高度相似；两文件还各自复制了状态标签表与“下一步状态说明” switch。
- 做法：把“未来班次过滤 + 候选构建 + 状态说明”合并为共享 feature 逻辑。

### #8.1 15 个页面重复错误文案模板（P2）
- 严重度：轻微（报告 8.1）；违反原则：①不打补丁。
- 证据：`apps/web/src/views/schedules/ManualScheduleView.vue:825` 及 `apps/web/src/views/calendar/CalendarView.vue:241`、`apps/web/src/features/swaps/SwapPanel.vue:494` 等 15 个文件。
- 问题：每个页面重复 `error instanceof ApiClientError ? error.message : '...暂时无法加载，请稍后重试。'` 模板，文案高度雷同但各有微调，属于前端版“复制粘贴后微调”。
- 做法：抽 `toUserMessage(error, fallback)` 工具并统一文案。

### #3.7 框架 4xx 全部归一化为 VALIDATION_FAILED（P2）
- 严重度：轻微（报告 3.7）；违反原则：①不打补丁。
- 证据：`apps/api/src/plugins/error-handler.ts:75`（75–103 行）。
- 问题：除 415 外，任何框架 4xx（400/404/429 等）一律映射为 VALIDATION_FAILED，丢失语义；这是轮次 56 修复时留下的“只修 415”的窄补丁。
- 做法：按状态码映射到契约错误码（400→VALIDATION_FAILED、404→NOT_FOUND、429→RATE_LIMITED），并为每个映射补测试。

### #2.2 数据库客户端 mysql2 内部属性 + 异步设时区（P2）
- 严重度：轻微（报告 2.2）；违反原则：①不打补丁。
- 证据：`packages/database/src/client.ts:23`（23–41 行）。
- 问题：`pool.pool.on(...)` 依赖 mysql2/promise 的私有底层对象；`SET time_zone='+00:00'` 在连接事件里异步执行，未等待完成，早发的查询可能落在服务器默认时区上，造成 NOW()/CURRENT_TIMESTAMP 不一致（本项目已历过时区/时间戳类事故）。
- 先做：写“连接建立后时区已生效”锁定测试（锁定当前行为）。
- 做法：在连接建立回调中同步完成时区设置（或用 drizzle 连接初始化选项），并加注释说明动机。
- 症状预警：偶发 NOW() 偏移 8 小时（与服务器默认时区差异）。

### #4.2/#4.3 冲突断言双轨 + 三服务“上帝对象”状态机（P2，大簇）
- 前置：#4.1 完成后再做。
- 严重度：轻微（报告 4.2/4.3）；违反原则：①不打补丁。
- 证据：
  - 4.2：`apps/api/src/modules/swaps/swap-service.ts:1590`（assertNoActiveWorkflowConflicts/assertNoSwapConflicts）；`apps/api/src/modules/duty-adjustments/duty-adjustment-service.ts:1381`（assertNoActiveWorkflows/assertNoDutyAdjustmentConflicts）。
  - 4.3：`apps/api/src/modules/swaps/swap-service.ts:87`、`apps/api/src/modules/duty-adjustments/duty-adjustment-service.ts:81`、`apps/api/src/modules/leaves/leave-service.ts:128`。
- 问题：WorkflowConflictService 已统一查询逻辑，但每个服务又各自包装成“预览冲突”“事务内断言”两套入口，消息文案与结构互不相同，仍属半重构残留（4.2）；三个 1000+ 行服务各自实例化同样的依赖、各自维护 runXxx 状态机，事件与通知写入参数大量重复（换班/加扣班各 7 个事务入口）（4.3）。
- 做法：统一为一个“预检 + 提交前断言”接口，冲突消息由共享服务生成（4.2）；把状态机（创建/接受/审批/拒绝/取消/撤销）抽成可复用的工作流骨架，服务只提供领域差异（4.3）。分多轮，每轮一个状态转移。

### #4.5 发布/生成缺少“仅未来日期”限制（P3，需用户确认规则）
- 严重度：严重（功能缺口 + 规则不一致，报告 4.5）；违反原则：①不打补丁。
- 证据：`apps/api/src/modules/schedules/publish-service.ts`、`apps/api/src/modules/schedules/generate-service.ts`；对比 `apps/api/src/modules/manual-schedules/apply-service.ts:355`（355–365 已拦截）。
- 问题：手动模板应用已阻止过去起始日期，但发布与自动生成路径没有等价校验；这是 debug 日志明确标注的“原规划仍待处理”事项。
- 做法：把“起始日期不得早于今天”收敛为共享领域断言，并在发布/生成入口复用。
- 需用户确认（见第 4 节）：起始日=今天是否允许、跨已过月份的发布如何处理、与补录页边界。

### #7.6 manual-adjustment“调”标记半移除（P3，需用户确认）
- 严重度：轻微（报告 7.6）；违反原则：③不留残渣。
- 证据：`apps/api/src/modules/calendar/calendar-query.ts:33`（33–47）；`apps/web/src/features/calendar/calendar-logic.ts:168`（168–190）；`apps/web/src/features/calendar/MonthGrid.vue:55`。
- 问题：assignment_manually_updated 事件已无任何生产者（全库搜索仅剩查询映射与测试），但日历仍保留 manual-adjustment → '调' 映射、标签与渲染；调试日志却说“‘调’标记从所有日历移除”。旧事件数据会继续显示“调”，新代码不再产生，属半移除状态。
- 做法：明确决策——保留旧数据兼容就在代码注释与文档中说明，否则删除映射并对旧事件做迁移。需用户确认（见第 4 节）。

### #3.5 idempotency 宽 catch 当重复键（P3）
- 严重度：轻微（报告 3.5）；违反原则：①不打补丁。
- 证据：`apps/api/src/plugins/idempotency.ts:34`（34–52）。
- 问题：首次 insert 的任意异常都被当作“重复键”，随后再查再插。虽然异常会继续传播，但把“唯一键冲突”和“数据库故障”混为一谈，故障路径会做两次无谓的插入尝试。
- 做法：捕获后先检查 MySQL 1062 错误码，或使用 INSERT ... ON DUPLICATE KEY 语义后再读。

### #3.6 CloudBase 处理器惰性单例竞态（P3）
- 严重度：轻微（报告 3.6）；违反原则：①不打补丁。
- 证据：`apps/api/src/cloudbase-handler.ts:31`（31–50）。
- 问题：`runtimeApp ??= createCloudbaseRuntimeApp()` 在冷启动并发请求下可能创建多个 Fastify 实例与连接池，只有一个被保留，其余泄漏到函数实例结束。
- 做法：在模块顶层同步创建一次，或对创建过程做 Promise 单例化。

### #9.2 云函数入口跨目录相对导入（P3）
- 严重度：轻微（报告 9.2）；违反原则：④部署一致。
- 证据：`infra/cloudbase/functions/schedule-api/src/main.js:1`、`infra/cloudbase/functions/schedule-jobs/src/main.js:1`。
- 问题：`import ... from '../../../../../apps/api/src/...'` 让 infra 隐式依赖 apps 源码路径，esbuild 打包后才能运行；目录一旦调整，部署静默失败。这是跨模块隐式耦合。
- 做法：给 @schedule/api 暴露明确的 cloudbase 入口导出，函数包只依赖包名。

### #9.1 cloudbaserc 未声明 env，环境不可复现（P3）
- 严重度：轻微（报告 9.1）；违反原则：④部署一致。
- 证据：`infra/cloudbase/cloudbaserc.json`（全文无 env 字段）；`.github/workflows/deploy-development.yml`。
...
- 做法：把非密钥配置声明进 cloudbaserc.json（密钥走 GitHub Secrets/控制台），并在部署前加一次环境完整性检查。需与控制台协调。

### #6.1/#6.2 迁移 0030/0031 复制 + 序列一致性无校验（P3）
- 严重度：轻微（报告 6.1/6.2）；违反原则：①不打补丁。
- 证据：`migrations/0030_backfill_swap_workflow_sequence.sql`、`migrations/0031_backfill_duty_adjustment_workflow_sequence.sql`；`apps/api/src/modules/workflows/workflow-sequence-allocator.ts:5`、`migrations/0029_seed_workflow_sequence_allocations.sql`。
- 问题：两份 SQL 只差 UPDATE 目标表，排名字查询完全一样，各自从全量联合查询重新计算排名，未来再追加同类表需第三份复制品（6.1）；运行时序列来自自增 ID、历史回填来自 ROW_NUMBER()，两者靠“恰好都按创建顺序”保持一致，迁移里没有断言（6.2）。
- 做法：合并为一次 UPDATE ... JOIN 双表更新（或生成脚本），并注明排序必须与分配器一致（6.1）；在 0031 末尾加校验迁移或测试，确认新旧序列空间不重叠（6.2）。

### #5.1 notification-retry 的 skipped 恒为 0（P3）
- 严重度：轻微（报告 5.1）；违反原则：③不留残渣。
- 证据：`apps/api/src/jobs/notification-retry.ts:57`（57–70、92–164）。
- 问题：processDelivery 把“通知不存在/订阅失效/重试耗尽”全部归为 failed，skipped 只在“推送未配置”路径有值；接口保留四个字段，运维无法区分“发送失败”与“跳过”。
- 做法：让 processDelivery 返回 failed | sent | skipped 三元结果并如实计数。

### #5.2 group-recycle 27 条裸 SQL 手写级联（P3，需用户确认）
- 严重度：轻微（报告 5.2）；违反原则：①不打补丁 ④部署一致。
- 证据：`apps/api/src/jobs/group-recycle.ts:12`（12–57）。
- 问题：所有需要级联清理的表必须人工维护在这张列表里，新增含 group_id 的表极容易漏加；scanned 实际等于 purged，命名与语义不符。
- 做法：改用外键级联或从 schema 元数据生成删除顺序，并修正 scanned 语义。需用户确认是否允许外键级联（影响历史数据与线上库结构，见第 4 节）。

### #5.3 统计重建静默吞错（P3）
- 严重度：轻微（报告 5.3）；违反原则：①不打补丁。
- 证据：`apps/api/src/jobs/statistics-rebuild.ts:34`（34–46）。
- 问题：`catch { failed += 1 }` 不记录任何错误上下文，失败月份无法定位。
- 做法：把错误写入 platformJobRuns.summary 或结构化日志。

### #4.4 两个软删除函数几乎相同（P3）
- 严重度：轻微（报告 4.4）；违反原则：①不打补丁。
- 证据：`apps/api/src/modules/schedules/schedule-repository.ts:749`（749–779 与 801–833）。
- 问题：softDeleteAssignments 与 softDeleteAssignmentsBefore 仅差一个 lt(businessDate) 条件，整段 SQL 与更新语句重复。
- 做法：合并为一个带可选截止日期的函数。

### #1.2 .env.example 缺开发模式开关（P3，随 #3.4 一并做）
- 严重度：轻微（报告 1.2）；违反原则：④部署一致。
- 证据：`.env.example`（全文）；`.env`（gitignored）。
- 问题：本地 .env 使用 AUTH_DEV_MODE=true / VITE_AUTH_DEV_MODE=true，但 .env.example 没有任何说明，新环境无法复现本地开发配置；AUTH_DEV_MODE 也没有进后端 env.ts 的 schema，属“只存在于运行时魔法字符串”的配置。
- 做法：在 .env.example 补两个开关项，并在 env.ts 中用 z.literal/z.enum 声明。

### #1.3 工作区残留产物（P3）
- 严重度：轻微（报告 1.3）；违反原则：③不留残渣。
- 证据：cloudfunctions/auth-identity-probe（空目录）；infra/cloudbase/functions/*/index.js（约 10 万行/5 万行打包产物）；local-api*.log、local-web*.log；tests/load/node_modules、tests/security/node_modules、infra/scripts/node_modules、packages/database/dist 等。
- 问题：均被 .gitignore/infra/cloudbase/.gitignore 忽略、未污染仓库，但工作区里残留空目录、构建产物、日志和嵌套依赖，任何“全库扫描”都会先撞上它们。
- 做法：清理空目录与日志（构建产物交给 CI 生成），或在根目录补一条清理脚本；清理命令本身不入库。

### #9.3 加载/安全测试直写 DB 与 stub 认证（P3）
- 严重度：轻微（报告 9.3）；违反原则：①不打补丁。
- 证据：`tests/load/run-load-test.ts:524`（524 行起）；`tests/security/security.integration.test.ts:273`。
- 问题：加载测试自带一套建表/直插逻辑（AnyInsertTable）和 load-token-* 认证桩，与生产 repository 逻辑各自演化，属于测试侧“第二套实现”。
- 做法：复用 @schedule/database schema 与 API 入口构造测试数据，减少直写 SQL。

### #7.2 客户端错误码表与契约联合类型双份维护（P3，并入 #7.1d）
- 严重度：轻微（报告 7.2）；违反原则：①不打补丁。
- 证据：`apps/web/src/api/client.ts:435`（435–443）；`packages/contracts/src/errors.ts`。
- 问题：knownApiErrorCodes 用字面量重建了 ApiErrorCode 联合类型；契约新增错误码时，漏改这里会让合法错误被当成未知响应。
- 做法：用 z.enum/运行时 schema 从 contracts 直接派生，或加一个编译期一致性测试。随 #7.1d 完成。

### #2.3 领域层时区常量与 Web 端重复（P3，并入 #7.3）
- 严重度：严重（与 #7.3 合并计，报告 2.3）；违反原则：①不打补丁。
- 证据：`packages/scheduling-domain/src/time.ts:1`（第 1 行 `8 * 60 * 60 * 1000`）。
- 问题：领域包已提供 getChinaStandardTimeBusinessDate，但 Web 端仍各自复制常量与函数（详见 #7.3），领域层这个“正确实现”没有被复用。
- 做法：将中国时区日期工具作为唯一公共包导出给 Web 使用。随 #7.3 完成。

### #8.2 视图层裸写时区/日期魔法数（P3，并入 #7.3）
- 严重度：轻微（报告 8.2）；违反原则：②代码自解释。
- 证据：`apps/web/src/views/statistics/StatisticsView.vue:34`、`apps/web/src/features/exports/ExportDialog.vue:112`、`apps/web/src/views/schedules/ManualScheduleView.vue:855`。
...
## 7. 附录：2026-08-06 全库合规审查报告全文（权威证据基线）

> 以下为审查报告原文整理，编号与第 2/3 节一一对应。证据行号以审查时（HEAD `69e843e`、main 与 origin/main 同步）为准；修复轮中若行号漂移，以 `rg` 定位为准。

### 7.1 根目录 / 工程配置与文档

#### 7.1.1 文档自相矛盾、重复记录已修复事项
...
### 7.2 packages（契约 / 数据库 / 领域 / 设计令牌）

#### 7.2.1 设计令牌双份维护（CSS + TS）
...
#### 7.2.2 数据库客户端使用 mysql2 内部属性并异步设时区
...
### 7.3 apps/api 基础层（入口 / 插件 / 认证 / 错误处理）

#### 7.3.1 环境特判补丁：显式保留 starts_at 散落 12 处
...
#### 7.3.7 框架 4xx 全部归一化为 VALIDATION_FAILED
...
### 7.4 apps/api 业务模块（排班 / 工作流 / 换班 / 加扣班 / 请假）

#### 7.4.1 换班与加扣班服务大段复制粘贴
...
#### 7.4.3 换班/加扣班/请假服务全部自带 eventWriter/notificationWriter/permissionService 三件套
...
### 7.7 apps/web 基础层（API 客户端 / PWA / 路由）

#### 7.7.1 手工响应校验器 2200 行 + 三个重复请求函数
...
- 注：报告标题写“8 个前端文件”，正文与严重等级汇总按“9 个文件”（列出的调用点实际为 9 个）。
...
#### 7.7.5 事件时间线遗留轮次 19/21 移除 UI 后的死代码
...
#### 7.9.3 加载/安全测试内的数据库直写与 stub 认证重复业务规则
