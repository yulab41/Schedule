...
Project-related release packages, smoke screenshots, logs, debug output, and build scratch data must stay under this repository, normally in the ignored `runtime/` tree. The sole worktree-path exception is the persistent, same-volume, short Codex development pool managed by `scripts/codex/manage-worktree-pool.ps1`; its slots live outside the repository, retain independent `node_modules`, and must not be deleted at conversation boundaries. Do not create other sibling `Schedule-*` directories or persistent `schedule-*` items in the operating-system temporary directory. Keep only the latest reusable release worktree at `runtime/release-worktree`; remove superseded release/test/debug copies after confirming they are landed or disposable. Unlanded development worktrees must remain intact under `runtime/external-project-worktrees/`. Credentials, upload private keys, and production secrets are the only other required exception and must remain outside the repository.
