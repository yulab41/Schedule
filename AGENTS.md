...
- 只把实际成功读取的 Skills 当作知识约束，只把实际成功调用的工具写成“已使用”；不得编造工具、
  参数、日志或验证结果。
- 微信开发者工具是本仓库允许的 Agent 执行面。LLM 可以调用 `wechatide` CLI 与开发者工具 MCP，
  覆盖状态检查、登录、打开/关闭项目窗口、编译、构建 npm、模拟器、页面自动化、Console/Network、
  截图、推送预览/预览二维码和体验版上传。进入业务操作前按 `wechatide-skill` 的 CLI 就绪门禁检查
  一次 `check_wechatide_status`，确认 `openid` 且 `versionRelation` 兼容后即可继续；不要重复检查。
- 上述工具调用不需要用户逐次确认。仍需用户当次明确批准的只有：提交审核、撤回审核、正式发布，
  以及删除云资源、生产数据库破坏性写入、真实支付等其他不可逆操作。
- 允许使用仓库既有 Node 静态构建、`miniprogram-ci`、`miniprogram-simulate`、测试、包体和视觉比较
  脚本，也允许 Agent 操作微信开发者工具；但这些结果都不能代替实体设备验收。
- 每项证据必须标明来自静态检查、Node 自动化、Agent 操作的开发者工具（模拟器/自动化/截图）、
  用户人工开发者工具操作或小米 14 体验版。模拟器、自动化与截图不得冒充实体设备验收结论。
...
- 体验版上传由 Agent 自主完成，不再需要用户当次明确同意；但上传前仍必须在轮次记录中写明修改内容、
  短 SHA、版本描述、脏树状态和测试页面。版本分配、冻结候选、Manifest/血缘和 allowlist 等发布身份
  要求继续按 runbook 执行。提交审核、撤回审核和正式发布始终需要明确批准。
...
All project-related worktrees, release packages, smoke screenshots, logs, debug output, and build scratch data must stay under this repository, normally in the ignored `runtime/` tree. Do not create sibling `Schedule-*` directories or persistent `schedule-*` items in the operating-system temporary directory. Release/upload candidates use registered, exclusively leased healthy warm slots under `runtime/wt`; the retired `runtime/release-worktree` is not an approved upload path and must not be recreated or taken over to satisfy an old checker. Remove superseded release/test/debug copies only after confirming they are landed or disposable; preserve unidentified legacy remnants. Unlanded development worktrees must remain intact under `runtime/external-project-worktrees/`. Credentials, upload private keys, and production secrets are the only required exception and must remain outside the repository.
