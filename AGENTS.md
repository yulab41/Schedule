...
- 只把实际成功读取的 Skills 当作知识约束，只把实际成功调用的工具写成“已使用”；不得编造工具、
  参数、日志或验证结果。
- 默认禁止：LLM 不得自行调用、启动、唤醒、重启、控制或自动化微信开发者工具 GUI/CLI（含状态检查、
  登录、编译、模拟器、Console、Network、截图、预览和上传命令）。
- 人工授权例外（2026-09-16 用户确认）：当用户在当次消息中明确授权“临时用开发者工具调试”时，可执行
  只读调试操作——打开项目、切换基础库、编译、Console/Network、截图、元素与层级检查。每轮须在轮次
  记录写明授权来源与时间、用途、实际执行的操作、结果及是否产生外部状态变化；开发者工具结果属于
  “用户授权的人工工具层”证据，不能替代小米 14 体验版验收。
- 受限动作（先问后做）：上传体验版、预览发布、提交审核、正式发布、读取账号或凭证、任何生产操作，
  都必须先向用户说明内容并取得当次确认，不得自行执行；正式上传仍须走 Node 版 `miniprogram-ci` 流程。
- 仍禁止：无授权的自动调用；把凭证、二维码、Cookie 或 AppID 写入仓库与日志；用开发者工具结果充当
  原生验收；改变 3.17.3 既有路径或生产数据。
